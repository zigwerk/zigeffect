# zigeffect Causal App-Facing Ten-Level Application Boundary Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build the alias-named ten-level app-facing application boundary that consumes ten-level report artifacts and hands applied evidence to the ten-level policy branch.

**Architecture:** Promote the proven nine-level application-boundary tool into a ten-level alias file while preserving full schema lineage in emitted artifacts. The tool remains local, record-only, and read-only; only `record-applied` can set `applied=true`, and only after reviewed evidence, safe after-report content, and verification command gates pass.
...
**Files:**
- Create: `packages/zigeffect/tools/causal_app_facing_ten_level_application_boundary.zig`
...
zig test tools/causal_app_facing_ten_level_application_boundary.zig
```
...
**Files:**
- Modify: `packages/zigeffect/tools/causal_app_facing_ten_level_application_boundary.zig`

- [ ] **Step 1: Copy the nine-level application-boundary implementation into the alias file**
...
evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_after_present
...
```text
usage: zig build causal-app-facing-ten-level-application-boundary -- --from-report <ten-level-report.json> plan|record-applied
```
...
- [ ] **Step 1: Add alias module, executable, step, and tests after the ten-level report block**
...
Add the schema immediately after the ten-level report entry with:
...
```zig
pub const recommendation = "start-app-facing-ten-level-policy";
pub const recommended_next_branch = "codex/zigeffect-causal-app-facing-ten-level-policy";
```

Add delivered item id `app-facing-ten-level-application-boundary`, branch
`codex/zigeffect-causal-app-facing-ten-level-application-boundary`, dependency
`app-facing-ten-level-report`, and verification commands for plan,
...
**Files:**
- Create: `packages/zigeffect/docs/app-facing-ten-level-application-boundary.md`
- Create: `packages/zigeffect/test/fixtures/app-facing-ten-level-application-boundary-after-safe.txt`
...
This local evaluation report records reviewed ten-level application boundary
...
Mark `codex/zigeffect-causal-app-facing-ten-level-application-boundary` as
delivered and add `codex/zigeffect-causal-app-facing-ten-level-policy` as the
next branch.
...
- [ ] **Step 6: Commit and checkpoint master**

Run:

```bash
git add docs/superpowers packages/zigeffect
git commit -m "feat(zigeffect): add app-facing ten-level application boundary"
git -C /Users/seanknowles/.config/superpowers/worktrees/yachdee/master-local-merge merge --ff-only codex/zigeffect-causal-app-facing-ten-level-application-boundary
git switch -c codex/zigeffect-causal-app-facing-ten-level-policy
```

Expected: the feature branch is committed, local `master` advances by fast-forward,
and the next ten-level policy branch is ready.
