# Zigeffect App-Facing Eleven-Level Policy Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [x]`) syntax for tracking.

**Goal:** Build the alias-named eleven-level app-facing policy that consumes applied eleven-level application-boundary evidence and hands approved policy evidence to the eleven-level evaluator branch.

**Architecture:** Promote the proven ten-level policy tool into an eleven-level alias file while updating the source contract to the applied eleven-level application-boundary artifact. The tool remains local and read-only; `approve` emits ready evidence only after all source and verification gates pass, while `reject` and unsafe sources emit blocked evidence.
...
**Files:**
- Create: `packages/zigeffect/tools/causal_app_facing_eleven_level_policy.zig`
...
test "app-facing eleven-level policy constants are stable" {
    try std.testing.expectEqualStrings("zigeffect.causal.app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-policy.v1", schema);
    try std.testing.expectEqual(@as(u32, 1), schema_version);
    try std.testing.expectEqualStrings("codex/zigeffect-causal-app-facing-eleven-level-policy", source_branch);
    try std.testing.expectEqualStrings("start-app-facing-eleven-level-evaluator", recommendation);
    try std.testing.expectEqualStrings("codex/zigeffect-causal-app-facing-eleven-level-evaluator", next_branch_if_ready);
}
```
...
**Files:**
- Modify: `packages/zigeffect/tools/causal_app_facing_eleven_level_policy.zig`

- [x] **Step 1: Copy the ten-level policy implementation into the alias file**
...
- [x] **Step 4: Add source eleven-level, ten-level, and nine-level lineage**
...
- [x] **Step 1: Add alias module, executable, step, and tests after the eleven-level application-boundary block**
...
Add the schema immediately after the eleven-level application-boundary entry with:
...
```zig
pub const recommendation = "start-app-facing-eleven-level-evaluator";
pub const recommended_next_branch = "codex/zigeffect-causal-app-facing-eleven-level-evaluator";
```

Add delivered item id `app-facing-eleven-level-policy`, branch
`codex/zigeffect-causal-app-facing-eleven-level-policy`, dependency
`app-facing-eleven-level-application-boundary`, and verification commands for
help, approve, reject, and blocked-source generation.
...
**Files:**
- Create: `packages/zigeffect/docs/app-facing-eleven-level-policy.md`
- Modify: `docs/superpowers/specs/2026-06-08-zigeffect-causal-agent-runtime-master-roadmap.md`

- [x] **Step 1: Add docs**

The docs must include consumes/emits schemas, approve/reject usage, gates,
non-authority, and handoff to `codex/zigeffect-causal-app-facing-eleven-level-evaluator`.

- [x] **Step 2: Update the master roadmap**

Mark `codex/zigeffect-causal-app-facing-eleven-level-policy` as delivered and
add `codex/zigeffect-causal-app-facing-eleven-level-evaluator` as the next
branch.
...
git commit -m "feat(zigeffect): add app-facing eleven-level policy"
git -C /Users/seanknowles/.config/superpowers/worktrees/yachdee/master-local-merge merge --ff-only codex/zigeffect-causal-app-facing-eleven-level-policy
git switch -c codex/zigeffect-causal-app-facing-eleven-level-evaluator
```

Expected: the feature branch is committed, local `master` advances by
fast-forward, and the next eleven-level evaluator branch is ready.
