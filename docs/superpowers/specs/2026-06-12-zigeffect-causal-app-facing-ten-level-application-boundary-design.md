# zigeffect Causal App-Facing Ten-Level Application Boundary Design

## Context

The prior milestone delivered `causal-app-facing-ten-level-report` and handed
off to `codex/zigeffect-causal-app-facing-ten-level-application-boundary`.
That report emits:
...
ready or advisory ten-level report artifacts and emits local evidence that a
reviewed boundary step is either planned, applied, or blocked. It follows the
nine-level application-boundary pattern: `plan` records local intent only, while
`record-applied` requires reviewed change evidence, before/after evidence, safe
after-report content, and complete verification commands.

The branch is intentionally a checkpoint after local `master` has been moved to
the current ten-level report commit. The new work should therefore stay on the
short alias branch and can later be merged back to `master` as a complete
milestone.
...
- Docs: `packages/zigeffect/docs/app-facing-ten-level-application-boundary.md`
...
- Next branch if applied: `codex/zigeffect-causal-app-facing-ten-level-policy`
- Recommendation: `start-app-facing-ten-level-policy`
...
- source ten-level report path, schema, status, and readiness
- source nine-level evaluator, policy, application-boundary, and report refs
- inherited eight-level and lower-level report and application-boundary evidence
...
- next branch alias for the ten-level policy milestone
...
The matching after-report fields use the same ten-segment prefix:
...
- source schema equals the ten-level report schema and schema version is `1`
...
`emitted_by = causal-app-facing-ten-level-application-boundary`, consumed by
agents, reviewers, non-blocking CI advisory readers, the local SolidJS workbench,
the production-hardening backlog, and the future ten-level policy branch.

The production hardening backlog must mark
`app-facing-ten-level-application-boundary` as delivered, update the
recommendation to `start-app-facing-ten-level-policy`, and point the recommended
next branch to `codex/zigeffect-causal-app-facing-ten-level-policy`.

The master roadmap must mark the current branch delivered and add the ten-level
...
- default output prefix replaces ten-level report suffix and compacts long paths
