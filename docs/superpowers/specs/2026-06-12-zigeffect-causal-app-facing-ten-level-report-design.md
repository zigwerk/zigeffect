# Zigeffect App-Facing Ten-Level Report Design

## Context

The nine-level evaluator now emits ready, advisory, and blocked local evaluator
artifacts from approved nine-level policy evidence plus bounded request and
support evidence. Those artifacts are still interpretation evidence only: they
do not grant CI, GitHub, app runtime, storage, deployment, public upload,
production-health, auto-apply, registry, or mutation authority.

This milestone adds the matching ten-level report producer. It consumes a ready
or advisory nine-level evaluator artifact, summarizes the tenth
...
to the ten-level application-boundary branch.

## Design

Promote the existing nine-level report pattern into the next short alias while
...
because the expanded ten-level lineage is not ergonomic for filesystems or
branch handling.

- Tool: `packages/zigeffect/tools/causal_app_facing_ten_level_report.zig`
- Build step: `causal-app-facing-ten-level-report`
- Executable: `zigeffect-causal-app-facing-ten-level-report`
- Docs: `packages/zigeffect/docs/app-facing-ten-level-report.md`
- Current branch: `codex/zigeffect-causal-app-facing-ten-level-report`
...
The source evaluator uses the nine-level alias tool. The report emits the fully
expanded ten-level schema because evaluator-to-report handoff adds one more
...
- source schema equals the nine-level evaluator schema and version `1`;
- source evaluator status is `ready` or `advisory-findings`;
- source `ready_for_next_branch=true`;
- source has no blocked findings;
- source nine-level policy status is `ready`, source policy decision is
  `approve`, and source mutation authority remains `none`;
- source nine-level application-boundary and report refs are present;
- source nine-level report status is ready;
- inherited eight-level policy and application evidence stay visible;
- request/support summaries, signal summaries, checks, denied claims, next
  queries, and local publication channels are present;
...
1. Read the nine-level evaluator artifact from a local JSON file.
2. Parse the evaluator artifact with alias-aware source fields:
   `source_nine_level_policy`, `source_nine_level_application_boundary`,
   `source_nine_level_report`, and inherited eight-level refs.
...
5. Emit deterministic JSON and text ten-level report artifacts.
6. Hand ready or advisory report artifacts to the ten-level
   application-boundary branch.

## Governance

Schema governance must register the ten-level report schema with
`emitted_by = causal-app-facing-ten-level-report`. The production hardening
backlog must mark `app-facing-ten-level-report` delivered, update the
recommendation to `start-app-facing-ten-level-application-boundary`, and point
the next branch to
`codex/zigeffect-causal-app-facing-ten-level-application-boundary`.

Compatibility tags must preserve the current posture: `strict-v1`,
`record-only`, `advisory-only`, `source-nine-level-evaluator`,
...
- default output path replaces the nine-level evaluator suffix with the
  ten-level report suffix and compacts long paths;
...
- schema governance contains the ten-level report schema;
- production backlog recommendation moves to the ten-level application-boundary
  branch.
...
The real artifact check must generate ready, advisory, and blocked ten-level
report artifacts from:

- `.zig-cache/causal-artifacts/app-facing-ci-nine-level-evaluator.json`
- `.zig-cache/causal-artifacts/app-facing-ci-nine-level-evaluator-advisory.json`
- `.zig-cache/causal-artifacts/app-facing-ci-nine-level-evaluator-blocked.json`
...
This branch does not create the ten-level application-boundary tool, required
status checks, workflow mutations, GitHub API calls, PR comments, public
artifact uploads, app config or app data mutations, app runtime projections,
raw prompt or payload capture, durable storage writes, NenDB writes, NenDB
adapter execution, Cockroach work, deployments, production-health claims,
hosted dashboards, auto-apply behavior, registry mutation, or mutation
authority.
