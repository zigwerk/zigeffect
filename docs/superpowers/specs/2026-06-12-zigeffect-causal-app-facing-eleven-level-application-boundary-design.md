# zigeffect Causal App-Facing Eleven-Level Application Boundary Design

## Context

The prior milestone delivered `causal-app-facing-eleven-level-report` and handed
off to `codex/zigeffect-causal-app-facing-eleven-level-application-boundary`.
That report emits:
...
consumes ready or advisory eleven-level report artifacts and emits local
evidence that a reviewed boundary step is either planned, applied, or blocked.
It follows the established app-facing rhythm: report, application boundary,
policy, evaluator, next report. `plan` records local intent only, while
`record-applied` requires reviewed application-change evidence, before and
after evidence, safe after-report content, and complete verification commands.

The branch starts after local `master` has been fast-forwarded to the
eleven-level report milestone, so this work can remain isolated on the short
alias branch until it is a complete milestone.

## Physical Naming Constraint

The artifact schema remains fully expanded and authoritative. Physical files,
build steps, docs, generated artifact prefixes, and branches use aliases because
further expanded names are not ergonomic for filesystems, shells, branch tools,
or agent context.
...
- Docs: `packages/zigeffect/docs/app-facing-eleven-level-application-boundary.md`
...
- Next branch if applied: `codex/zigeffect-causal-app-facing-eleven-level-policy`
- Recommendation: `start-app-facing-eleven-level-policy`
...
- source eleven-level report path, schema, status, and readiness
- direct ten-level policy, application-boundary, and report refs from the
  eleven-level source report
- inherited nine-level policy, application-boundary, and report refs
...
- next branch alias for the eleven-level policy milestone
...
The application output fields use the eleven-segment application prefix:
...
- source schema equals the eleven-level report schema and schema version is `1`
- source report was generated by `causal-app-facing-eleven-level-report`
...
`emitted_by = causal-app-facing-eleven-level-application-boundary`, consumed by
agents, reviewers, non-blocking CI advisory readers, the local SolidJS workbench,
the production-hardening backlog, and the future eleven-level policy branch.

The production hardening backlog must mark
`app-facing-eleven-level-application-boundary` as delivered, update the
recommendation to `start-app-facing-eleven-level-policy`, and point the
recommended next branch to `codex/zigeffect-causal-app-facing-eleven-level-policy`.

The master roadmap must mark the current branch delivered and add the
eleven-level policy branch as the next milestone.
...
- default output prefix replaces eleven-level report suffix and compacts long
  paths
- plan mode emits planned state without applied authority
- plan mode carries direct ten-level and inherited nine-level lineage
