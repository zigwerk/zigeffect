...
pub const Service = struct {
    ...
    pub fn from(comptime Provider: type, provider: *Provider) Service {
        return .{
            .pointer = provider,
            .wall_fn = struct { fn call(raw: *anyopaque) u64 { return (@as(*Provider, @ptrCast(@alignCast(raw)))).wallMillis(); } }.call,
            .monotonic_fn = struct { fn call(raw: *anyopaque) i128 { return (@as(*Provider, @ptrCast(@alignCast(raw)))).monotonicNanos(); } }.call,
            .sleep_fn = struct { fn call(raw: *anyopaque, millis: u64) anyerror!void { return (@as(*Provider, @ptrCast(@alignCast(raw)))).sleepMillis(millis); } }.call,
        };
    }

    pub fn snapshot(self: Service) Snapshot { return .{ .wall_millis = self.wall_fn(self.pointer), .monotonic_nanos = self.monotonic_fn(self.pointer) }; }
    pub fn sleepMillis(self: Service, millis: u64) !void { return self.sleep_fn(self.pointer, millis); }
};

pub const RealClock = struct {
    pub const capability = Capability.Descriptor{
        .id = "zigeffect-std.clock.system", .kind = .clock, .maturity = .production_candidate,
        .package = "zigeffect-std", .version = "0.1.0", .features = &.{ "wall", "monotonic", "sleep" }, .side_effects = .real,
    ...
    io: std.Io,
    pub fn init(io: std.Io) RealClock { return .{ .io = io }; }
    pub fn asService(self: *RealClock) Service { return Service.from(RealClock, self); }
    ...
    pub fn monotonicNanos(self: *RealClock) i128 { return std.Io.Clock.awake.now(self.io).nanoseconds; }
...
pub const FakeClock = struct {
    ...
    pub fn asService(self: *FakeClock) Service { return Service.from(FakeClock, self); }
    pub fn wallMillis(self: *FakeClock) u64 { return self.current_millis; }
    pub fn monotonicNanos(self: *FakeClock) i128 { return @as(i128, self.current_millis) * std.time.ns_per_ms; }
    pub fn sleepMillis(self: *FakeClock, millis: u64) !void { self.sleep(millis); }
