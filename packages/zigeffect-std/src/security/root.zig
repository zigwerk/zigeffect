...
pub const SecretValue = struct {
    ...
    bytes: []u8,
    pub fn initAlloc(allocator: std.mem.Allocator, bytes: []const u8) !SecretValue {
        if (bytes.len == 0) return error.EmptySecret;
        return .{ .allocator = allocator, .bytes = try allocator.dupe(u8, bytes) };
    }
    pub fn eql(self: *const SecretValue, candidate: []const u8) bool {
        return secureEql(self.bytes, candidate);
    }
    pub fn display(_: *const SecretValue) []const u8 {
...
pub const Auth = fx.kernel.Service("zigeffect/std/AuthContext", AuthContext);

pub fn authLayer(context: AuthContext) @TypeOf(fx.kernel.Layer.succeed(Auth, context)) {
    return fx.kernel.Layer.succeed(Auth, context);
}
...
pub const SignedSession = struct {
    pub fn signAlloc(allocator: std.mem.Allocator, claims: SessionClaims, key: []const u8) ![]u8 {
        ...
        const payload = try std.json.Stringify.valueAlloc(allocator, claims, .{});
        defer allocator.free(payload);
        const Encoder = std.base64.url_safe_no_pad.Encoder;
        const encoded_len = Encoder.calcSize(payload.len);
        const output = try allocator.alloc(u8, encoded_len + 1 + Encoder.calcSize(32));
...
pub const JwtVerifier = struct {
    ...
    pub fn from(comptime T: type, pointer: *T) JwtVerifier {
        return .{ .pointer = pointer, .verify_fn = struct {
            fn call(raw: *anyopaque, allocator: std.mem.Allocator, token: []const u8, now_ms: u64) anyerror!AuthContext {
                return (@as(*T, @ptrCast(@alignCast(raw)))).verifyJwtAlloc(allocator, token, now_ms);
            }
        }.call };
    }
    pub fn verifyAlloc(self: JwtVerifier, allocator: std.mem.Allocator, token: []const u8, now_ms: u64) !AuthContext {
        if (token.len == 0 or token.len > 64 * 1024) return error.InvalidJwt;
        return self.verify_fn(self.pointer, allocator, token, now_ms);
    }
};
...
pub const AuthorizationPolicy = struct {
    ...
    pub fn from(comptime T: type, pointer: *T) AuthorizationPolicy {
        return .{ .pointer = pointer, .decide_fn = struct {
            fn call(raw: *anyopaque, context: AuthContext, request: AuthorizationRequest) AuthorizationDecision {
                return (@as(*T, @ptrCast(@alignCast(raw)))).authorize(context, request);
            }
        }.call };
    }
    pub fn decide(self: AuthorizationPolicy, context: AuthContext, request: AuthorizationRequest) AuthorizationDecision {
        return self.decide_fn(self.pointer, context, request);
    }
};
...
pub fn secureCookieAlloc(allocator: std.mem.Allocator, name: []const u8, value: []const u8, options: CookieOptions) ![]u8 {
    ...
    if (options.domain) |domain| {
        if (containsCookieControl(domain)) return error.InvalidCookie;
        try output.print(allocator, "; Domain={s}", .{domain});
    }
    ...
    try output.print(allocator, "; SameSite={s}", .{switch (options.same_site) {
        .strict => "Strict",
        .lax => "Lax",
        .none => "None",
    }});
...
pub const PasswordHasher = struct {
    ...
    pub fn from(comptime T: type, pointer: *T) PasswordHasher {
        return .{ .pointer = pointer, .hash_fn = struct {
            fn call(raw: *anyopaque, allocator: std.mem.Allocator, password: []const u8) anyerror![]u8 {
                return (@as(*T, @ptrCast(@alignCast(raw)))).hashPasswordAlloc(allocator, password);
            }
        }.call, .verify_fn = struct {
            fn call(raw: *anyopaque, encoded: []const u8, password: []const u8) anyerror!bool {
                return (@as(*T, @ptrCast(@alignCast(raw)))).verifyPassword(encoded, password);
            }
        }.call, .algorithm_fn = struct {
            fn call(raw: *anyopaque) []const u8 {
                return (@as(*T, @ptrCast(@alignCast(raw)))).algorithm();
            }
        }.call };
    }
};
...
pub const Argon2idHasher = struct {
    ...
    pub fn algorithm(_: *Argon2idHasher) []const u8 {
        return "argon2id";
    }
...
pub const RotationHook = struct {
    pointer: *anyopaque,
    rotate_fn: *const fn (*anyopaque, Secrets.Reference) anyerror!u64,
    pub fn from(comptime T: type, pointer: *T) RotationHook {
        return .{ .pointer = pointer, .rotate_fn = struct {
            fn call(raw: *anyopaque, reference: Secrets.Reference) anyerror!u64 {
                return (@as(*T, @ptrCast(@alignCast(raw)))).rotate(reference);
            }
        }.call };
    }
    pub fn rotate(self: RotationHook, reference: Secrets.Reference) !u64 {
        try reference.validate();
        return self.rotate_fn(self.pointer, reference);
    }
};

fn validCookieToken(value: []const u8) bool {
    if (value.len == 0 or value.len > 128) return false;
    for (value) |byte| if (!(std.ascii.isAlphanumeric(byte) or byte == '_' or byte == '-')) return false;
    return true;
}
fn containsCookieControl(value: []const u8) bool {
    for (value) |byte| if (byte < 0x21 or byte == ';' or byte == ',' or byte == 0x7f) return true;
    return false;
}
...
test "constant-time API key authentication propagates redacted auth context" {
    ...
    try context.validate(100);
    try std.testing.expectEqualStrings("service-a", context.subject);
    try std.testing.expectEqual(@as(u64, 4), context.credential_epoch);
...
test "signed sessions reject tampering expiry replay-shaped input and secrets" {
    ...
    const token = try SignedSession.signAlloc(std.testing.allocator, .{ .subject = "user-1", .issued_at_ms = 10, .expires_at_ms = 20, .nonce = "0123456789abcdef" }, key);
    defer std.testing.allocator.free(token);
    var parsed = try SignedSession.verifyAlloc(std.testing.allocator, token, key, 15);
    defer parsed.deinit();
...
test "secure cookies and CSRF reject hostile input" {
    const cookie = try secureCookieAlloc(std.testing.allocator, "session", "opaque-value", .{});
