...
pub const UnitOfWork = struct {
    ...
    pub fn execute(self: *UnitOfWork, allocator: std.mem.Allocator, statement: Sql.Statement) !void {
        if (self.finished) return error.UnitOfWorkFinished;
        return self.execute_fn(self.pointer, allocator, statement);
    }
    pub fn enqueue(self: *UnitOfWork, request: Enqueue) !Message {
        if (self.finished) return error.UnitOfWorkFinished;
        return self.enqueue_fn(self.pointer, request);
    }
    pub fn recordInbox(self: *UnitOfWork, request: InboxRecord) !InboxOutcome {
        if (self.finished) return error.UnitOfWorkFinished;
        return self.inbox_fn(self.pointer, request);
    }
    pub fn commit(self: *UnitOfWork) !void {
        if (self.finished) return;
        try self.commit_fn(self.pointer);
        self.finished = true;
    }
    pub fn rollback(self: *UnitOfWork) !void {
        if (self.finished) return;
        try self.rollback_fn(self.pointer);
        self.finished = true;
    }
    pub fn deinit(self: *UnitOfWork) void {
        if (!self.finished) self.rollback_fn(self.pointer) catch {};
...
pub const Store = struct {
    ...
    pub fn begin(self: Store, allocator: std.mem.Allocator) !UnitOfWork {
        return self.begin_fn(self.pointer, allocator);
    }
    pub fn claimNext(self: Store, now_ms: u64) !Message {
        return self.claim_fn(self.pointer, now_ms);
    }
    pub fn markDelivered(self: Store, id: u64, now_ms: u64) !void {
        return self.delivered_fn(self.pointer, id, now_ms);
    }
};
