...
pub const TimeoutPolicy = struct {
    duration_ms: u64,
    pub fn validate(self: TimeoutPolicy) !void {
        if (self.duration_ms == 0) return error.InvalidTimeoutPolicy;
    }
    pub fn deadline(self: TimeoutPolicy, started_ms: u64) u64 {
        return std.math.add(u64, started_ms, self.duration_ms) catch std.math.maxInt(u64);
    }
    pub fn expired(self: TimeoutPolicy, started_ms: u64, now_ms: u64) bool {
        return now_ms >= self.deadline(started_ms);
    }
};
...
pub const CircuitBreaker = struct {
    ...
    pub fn allow(self: *CircuitBreaker, now_ms: u64) bool {
        if (self.state == .open and now_ms -| self.opened_at_ms >= self.policy.open_ms) {
    ...
    pub fn success(self: *CircuitBreaker) void {
        switch (self.state) {
            ...
            .half_open => {
                ...
                if (self.half_open_success_count >= self.policy.half_open_successes) {
                    self.state = .closed;
...
pub const Bulkhead = struct {
    ...
    in_flight: usize = 0,
    rejected: usize = 0,
    pub fn init(max_in_flight: usize) !Bulkhead {
        if (max_in_flight == 0) return error.InvalidBulkhead;
        return .{ .max_in_flight = max_in_flight };
    }
    pub fn acquire(self: *Bulkhead) !void {
...
pub const RateLimiter = struct {
    ...
    last_refill_ms: u64,
    rejected: usize = 0,
    pub fn init(capacity: u64, refill_per_second: u64, now_ms: u64) !RateLimiter {
        if (capacity == 0 or refill_per_second == 0) return error.InvalidRateLimit;
        return .{ .capacity = capacity, .refill_per_second = refill_per_second, .tokens = capacity, .last_refill_ms = now_ms };
    }
...
pub const Evidence = struct {
    ...
    pub fn deinit(self: *Evidence) void {
        for (self.transitions.items) |item| self.allocator.free(item.component);
        self.transitions.deinit(self.allocator);
    }
    pub fn record(self: *Evidence, transition: Transition) !void {
        var owned = transition;
        owned.component = try self.allocator.dupe(u8, transition.component);
        try self.transitions.append(self.allocator, owned);
    }
    pub fn workbenchJsonAlloc(self: *const Evidence, allocator: std.mem.Allocator) ![]u8 {
        return Secrets.safeJsonAlloc(allocator, .{ .schema = "zigeffect.resilience.transitions.v1", .transitions = self.transitions.items }, .{});
    }
};
    ...
    pub fn retry(self: Driver, component: []const u8, policy: RetryPolicy, failure: External.Failure, completed_attempts: usize) !RetryDecision {
        ...
        switch (decision) {
            .retry_after_ms => |delay| {
                if (self.evidence) |target| try target.record(.{ .kind = .retry_scheduled, .component = component, .at_ms = now, .failure_class = failure.class });
                try self.clock.sleepMillis(delay);
            },
...
pub const ManagedService = struct {
    ...
    pub fn from(comptime T: type, name: []const u8, pointer: *T) ManagedService {
        return .{ .name = name, .pointer = pointer, .ready_fn = struct {
            fn call(raw: *anyopaque) bool {
                return (@as(*T, @ptrCast(@alignCast(raw)))).readiness();
            }
        }.call, .drain_fn = struct {
            fn call(raw: *anyopaque) anyerror!void {
                return (@as(*T, @ptrCast(@alignCast(raw)))).drain();
            }
        }.call, .stop_fn = struct {
            fn call(raw: *anyopaque) anyerror!void {
                return (@as(*T, @ptrCast(@alignCast(raw)))).shutdown();
            }
        }.call };
    }
};
...
pub const ServiceCoordinator = struct {
    ...
    pub fn init(allocator: std.mem.Allocator) ServiceCoordinator {
        return .{ .allocator = allocator };
    }
    pub fn deinit(self: *ServiceCoordinator) void {
    ...
    pub fn register(self: *ServiceCoordinator, service: ManagedService) !void {
        if (service.name.len == 0) return error.InvalidServiceName;
        try self.services.append(self.allocator, service);
    }
    pub fn ready(self: *const ServiceCoordinator) bool {
        for (self.services.items) |service| if (!service.ready_fn(service.pointer)) return false;
        return true;
    }
    pub fn drain(self: *ServiceCoordinator) !void {
        var index = self.services.items.len;
        while (index > 0) {
            index -= 1;
            try self.services.items[index].drain_fn(self.services.items[index].pointer);
        }
    }
    pub fn shutdown(self: *ServiceCoordinator) !void {
        var index = self.services.items.len;
        while (index > 0) {
            index -= 1;
            try self.services.items[index].stop_fn(self.services.items[index].pointer);
        }
    }
};
...
test "managed services drain and stop in dependency-safe reverse order" {
    const Resource = struct {
        ...
        drain_value: u8,
        stop_value: u8,
        pub fn readiness(_: *@This()) bool {
            return true;
        }
        pub fn drain(self: *@This()) !void {
            self.order[self.index.*] = self.drain_value;
            self.index.* += 1;
        }
        pub fn shutdown(self: *@This()) !void {
            self.order[self.index.*] = self.stop_value;
            self.index.* += 1;
        }
    };
    var order = [_]u8{0} ** 4;
    ...
    var coordinator = ServiceCoordinator.init(std.testing.allocator);
    defer coordinator.deinit();
    try coordinator.register(ManagedService.from(Resource, "database", &database));
    try coordinator.register(ManagedService.from(Resource, "http", &http));
    try std.testing.expect(coordinator.ready());
    try coordinator.drain();
    try coordinator.shutdown();
