...
- `CausalGraph` provides a bounded Zig-native local graph WAL over the core
  NenDB-compatible writer contract, restart-safe durable event ids, parent-child
  traversal, partial-tail recovery, committed-corruption detection, and
  read-only snapshots. It is the default graph database generated by the CLI;
  it is not the upstream NenDB package.
