...
test "runtime emits causal run and exit events when store is attached" {
    ...
    try std.testing.expectEqual(snapshot.events[0].run_id.?, snapshot.events[1].run_id.?);
...
test "runtime causal events preserve failure and trace context" {
    ...
    try std.testing.expectEqual(@as(?u64, 101), snapshot.events[1].trace_id);
    try std.testing.expectEqual(@as(?u64, 202), snapshot.events[1].span_id);
}
