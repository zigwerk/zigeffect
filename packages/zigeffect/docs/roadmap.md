...
- Delivered: `causal-production-telemetry-ci-archive-evidence-policy`
  ...
  `codex/zigeffect-causal-production-telemetry-ci-gate-readiness`. It keeps
...
- Delivered: `causal-production-telemetry-ci-gate-application-boundary`
  ...
  blocked boundary evidence, and hands off to
  `codex/zigeffect-causal-production-telemetry-ci-gate-dry-run-policy`. It
...
- Delivered: `causal-production-telemetry-ci-gate-dry-run-policy` publishes
  ...
  ready or blocked policy evidence, and hands off to
  `codex/zigeffect-causal-production-telemetry-ci-gate-dry-run-evaluator`. It
  keeps CI gate enforcement, required status checks, workflow mutation by the
  tool, artifact upload execution, live telemetry, durable writes, NenDB
  writes, hosted dashboard claims, production cluster claims, alternate
  renderers, and production mutation disabled.
...
- Delivered: `causal-production-telemetry-ci-gate-advisory-ci-report`
  ...
  JSON/text reviewer guidance, records publication channels, and hands off to
  ...
  the tool, artifact upload execution, GitHub step summary writes, pull request
...
- Delivered:
  ...
  allowed and denied interpretations for externally published advisory
  reports, and hands off to
  `codex/zigeffect-causal-production-telemetry-ci-gate-required-status-check-readiness`.
  It keeps advisory reports non-blocking and keeps CI gate enforcement,
  required status checks, workflow mutation by the tool, artifact upload
  execution by the tool, GitHub step summary writes by the tool, pull request
  comments by the tool, live telemetry, durable writes, NenDB writes, hosted
  dashboard claims, production cluster claims, alternate renderers, and
  production mutation disabled.
- Delivered:
  ...
  It keeps required checks, merge blocking, branch protection mutation, GitHub
  API mutation, workflow mutation by the tool, artifact upload execution by
  the tool, GitHub step summary writes by the tool, pull request comments by
  the tool, live telemetry, durable writes, NenDB writes, hosted dashboard
  claims, production cluster claims, alternate renderers, and production
  mutation disabled.
- Delivered:
...
- Delivered:
  ...
  files, reports ready, advisory, or blocked findings for active-enforcement
  and merge-blocking observations, and hands off to
...
- Delivered:
  ...
  reports, preserves ready, advisory, and blocked findings, and hands off to
  `codex/zigeffect-causal-production-telemetry-ci-gate-required-status-check-enforcement-report-application-boundary`.
  It keeps GitHub API mutation by the tool, branch-protection mutation by the
  tool, workflow mutation by the tool, check-run creation by the tool, required
  status check creation by the tool, artifact upload execution by the tool,
  GitHub step summary writes by the tool, pull request comments by the tool,
...
- Delivered:
  ...
  consumes enforcement report artifacts, records planned or externally applied
  application-boundary evidence, keeps `applied=false` unless reviewed
  application-change, before, after, after-report, and verification evidence
  pass, and hands off to
...
- Delivered:
  ...
  consumes enforcement report application-boundary artifacts, records planned
  or applied interpretation policy, separates `ready_for_next_branch` from
  `published_report_policy_ready`, and hands off to
  `codex/zigeffect-causal-production-hardening-backlog-refresh`. It keeps
  GitHub API mutation by the tool, branch-protection mutation by the tool,
  workflow mutation by the tool, check-run creation by the tool, required
  status check creation by the tool, artifact upload execution by the tool,
  GitHub step summary writes by the tool, pull request comments by the tool,
  live telemetry, durable writes, NenDB writes, hosted dashboard claims,
  production cluster claims, alternate renderers, and production mutation
