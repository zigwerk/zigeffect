...
| 15 | Effect-grade standard library | **M1-M11 delivered**: service kernel, production Schema, effect-native config/JSON/secrets, CLI app framework, streams/queues/pubsub/sinks, local process/workspace/FS adapters, observability recorder/artifacts, HTTP/WebSocket contracts/adapters, SQL contracts plus local Postgres adapter, and local agent toolkit | `packages/zigeffect-std`, `packages/zigeffect-postgres`, `docs/superpowers/specs/2026-06-25-zigeffect-std-effectts-grade-roadmap-design.md` |
