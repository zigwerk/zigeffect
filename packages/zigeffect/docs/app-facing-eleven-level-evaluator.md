...
`causal-app-facing-eleven-level-evaluator` consumes approved eleven-level policy
...
- Tool file: `packages/zigeffect/tools/causal_app_facing_eleven_level_evaluator.zig`
- Build step: `causal-app-facing-eleven-level-evaluator`
- Executable: `zigeffect-causal-app-facing-eleven-level-evaluator`
- Docs: `packages/zigeffect/docs/app-facing-eleven-level-evaluator.md`
- Current branch: `codex/zigeffect-causal-app-facing-eleven-level-evaluator`
- Next branch: `codex/zigeffect-causal-app-facing-twelve-level-report`

## Usage

```sh
cd packages/zigeffect
zig build causal-app-facing-eleven-level-evaluator -- \
  --from-policy ../../.zig-cache/causal-artifacts/app-facing-ci-eleven-level-policy.json \
  evaluate \
  --reason "reviewed app-facing eleven-level evaluator" \
  --request ../../.zig-cache/causal-artifacts/app-facing-ci-eleven-level-evaluator-request.json \
  --evidence ../../.zig-cache/causal-artifacts/app-facing-ci-eleven-level-evaluator-support.txt \
  --out-prefix ../../.zig-cache/causal-artifacts/app-facing-ci-eleven-level-evaluator
```
...
- the source policy schema and version match the eleven-level policy producer;
- the source policy decision is `approve`;
- the source policy status is `ready` and `ready_for_next_branch=true`;
- source aliases identify the eleven-level policy, application-boundary, and
  report artifacts;
- ten-level policy, application-boundary, and report lineage is present;
...
Ready or advisory eleven-level evaluator artifacts may start:
