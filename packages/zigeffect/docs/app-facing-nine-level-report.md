...
`causal-app-facing-nine-level-report` consumes a ready or advisory eight-level
evaluator artifact and emits a compact local nine-level report for agents,
...
nine-level lineage is not ergonomic for filesystems or branch handling.
...
Ready output requires a supported eight-level evaluator schema, a ready or
advisory source status, `ready_for_next_branch=true`, no blocked findings,
approved ready eight-level policy evidence, applied eight-level application
evidence, inherited report evidence, request/support summaries, denied claims,
next queries, local publication channels, and disabled authority flags.

Advisory evaluator artifacts produce advisory nine-level reports when all hard
checks pass. Unsupported source schema, blocked source status, blocked findings,
authority drift, missing evidence, or non-local publication posture produces a
blocked report.
