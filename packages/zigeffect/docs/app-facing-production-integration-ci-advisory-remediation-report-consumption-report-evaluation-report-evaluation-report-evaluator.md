# App-Facing CI Advisory Remediation Report Consumption Report Evaluation Report Evaluation Report Evaluator

`causal-app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-evaluator` consumes a ready consumption-report evaluation-report-evaluation-report policy artifact plus explicit local request/evidence files and emits a read-only evaluator artifact for the next local evaluation-report-evaluation-report-evaluation-report handoff.

## Command

```bash
zig build causal-app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-evaluator -- \
  --from-policy <consumption-report-evaluation-report-evaluation-report-policy.json> \
  evaluate \
  --reason <reason> \
  --request <path>... \
  [--evidence <path>]... \
...
The source policy must use schema `zigeffect.causal.app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-policy.v1`, be approved, be ready for the next branch, and preserve `mutation_authority = none`.

The evaluator also requires evaluation-report-evaluation-report application-boundary refs, applied evaluation-report-evaluation-report application status, before/after evidence, after digest evidence, inherited consumption-report refs, source report application evidence, request/support/signal summaries, advisory and blocked findings, source denied claims, denied application claims, next queries, local publication channels, policy checks, interpretation rules, consumption scopes, negative fixtures, and verified command evidence.
