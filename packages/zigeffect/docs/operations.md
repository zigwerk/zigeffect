...
archive evidence policy, CI gate readiness, CI gate application boundary
contracts, CI gate dry-run policy, CI gate dry-run evaluator, CI gate advisory
CI report, CI gate advisory CI report application boundary, CI gate advisory
...
Run the CI gate required status check readiness review after a ready
publication-policy artifact exists:
...
The CI gate required status check readiness review records schema
`zigeffect.causal.production-telemetry-ci-gate-required-status-check-readiness.v1`.
It consumes ready publication-policy artifacts, emits ready or blocked
readiness evidence, defines candidate required-check profiles with activation
disabled, and records activation guardrails. Do not treat it as a required
status check, merge blocker, branch protection update, GitHub API mutation,
workflow mutation by the tool, artifact upload execution, GitHub step summary
write, pull request comment, runtime pipeline execution, live telemetry,
network send, collector configuration, OTLP serialization, NenDB write,
durable production write, hosted dashboard readiness, production cluster
...
Run the CI gate required status check policy after planned or applied
required-status-check application-boundary evidence exists:
...
The CI gate required status check policy records schema
`zigeffect.causal.production-telemetry-ci-gate-required-status-check-policy.v1`.
It consumes planned or externally applied required-status-check application
boundary artifacts, records `source_applied`, keeps
`merge_blocker_claim_allowed=false`, and emits enforcement-readiness handoff
evidence. Do not treat it as an active required status check, merge blocker,
branch-protection mutation, GitHub API mutation, workflow mutation by the tool,
check-run creation by the tool, CI upload execution, live telemetry, durable
production write, NenDB write, production cluster readiness, non-NenDB adapter
scope, alternate renderer scope, or production mutation authority.

Run the CI gate required status check enforcement-readiness review after an
applied policy source and explicit enforcement evidence exist:
...
The CI gate required status check enforcement-readiness artifact records schema
`zigeffect.causal.production-telemetry-ci-gate-required-status-check-enforcement-readiness.v1`.
It requires `source_applied=true`, exact required check names,
...
it can mark `enforcement_readiness_status="ready"`. It still keeps
`active_enforcement_claim_allowed=false`,
`merge_blocker_claim_allowed=false`, and `mutation_authority="none"`. Do not
treat it as active required-check enforcement, active merge blocking, GitHub
mutation, branch-protection mutation by this tool, workflow mutation by this
tool, check-run creation by this tool, CI upload execution, live telemetry,
durable production write, NenDB write, production cluster readiness,
non-NenDB adapter scope, alternate renderer scope, or production mutation
authority.

Run the CI gate required status check enforcement application boundary after a
ready enforcement-readiness artifact exists:
...
The CI gate required status check enforcement application-boundary artifact
records schema
`zigeffect.causal.production-telemetry-ci-gate-required-status-check-enforcement-application-boundary.v1`.
It consumes ready enforcement-readiness evidence and records planned or
externally applied active required-check enforcement evidence. `record-applied`
sets `applied=true` only with branch-protection before/after evidence,
...
that claim is made. Do not treat it as GitHub mutation by the tool,
branch-protection mutation by the tool, workflow mutation by the tool,
check-run creation by the tool, CI upload execution, live telemetry, durable
production write, NenDB write, production cluster readiness, non-NenDB adapter
scope, alternate renderer scope, or production mutation authority.
...
Run the CI gate required status check enforcement evaluator after an
enforcement policy artifact exists:
...
The evaluator records schema
`zigeffect.causal.production-telemetry-ci-gate-required-status-check-enforcement-evaluator.v1`.
It consumes ready enforcement-policy artifacts plus explicit bounded evidence
files, emits ready, advisory, or blocked findings, and hands off to the
enforcement report branch. Do not treat it as GitHub mutation by the tool,
branch-protection mutation by the tool, workflow mutation by the tool,
check-run creation by the tool, CI upload execution, live telemetry, durable
production write, NenDB write, non-NenDB adapter scope, alternate renderer
scope, production health, production cluster readiness, or production mutation
authority.

Run the CI gate required status check enforcement report after an enforcement
...
It consumes ready, advisory, or blocked evaluator artifacts, writes local
JSON/text reports only, preserves blocked findings, and hands off to the
enforcement report application-boundary branch. Do not treat it as GitHub
...
summary writes, pull-request comments, required status check creation, live
telemetry, durable production write, NenDB write, non-NenDB adapter scope,
alternate renderer scope, production health, production cluster readiness, or
production mutation authority.

Run the CI gate required status check enforcement report application boundary
after an enforcement report artifact exists:
...
The application-boundary report records schema
`zigeffect.causal.production-telemetry-ci-gate-required-status-check-enforcement-report-application-boundary.v1`.
It consumes ready or advisory report artifacts, records planned or externally
applied application evidence, keeps `applied=false` unless reviewed
application-change, before, after, after-report, and verification evidence all
pass, and hands off to the enforcement report policy branch. Do not treat it as
GitHub mutation by the tool, branch-protection mutation by the tool, workflow
mutation by the tool, check-run creation by the tool, required status check
creation by the tool, CI upload execution, GitHub step summary writes,
pull-request comments, live telemetry, durable production write, NenDB write,
non-NenDB adapter scope, alternate renderer scope, production health,
production cluster readiness, or production mutation authority.
