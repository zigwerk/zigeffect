...
`causal-app-facing-nine-level-evaluator` consumes approved nine-level policy
...
- Next branch: `codex/zigeffect-causal-app-facing-ten-level-report`

## Usage

```bash
zig build causal-app-facing-nine-level-evaluator -- \
  --from-policy ../../.zig-cache/causal-artifacts/app-facing-ci-nine-level-policy.json \
  evaluate \
  --reason "reviewed app-facing nine-level evaluator" \
  --request ../../.zig-cache/causal-artifacts/app-facing-ci-nine-level-evaluator-request.json \
  --evidence ../../.zig-cache/causal-artifacts/app-facing-ci-nine-level-evaluator-support.txt \
  --out-prefix ../../.zig-cache/causal-artifacts/app-facing-ci-nine-level-evaluator
```
...
- the source policy schema and version match the nine-level policy producer;
- the source policy decision is `approve`;
- the source policy status is `ready` and `ready_for_next_branch=true`;
- source aliases identify the nine-level policy, application-boundary, and
  report artifacts;
...
Ready or advisory nine-level evaluator artifacts may start:

```text
codex/zigeffect-causal-app-facing-ten-level-report
```
