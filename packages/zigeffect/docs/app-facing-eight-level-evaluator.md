...
`causal-app-facing-eight-level-evaluator` consumes approved eight-level policy
...
- Next branch: `codex/zigeffect-causal-app-facing-nine-level-report`

## Usage

```bash
zig build causal-app-facing-eight-level-evaluator -- \
  --from-policy ../../.zig-cache/causal-artifacts/app-facing-ci-eight-level-policy.json \
  evaluate \
  --reason "reviewed app-facing eight-level evaluator" \
  --request ../../.zig-cache/causal-artifacts/app-facing-ci-eight-level-evaluator-request.json \
  --evidence ../../.zig-cache/causal-artifacts/app-facing-ci-eight-level-evaluator-support.txt \
  --out-prefix ../../.zig-cache/causal-artifacts/app-facing-ci-eight-level-evaluator
```
...
- the source policy schema and version match the eight-level policy producer;
- the source policy decision is `approve`;
- the source policy status is `ready` and `ready_for_next_branch=true`;
- source aliases identify the eight-level policy, application-boundary, and
  report artifacts;
...
Ready or advisory eight-level evaluator artifacts may start:
