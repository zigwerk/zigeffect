...
const source_application_boundary_suffix = "-ci-gate-advisory-ci-report-application-boundary.json";
const output_prefix_suffix = "-ci-gate-advisory-ci-report-publication-policy";
const compact_output_prefix_name = "production-telemetry-ci-gate-advisory-ci-report-publication-policy";
...
const Options = struct {
    ...
    decision: Decision,
    reviewed_by: []const u8 = "ci-gate-advisory-ci-report-publication-policy-reviewer",
    policy: []const u8 = "manual-production-telemetry-ci-gate-advisory-ci-report-publication-policy",
    reason: []const u8,
...
const ApplicationBoundaryArtifact = struct {
    schema: []const u8,
    schema_version: u32,
    source_advisory_ci_report: []const u8 = "",
...
const interpretation_rules: []const InterpretationRule = &.{
    ...
    .{ .id = "future-readiness-input", .consumer_role = "future-readiness-tool", .allowed_use = "input to required-status-check readiness analysis only", .failure_effect = "readiness-input", .denied_claim = "required status check enabled" },
};
...
const negative_fixtures: []const NegativeFixture = &.{
    ...
    .{ .id = "production-health-claim-denied", .artifact_state = "production_health=proven", .decision = "deny", .failed_gate = "interpretation-rules-present", .reason = "CI advisory report evidence is not production health evidence" },
    .{ .id = "cluster-readiness-claim-denied", .artifact_state = "production_cluster_ready=true", .decision = "deny", .failed_gate = "interpretation-rules-present", .reason = "advisory report evidence cannot prove cluster readiness" },
...
fn parseOptions(allocator: std.mem.Allocator, args: []const []const u8) !Options {
    ...
    var reviewed_by: []const u8 = "ci-gate-advisory-ci-report-publication-policy-reviewer";
    var policy: []const u8 = "manual-production-telemetry-ci-gate-advisory-ci-report-publication-policy";
...
fn evaluatePolicy(
    ...
    try appendCheck(allocator, &checks, "source-schema", if (std.mem.eql(u8, source.schema, source_application_boundary_schema) and source.schema_version == 1) .pass else .fail, "source advisory CI report application-boundary schema is supported");
...
fn interpretationCatalogsComplete() bool {
    return hasInterpretationRule("reviewer-triage-summary") and
        ...
        hasInterpretationRule("future-readiness-input") and
...
fn formatPolicyJson(
    ...
    try appendJsonField(allocator, &output, "schema", production_telemetry_ci_gate_advisory_ci_report_publication_policy_schema, true);
    ...
    try appendJsonField(allocator, &output, "advisory_ci_report_publication_policy_status", policyStatusText(result.status), true);
...
fn formatPolicyText(
    ...
    try output.appendSlice(allocator, "zigeffect production telemetry CI gate advisory CI report publication policy\n");
    ...
    try output.print(allocator, "advisory_ci_report_publication_policy_status: {s}\n", .{policyStatusText(result.status)});
    try output.print(allocator, "ready_for_next_branch: {}\n", .{result.ready_for_next_branch});
...
fn agentGuidance(status: PolicyStatus) []const []const u8 {
    return switch (status) {
        .ready => &.{
            "Use ready advisory CI report publication policy as interpretation evidence only.",
            "Advisory reports may guide reviewers and agents but must not block merges or claim production readiness.",
            "Start required-status-check readiness only as a separate record-only readiness branch.",
        },
        .blocked => &.{
            "Treat blocked advisory CI report publication policy evidence as a stop sign.",
            ...
            "Do not infer required checks CI gate enforcement production health cluster readiness or mutation authority from blocked policy evidence.",
        },
    };
}
...
fn usage() []const u8 {
    return "usage: zig build causal-production-telemetry-ci-gate-advisory-ci-report-publication-policy -- --from-application <advisory-ci-report-application-boundary.json> approve|reject --reason <reason> [--by <actor>] [--policy <policy>] [--verified-command <command>]... [--out-prefix <path-prefix>]\n";
}

fn failUsage(err: anyerror) noreturn {
    std.debug.print("causal-production-telemetry-ci-gate-advisory-ci-report-publication-policy error: {s}\n{s}", .{ @errorName(err), usage() });
    std.process.exit(2);
}
...
test "ci gate advisory CI report publication policy schema and branch constants are stable" {
    ...
    try std.testing.expect(containsString(required_verification_commands, "zig build causal-production-telemetry-ci-gate-advisory-ci-report-application-boundary"));
    try std.testing.expect(containsString(required_verification_commands, "zig build release-gate-report"));
}

test "parses ci gate advisory CI report publication policy approval and rejection options" {
    var approve_options = try parseOptions(std.testing.allocator, &.{
        "zigeffect-causal-production-telemetry-ci-gate-advisory-ci-report-publication-policy",
        "--from-application",
        ".zig-cache/causal-artifacts/advisory-ci-report-application-boundary.json",
        "approve",
        "--reason",
        "CI advisory report publication policy reviewed",
        "--by",
        "codex",
        "--policy",
    ...
    try std.testing.expectEqualStrings(".zig-cache/causal-artifacts/advisory-ci-report-application-boundary.json", approve_options.application_boundary_path);
    try std.testing.expectEqualStrings("codex", approve_options.reviewed_by);
    try std.testing.expectEqualStrings("manual-production-telemetry-ci-gate-advisory-ci-report-publication-policy", approve_options.policy);
    ...
    var reject_options = try parseOptions(std.testing.allocator, &.{
        ...
        "reject",
        "--reason",
        "negative CI advisory report publication policy path",
    });
...
test "default output path replaces application boundary suffix and compacts long names" {
    const options = Options{
        .application_boundary_path = "../../.zig-cache/causal-artifacts/example-ci-gate-advisory-ci-report-application-boundary.json",
        .decision = .approve,
        .reason = "planned",
    };
    ...
    try std.testing.expectEqualStrings("../../.zig-cache/causal-artifacts/example-ci-gate-advisory-ci-report-publication-policy.json", paths.json_path);
    try std.testing.expectEqualStrings("../../.zig-cache/causal-artifacts/example-ci-gate-advisory-ci-report-publication-policy.txt", paths.text_path);

    const long_options = Options{
        .application_boundary_path = "../../.zig-cache/causal-artifacts/production-telemetry-capture-fixtures-readiness-review-implementation-proposal-exporter-boundary-local-pipeline-fixtures-nendb-retention-fixtures-workbench-readonly-preview-ci-artifact-preview-ci-harness-boundary-ci-gate-advisory-ci-report-application-boundary.json",
        .decision = .approve,
        .reason = "planned",
    };
    ...
    try std.testing.expect(contains(long_paths.json_path, "production-telemetry-ci-gate-advisory-ci-report-publication-policy-"));
}
...
test "approved publication policy is ready and preserves disabled authority" {
    const reports = try formatReports(std.testing.allocator, .{
        .options = .{
            .application_boundary_path = ".zig-cache/causal-artifacts/advisory-ci-report-application-boundary.json",
            .decision = .approve,
    ...
    try std.testing.expect(contains(reports.json, "\"schema\": \"zigeffect.causal.production-telemetry-ci-gate-advisory-ci-report-publication-policy.v1\""));
    try std.testing.expect(contains(reports.json, "\"advisory_ci_report_publication_policy_status\": \"ready\""));
    ...
    try std.testing.expect(contains(reports.json, "\"id\": \"future-readiness-input\""));
    ...
    try std.testing.expect(contains(reports.json, "\"next_branch_if_ready\": \"codex/zigeffect-causal-production-telemetry-ci-gate-required-status-check-readiness\""));
    try std.testing.expect(contains(reports.text, "advisory_ci_report_publication_policy_status: ready"));
    try std.testing.expect(contains(reports.text, "next branch if ready: codex/zigeffect-causal-production-telemetry-ci-gate-required-status-check-readiness"));
}
...
test "rejection incomplete verification and invalid sources block publication policy" {
    ...
    try std.testing.expect(contains(rejected.json, "\"advisory_ci_report_publication_policy_status\": \"blocked\""));
    ...
    try std.testing.expect(contains(incomplete.json, "\"advisory_ci_report_publication_policy_status\": \"blocked\""));
    ...
    try std.testing.expect(contains(planned_source.json, "\"advisory_ci_report_publication_policy_status\": \"blocked\""));
    ...
    try std.testing.expect(contains(unsafe_source.json, "\"advisory_ci_report_publication_policy_status\": \"blocked\""));
...
const sample_applied_application_boundary_json =
    ...
    \\  "schema_version": 1,
    \\  "source_advisory_ci_report": ".zig-cache/causal-artifacts/advisory-ci-report.json",
    ...
    \\  "before_evidence": ["advisory CI report local artifact existed before application"],
    \\  "after_evidence": ["advisory CI report application boundary reviewed after publication"],
    ...
    \\  "required_verification_commands": ["zig build causal-production-telemetry-ci-gate-advisory-ci-report", "zig build causal-artifacts"],
    \\  "verified_commands": ["zig build causal-production-telemetry-ci-gate-advisory-ci-report", "zig build causal-artifacts"],
    \\  "generated_by": "causal-production-telemetry-ci-gate-advisory-ci-report-application-boundary"
    \\}
;
...
const sample_planned_application_boundary_json =
    ...
    \\  "required_verification_commands": ["zig build causal-production-telemetry-ci-gate-advisory-ci-report"],
    \\  "verified_commands": [],
    \\  "generated_by": "causal-production-telemetry-ci-gate-advisory-ci-report-application-boundary"
    \\}
;
