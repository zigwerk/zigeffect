...
const Options = struct {
    ...
    reason: []const u8,
    reviewed_by: []const u8 = "app-facing-nine-level-report-reviewer",
    policy: []const u8 = "manual-app-facing-nine-level-report",
...
fn evaluateReport(allocator: std.mem.Allocator, source: EvaluatorArtifact) !ReportResult {
    ...
    try appendCheck(allocator, &checks, "source-evaluator-schema", if (sourceSchemaValid(source)) .pass else .fail, "source eight-level evaluator schema is supported");
    ...
    try appendCheck(allocator, &checks, "source-ready-for-next-branch", if (source.ready_for_next_branch) .pass else .fail, "source evaluator is ready for nine-level report work");
    ...
    try appendCheck(allocator, &checks, "source-eight-level-policy-ready", if (sourcePolicyReady(source)) .pass else .fail, "source eight-level policy application and inherited refs remain ready");
    try appendCheck(allocator, &checks, "source-eight-level-application-evidence", if (sourceReportApplicationEvidencePresent(source)) .pass else .fail, "source eight-level and inherited report application evidence next queries and local channels are present");
...
fn sourcePolicyReady(source: EvaluatorArtifact) bool {
    return sourceEightLevelPolicy(source).len > 0 and
...
fn formatReportJson(
    ...
    try appendJsonField(allocator, &output, "consumption_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_status", reportStatusText(result.status), true);
...
fn formatReportText(
    ...
    try output.appendSlice(allocator, "zigeffect app-facing nine-level report\n");
    ...
    try output.print(allocator, "consumption report evaluation report evaluation report evaluation report evaluation report evaluation report evaluation report evaluation report evaluation report evaluation report status: {s}\n", .{reportStatusText(result.status)});
...
fn headlineForStatus(status: ReportStatus) []const u8 {
    return switch (status) {
        .ready => "App-facing eight-level evaluator evidence is ready for nine-level report presentation.",
        .advisory => "App-facing eight-level evaluator evidence is reportable with advisory findings.",
        .blocked => "App-facing eight-level evaluator evidence is blocked and must not be used as reportable context.",
    };
}
...
fn usage() []const u8 {
    return "usage: zig build causal-app-facing-nine-level-report -- --from-evaluator <eight-level-evaluator.json> summarize --reason <reason> [--by <actor>] [--policy <policy>] [--out-prefix <path-prefix>]\n";
}

fn failUsage(err: anyerror) noreturn {
    std.debug.print("causal-app-facing-nine-level-report error: {s}\n{s}", .{ @errorName(err), usage() });
    std.process.exit(2);
}

test "app-facing nine-level report constants are stable" {
    try std.testing.expectEqualStrings("zigeffect.causal.app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report.v1", schema);
...
test "parses app-facing nine-level report options" {
    const options = try parseOptions(&.{
        ...
        "summarize",
        "--reason",
        "reviewed app-facing nine-level report",
        "--by",
        "codex",
        "--policy",
        "manual-app-facing-nine-level-report",
        "--out-prefix",
        ".zig-cache/causal-artifacts/custom-nine-level-report",
    });

    try std.testing.expectEqualStrings(".zig-cache/causal-artifacts/app-facing-ci-eight-level-evaluator.json", options.evaluator_path);
    try std.testing.expectEqualStrings("reviewed app-facing nine-level report", options.reason);
    try std.testing.expectEqualStrings("codex", options.reviewed_by);
    try std.testing.expectEqualStrings("manual-app-facing-nine-level-report", options.policy);
    try std.testing.expectEqualStrings(".zig-cache/causal-artifacts/custom-nine-level-report", options.out_prefix.?);
}

test "app-facing nine-level report rejects invalid options" {
...
test "default output path replaces eight-level evaluator suffix" {
    const paths = try outputPathsForOptions(std.testing.allocator, .{
        .evaluator_path = "../../.zig-cache/causal-artifacts/example-ci-eight-level-evaluator.json",
        .reason = "reviewed",
    });
    defer paths.deinit(std.testing.allocator);

    try std.testing.expectEqualStrings("../../.zig-cache/causal-artifacts/example-ci-nine-level-report.json", paths.json_path);
    try std.testing.expectEqualStrings("../../.zig-cache/causal-artifacts/example-ci-nine-level-report.txt", paths.text_path);
}
...
test "ready evaluator produces ready nine-level report" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, reports.json, "\"schema\": \"zigeffect.causal.app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report.v1\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, reports.json, "\"consumption_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_status\": \"ready\"") != null);
    ...
    try std.testing.expect(std.mem.indexOf(u8, reports.json, "\"next_branch_if_ready\": \"codex/zigeffect-causal-app-facing-nine-level-application-boundary\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, reports.text, "consumption report evaluation report evaluation report evaluation report evaluation report evaluation report evaluation report evaluation report evaluation report evaluation report status: ready") != null);

    var parsed = try std.json.parseFromSlice(std.json.Value, std.testing.allocator, reports.json, .{});
    parsed.deinit();
}

test "advisory evaluator produces advisory nine-level report" {
    const reports = try formatReports(std.testing.allocator, .{
        .options = sampleOptions(),
    ...
    try std.testing.expect(std.mem.indexOf(u8, reports.json, "\"consumption_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_status\": \"advisory\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, reports.json, "\"ready_for_next_branch\": true") != null);
...
test "blocked evaluator produces blocked nine-level report" {
    const reports = try formatReports(std.testing.allocator, .{
        .options = sampleOptions(),
    ...
    try std.testing.expect(std.mem.indexOf(u8, reports.json, "\"consumption_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_status\": \"blocked\"") != null);
...
fn sampleOptions() Options {
    return .{
        .evaluator_path = "../../.zig-cache/causal-artifacts/app-facing-ci-eight-level-evaluator.json",
        .reason = "reviewed app-facing nine-level report",
    };
}
