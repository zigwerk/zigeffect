...
pub const schema = "zigeffect.causal.app-facing-production-integration-ci-advisory-remediation-report-application-boundary.v1";
pub const schema_version: u32 = 1;
pub const source_branch = "codex/zigeffect-causal-app-facing-production-integration-ci-advisory-remediation-report-application-boundary";
pub const recommendation = "start-app-facing-production-integration-ci-advisory-remediation-report-publication-policy";
pub const next_branch_if_applied = "codex/zigeffect-causal-app-facing-production-integration-ci-advisory-remediation-report-publication-policy";

const source_report_schema = "zigeffect.causal.app-facing-production-integration-ci-advisory-remediation-report.v1";
const generated_by = "causal-app-facing-production-integration-ci-advisory-remediation-report-application-boundary";
const max_source_bytes = 1024 * 1024;
const source_report_suffix = "-ci-advisory-remediation-report.json";
const output_prefix_suffix = "-ci-advisory-remediation-report-application-boundary";
const compact_output_prefix_name = "app-facing-production-integration-ci-advisory-remediation-report-application-boundary";
...
const Options = struct {
    ...
    mode: Mode,
    reviewed_by: []const u8 = "app-facing-ci-advisory-remediation-report-application-boundary-reviewer",
    policy: []const u8 = "manual-app-facing-production-integration-ci-advisory-remediation-report-application-boundary",
    reason: []const u8,
...
const SourceReportArtifact = struct {
    ...
    source_preview_status: []const u8 = "",
    status: []const u8 = "",
    ci_advisory_remediation_report_status: []const u8 = "",
...
const prohibited_report_markers: []const []const u8 = &.{
    ...
    "production_health_claim_enabled=true",
...
const application_boundary_rules: []const []const u8 = &.{
    "Plan mode records advisory remediation report publication intent only and never records applied state.",
    ...
    "Future publication policy work may interpret externally published advisory remediation reports without creating required checks.",
...
const denied_publication_claims: []const []const u8 = &.{
    ...
    "app-config-written-by-tool",
    "app-data-written-by-tool",
    "deployment-mutated-by-tool",
    "live-telemetry-ingested",
    "runtime-pipeline-enabled",
    "durable-production-write-enabled",
    "nendb-write-enabled",
    "nendb-adapter-executed",
...
fn parseOptions(allocator: std.mem.Allocator, args: []const []const u8) !Options {
    ...
    var reviewed_by: []const u8 = "app-facing-ci-advisory-remediation-report-application-boundary-reviewer";
    var policy: []const u8 = "manual-app-facing-production-integration-ci-advisory-remediation-report-application-boundary";
...
fn evaluateBoundary(
    ...
    try appendCheck(allocator, &checks, "source-report-schema", if (std.mem.eql(u8, source.schema, source_report_schema) and source.schema_version == 1) .pass else .fail, "source advisory remediation report schema is supported");
    try appendCheck(allocator, &checks, "source-report-status", if (sourceReportReady(source)) .pass else .fail, "source advisory remediation report is ready for next branch");
    try appendCheck(allocator, &checks, "source-publication-disabled", if (sourcePublicationDisabled(source)) .pass else .fail, "source advisory remediation report did not publish externally");
    try appendCheck(allocator, &checks, "source-authority-disabled", if (sourceAuthorityDisabled(source)) .pass else .fail, "source keeps CI enforcement workflow mutation uploads live runtime app durable and NenDB authority disabled");
    try appendCheck(allocator, &checks, "source-bridge-record-carried", if (sourceBridgeRecordCarried(source)) .pass else .fail, "source carries the CI advisory remediation bridge record");
...
fn sourceReportReady(source: SourceReportArtifact) bool {
    return source.ready_for_next_branch and
        std.mem.eql(u8, sourceReportStatus(source), "ready");
}
...
fn sourceAuthorityDisabled(source: SourceReportArtifact) bool {
    return !source.applied and
        ...
        source.advisory_report and
        source.read_only_preview and
        source.solid_webui_enabled and
        std.mem.eql(u8, source.solid_webui_renderer, "solidjs") and
        std.mem.eql(u8, source.webui_bridge, "webui-dev/zig-webui") and
        ...
        !source.mutation_proof_claim_enabled and
        !source.auto_apply_enabled and
        !source.production_health_claim_enabled;
}
...
fn sourceReportStatus(source: SourceReportArtifact) []const u8 {
    if (source.status.len > 0) return source.status;
    return source.ci_advisory_remediation_report_status;
}

fn sourceBridgeRecordCarried(source: SourceReportArtifact) bool {
    for (source.bridge_records) |record| {
        if (std.mem.eql(u8, record.id, "ci-advisory-remediation-report-bridge") and
            std.mem.eql(u8, record.bridge_kind, "ci-advisory-report") and
            containsString(record.retained_refs, "ci_artifact_ref") and
            containsString(record.retained_refs, "advisory_status_ref"))
        {
            return true;
        }
    }
...
fn sourceVerificationRecorded(source: SourceReportArtifact) bool {
    return source.required_verification_commands.len > 0 and
        ...
        containsString(source.required_verification_commands, "zig build causal-app-facing-production-integration-solid-webui-readonly-preview") and
        containsString(source.required_verification_commands, "zig build causal-production-hardening-backlog -- --format json") and
...
fn formatBoundaryJson(
    ...
    try appendJsonField(allocator, &output, "source_advisory_remediation_report", options.report_path, true);
    try appendJsonField(allocator, &output, "source_report_status", sourceReportStatus(source), true);
    ...
    try appendBridgeRecordsJson(allocator, &output, source.bridge_records);
    try output.appendSlice(allocator, ",\n  \"application_checks\": ");
...
fn formatBoundaryText(
    ...
    try output.appendSlice(allocator, "zigeffect app-facing CI advisory remediation report application boundary\n");
    try output.print(allocator, "schema: {s}\n", .{schema});
    try output.print(allocator, "source advisory remediation report: {s}\n", .{options.report_path});
    try output.print(allocator, "source report status: {s}\n", .{sourceReportStatus(source)});
    ...
    try appendBridgeRecordsText(allocator, &output, source.bridge_records);
...
fn appendBridgeRecordsJson(allocator: std.mem.Allocator, output: *std.ArrayList(u8), records: []const SourceBridgeRecord) !void {
    ...
    for (records, 0..) |record, index| {
        ...
        try output.appendSlice(allocator, ", \"status\": ");
        try appendJsonString(allocator, output, record.status);
        try output.appendSlice(allocator, ", \"source_ref\": ");
        try appendJsonString(allocator, output, record.source_ref);
        try output.appendSlice(allocator, ", \"target_ref\": ");
        try appendJsonString(allocator, output, record.target_ref);
...
fn appendBridgeRecordsText(allocator: std.mem.Allocator, output: *std.ArrayList(u8), records: []const SourceBridgeRecord) !void {
    try output.appendSlice(allocator, "source bridge records:\n");
    if (records.len == 0) {
        try output.appendSlice(allocator, "- none\n\n");
        return;
    }
    for (records) |record| {
        try output.print(allocator, "- {s}: {s} ({s})\n", .{ record.id, record.status, record.bridge_kind });
    }
    try output.append(allocator, '\n');
}
...
fn agentGuidance(status: ReportApplicationStatus) []const []const u8 {
    return switch (status) {
        .planned => &.{
            "Use planned advisory remediation report application boundary evidence to prepare publication policy only.",
        ...
        .applied => &.{
            "Use applied advisory remediation report application boundary evidence to start publication policy work only.",
        ...
        .blocked => &.{
            "Treat blocked advisory remediation report application boundary evidence as a stop sign.",
...
fn failUsage(err: anyerror) noreturn {
    std.debug.print("causal-app-facing-production-integration-ci-advisory-remediation-report-application-boundary error: {s}\n{s}", .{ @errorName(err), usage() });
    std.process.exit(2);
}
...
const ready_source_report_json =
    ...
    \\  "bridge_records": [
    \\    { "id": "ci-advisory-remediation-report-bridge", "status": "advisory-only", "source_ref": "ci_artifact_ref", "target_ref": "advisory_status_ref", "bridge_kind": "ci-advisory-report", "retained_refs": ["ci_artifact_ref", "advisory_status_ref"], "blocked_claims": ["required-status-check", "ci-enforcement"] }
    \\  ],
    \\  "checks": [
    \\    { "name": "ci-advisory-review-state", "status": "pass", "detail": "advisory only" },
    \\    { "name": "ci-enforcement-disabled", "status": "pass", "detail": "no CI enforcement" },
    \\    { "name": "app-mutation-controls-disabled", "status": "pass", "detail": "no app mutation controls" }
    \\  ],
    \\  "blocked_claims": ["required-status-check-active", "github-api-mutated-by-tool", "nendb-adapter-executed"],
    \\  "required_verification_commands": [
    \\    "bun run zigeffect:workbench:typecheck",
    \\    "bun run zigeffect:workbench:test",
    ...
    \\    "zig build causal-production-hardening-backlog -- --format json"
    \\  ],
    \\  "generated_by": "causal-app-facing-production-integration-ci-advisory-remediation-report",
    \\  "source_branch": "codex/zigeffect-causal-app-facing-production-integration-ci-advisory-remediation-report",
...
const blocked_source_report_json =
    ...
    \\  "schema_version": 1,
    \\  "status": "blocked",
    \\  "ci_advisory_remediation_report_status": "blocked",
    ...
    \\  ],
    \\  "checks": [
    \\    { "name": "ci-advisory-review-state", "status": "fail", "detail": "blocked" }
    \\  ],
...
test "app-facing advisory remediation report application boundary schema and branch constants are stable" {
    try std.testing.expectEqualStrings("zigeffect.causal.app-facing-production-integration-ci-advisory-remediation-report-application-boundary.v1", schema);
    try std.testing.expectEqual(@as(u32, 1), schema_version);
    try std.testing.expectEqualStrings("codex/zigeffect-causal-app-facing-production-integration-ci-advisory-remediation-report-application-boundary", source_branch);
    try std.testing.expectEqualStrings("start-app-facing-production-integration-ci-advisory-remediation-report-publication-policy", recommendation);
    try std.testing.expectEqualStrings("codex/zigeffect-causal-app-facing-production-integration-ci-advisory-remediation-report-publication-policy", next_branch_if_applied);
    try std.testing.expect(containsString(required_verification_commands, "zig build causal-app-facing-production-integration-ci-advisory-remediation-report"));
    try std.testing.expect(containsString(required_verification_commands, "bun run zigeffect:workbench:typecheck"));
}

test "parses advisory remediation report application boundary plan and record-applied options" {
    var plan_options = try parseOptions(std.testing.allocator, &.{
        "zigeffect-causal-app-facing-production-integration-ci-advisory-remediation-report-application-boundary",
        "--from-report",
        ".zig-cache/causal-artifacts/report.json",
        "plan",
        "--reason",
        "app-facing advisory report application boundary planned",
    });
    defer plan_options.deinit(std.testing.allocator);
    try std.testing.expectEqual(Mode.plan, plan_options.mode);
    try std.testing.expectEqualStrings("app-facing-ci-advisory-remediation-report-application-boundary-reviewer", plan_options.reviewed_by);
    try std.testing.expectEqualStrings("manual-app-facing-production-integration-ci-advisory-remediation-report-application-boundary", plan_options.policy);
...
test "default output path replaces advisory remediation report suffix" {
    const options: Options = .{
        .report_path = ".zig-cache/causal-artifacts/example-ci-advisory-remediation-report.json",
        .mode = .plan,
        .reason = "planned",
    };
    const paths = try outputPathsForOptions(std.testing.allocator, options);
    defer paths.deinit(std.testing.allocator);
    try std.testing.expectEqualStrings(".zig-cache/causal-artifacts/example-ci-advisory-remediation-report-application-boundary.json", paths.json_path);
    try std.testing.expectEqualStrings(".zig-cache/causal-artifacts/example-ci-advisory-remediation-report-application-boundary.txt", paths.text_path);
}

test "long default output path stays within filesystem-safe basename length" {
    const options: Options = .{
        .report_path = "../../.zig-cache/causal-artifacts/app-facing-production-integration-fixtures-readiness-review-implementation-proposal-app-facing-boundary-local-fixtures-nendb-handoff-fixtures-solid-webui-readonly-preview-ci-advisory-remediation-report-reviewer-publication-boundary-evidence-before-after-verification-commands-application-record-ci-advisory-remediation-report.json",
        .mode = .plan,
        .reason = "planned",
    };
    const paths = try outputPathsForOptions(std.testing.allocator, options);
    ...
    try std.testing.expect(contains(paths.json_path, "app-facing-production-integration-ci-advisory-remediation-report-application-boundary-"));
}

test "ready advisory report source produces planned application boundary" {
    const options: Options = .{
        .report_path = ".zig-cache/causal-artifacts/source-ci-advisory-remediation-report.json",
        .mode = .plan,
        .reason = "planned",
    };
