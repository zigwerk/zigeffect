...
pub const schema = "zigeffect.causal.app-facing-production-integration-ci-advisory-remediation-report.v1";
pub const schema_version: u32 = 1;
pub const source_schema = "zigeffect.causal.app-facing-production-integration-solid-webui-readonly-preview.v1";
pub const tool_name = "causal-app-facing-production-integration-ci-advisory-remediation-report";
pub const source_branch = "codex/zigeffect-causal-app-facing-production-integration-ci-advisory-remediation-report";
pub const recommendation = "start-app-facing-production-integration-ci-advisory-remediation-report-application-boundary";
pub const next_branch_if_ready = "codex/zigeffect-causal-app-facing-production-integration-ci-advisory-remediation-report-application-boundary";
pub const output_suffix = "-ci-advisory-remediation-report";
...
const ReportStatus = enum { ready, blocked };
const CheckStatus = enum { pass, fail };

const Options = struct {
    from_preview: []const u8,
    decision: Decision,
    reason: []const u8,
...
fn parseOptions(allocator: std.mem.Allocator, args: []const []const u8) !Options {
    if (args.len < 2) return error.MissingFromPreview;
    if (!std.mem.eql(u8, args[1], "--from-preview")) return error.UnknownFlag;
    if (args.len < 3) return error.MissingFromPreview;
    const from_preview = args[2];
    if (!std.mem.endsWith(u8, from_preview, ".json")) return error.InvalidPreviewPath;
...
fn outputPathsForOptions(allocator: std.mem.Allocator, options: Options) !OutputPaths {
    const prefix = if (options.out_prefix) |out_prefix|
        ...
        if (!std.mem.endsWith(u8, options.from_preview, ".json")) return error.InvalidPreviewPath;
        const base = options.from_preview[0 .. options.from_preview.len - ".json".len];
        const stripped = if (std.mem.endsWith(u8, base, "-solid-webui-readonly-preview"))
            base[0 .. base.len - "-solid-webui-readonly-preview".len]
        else
            base;
...
fn statusText(status: ReportStatus) []const u8 {
...
const ReportInput = struct {
    options: Options,
    source_preview_json: []const u8,
};

const ReportReports = struct {
    json: []const u8,
    text: []const u8,

    fn deinit(self: ReportReports, allocator: std.mem.Allocator) void {
...
const SourcePreviewArtifact = struct {
    schema: []const u8 = "",
    status: []const u8 = "",
    solid_webui_readonly_preview_status: []const u8 = "",
    decision: []const u8 = "",
    applied: bool = true,
    ...
    react_renderer_enabled: bool = true,
    alternate_renderer_enabled: bool = true,
...
fn formatReports(allocator: std.mem.Allocator, input: ReportInput) !ReportReports {
    var parsed = try std.json.parseFromSlice(SourcePreviewArtifact, allocator, input.source_preview_json, .{ .ignore_unknown_fields = true });
    defer parsed.deinit();

    const result = try evaluateReport(allocator, input.options, parsed.value);
    defer result.deinit(allocator);

    const json = try formatReportJson(allocator, input.options, parsed.value, result);
    errdefer allocator.free(json);
    const text = try formatReportText(allocator, input.options, result);
...
fn evaluateReport(
    ...
    try appendCheck(allocator, &checks, "preview-schema", if (std.mem.eql(u8, bridge.schema, source_schema)) .pass else .fail, "source SolidJS read-only preview schema is supported");
    try appendCheck(allocator, &checks, "preview-status", if (std.mem.eql(u8, bridge_status, "ready")) .pass else .fail, "source SolidJS read-only preview is ready");
    try appendCheck(allocator, &checks, "preview-decision-approved", if (std.mem.eql(u8, bridge.decision, "approve")) .pass else .fail, "source SolidJS read-only preview was approved");
    try appendCheck(allocator, &checks, "report-decision", if (options.reason.len > 0) .pass else .fail, "report decision records a review reason");
    try appendCheck(allocator, &checks, "decision-approved", if (options.decision == .approve) .pass else .fail, "review decision approves this advisory remediation report artifact");
    try appendCheck(allocator, &checks, "authority-boundary", if (sourceAuthorityDisabled(bridge)) .pass else .fail, "source preview and report keep mutation authority disabled");
    try appendCheck(allocator, &checks, "source-chain-linked", if (sourceChainLinked(bridge)) .pass else .fail, "source preview keeps upstream handoff links or bridge records");
    try appendCheck(allocator, &checks, "preview-checks-passed", if (sourceChecksPassed(bridge.checks)) .pass else .fail, "all source preview checks passed");
    try appendCheck(allocator, &checks, "preview-verification-recorded", if (sourceVerificationRecorded(bridge)) .pass else .fail, "source preview recorded verification command evidence");
    try appendCheck(allocator, &checks, "bridge-catalog-present", if (bridgeCatalogPresent(bridge_records)) .pass else .fail, "source preview catalog contains the six app-facing review records");
    try appendCheck(allocator, &checks, "ci-advisory-bridge-present", if (hasBridgeRecord(bridge_records, "ci-advisory-remediation-report-bridge")) .pass else .fail, "source preview retains the CI advisory bridge record");
    try appendCheck(allocator, &checks, "ci-advisory-review-state", if (ciAdvisoryRecordIsAdvisoryOnly(bridge_records)) .pass else .fail, "CI advisory bridge remains advisory-only");
    try appendCheck(allocator, &checks, "report-panels-present", if (report_panels.len == 12) .pass else .fail, "report panel catalog is complete");
    try appendCheck(allocator, &checks, "report-sections-present", if (app_report_sections.len == 6) .pass else .fail, "app report section catalog is complete");
    try appendCheck(allocator, &checks, "advisory-report-readonly", if (advisory_report and read_only_preview and solid_webui_enabled and !applied and std.mem.eql(u8, mutation_authority, "none")) .pass else .fail, "report is advisory and read-only");
    ...
    try appendCheck(allocator, &checks, "webui-bridge-scope", if (std.mem.eql(u8, webui_bridge, "webui-dev/zig-webui")) .pass else .fail, "report remains scoped to webui-dev/zig-webui");
    try appendCheck(allocator, &checks, "ci-enforcement-disabled", if (ciAuthorityDisabled()) .pass else .fail, "CI report remains advisory without enforcement publication or GitHub mutation authority");
    ...
    try appendCheck(allocator, &checks, "report-verification-recorded", if (verifiedCommandsContainAll(options.verified_commands, required_verification_commands)) .pass else .fail, "report review recorded every required verification command");
...
fn sourceBridgeStatus(bridge: SourcePreviewArtifact) []const u8 {
    if (bridge.status.len > 0) return bridge.status;
    return bridge.solid_webui_readonly_preview_status;
}

fn sourceBridgeRecords(bridge: SourcePreviewArtifact) []const SourceBridgeRecord {
    if (bridge.bridge_records.len > 0) return bridge.bridge_records;
    return bridge.audit_remediation_bridge_records;
}

fn sourceAuthorityDisabled(bridge: SourcePreviewArtifact) bool {
    return !bridge.applied and
        std.mem.eql(u8, bridge.mutation_authority, "none") and
        bridge.read_only_preview and
        bridge.solid_webui_enabled and
        std.mem.eql(u8, bridge.solid_webui_renderer, "solidjs") and
        std.mem.eql(u8, bridge.webui_bridge, "webui-dev/zig-webui") and
        !bridge.hosted_live_dashboard_enabled and
        !bridge.app_mutation_controls_enabled and
        ...
        !bridge.react_renderer_enabled and
        !bridge.alternate_renderer_enabled;
}

fn sourceChainLinked(bridge: SourcePreviewArtifact) bool {
    return bridge.source_bridge.len > 0 or
        bridge.source_handoff.len > 0 or
...
fn sourceVerificationRecorded(bridge: SourcePreviewArtifact) bool {
...
fn ciAdvisoryRecordIsAdvisoryOnly(records: []const SourceBridgeRecord) bool {
    for (records) |record| {
        if (std.mem.eql(u8, record.id, "ci-advisory-remediation-report-bridge")) {
            return std.mem.eql(u8, bridgeRecordStatus(record), "advisory-only") and
                containsString(record.retained_refs, "ci_artifact_ref") and
                containsString(record.retained_refs, "advisory_status_ref") and
                containsString(record.blocked_claims, "required-status-check") and
                containsString(record.blocked_claims, "ci-enforcement");
        }
    }
    return false;
}

fn ciAuthorityDisabled() bool {
    return !ci_gate_enabled and
...
fn hasCheck(checks: []const ReportCheck, name: []const u8, status: CheckStatus) bool {
    for (checks) |check| {
...
fn formatReportJson(
    ...
    try output.appendSlice(allocator, ",\n  \"source_preview\": ");
    try appendJsonString(allocator, &output, options.from_preview);
    ...
    try output.appendSlice(allocator, ",\n  \"ci_advisory_remediation_report_status\": ");
    try appendJsonString(allocator, &output, statusText(result.status));
    try output.appendSlice(allocator, ",\n  \"decision\": ");
    ...
    try appendStringArray(allocator, &output, report_panels);
    try output.appendSlice(allocator, ",\n  \"app_preview_sections\": ");
    try appendReportSectionsJson(allocator, &output, app_report_sections);
    try output.appendSlice(allocator, ",\n");
    try output.appendSlice(allocator, "  \"report_panels\": ");
    try appendStringArray(allocator, &output, report_panels);
    try output.appendSlice(allocator, ",\n  \"app_report_sections\": ");
    try appendReportSectionsJson(allocator, &output, app_report_sections);
    try output.appendSlice(allocator, ",\n  \"bridge_records\": ");
...
fn formatReportText(
    ...
    try output.appendSlice(allocator, "zigeffect app-facing production integration CI advisory remediation report\n");
    try output.print(allocator, "schema: {s}\n", .{schema});
    try output.print(allocator, "source schema: {s}\n", .{source_schema});
    try output.print(allocator, "source preview: {s}\n", .{options.from_preview});
    try output.print(allocator, "source preview status: {s}\n", .{result.source_status});
    ...
    try output.print(allocator, "advisory report: {}\n", .{advisory_report});
    ...
    try appendTextList(allocator, &output, "report panels", report_panels);
    try appendReportSectionsText(allocator, &output, app_report_sections);
...
fn appendReportSectionsJson(allocator: std.mem.Allocator, output: *std.ArrayList(u8), sections: []const ReportSection) !void {
    try output.append(allocator, '[');
...
fn appendReportSectionsText(allocator: std.mem.Allocator, output: *std.ArrayList(u8), sections: []const ReportSection) !void {
    try output.appendSlice(allocator, "app report sections:\n");
...
fn agentGuidance(status: ReportStatus) []const []const u8 {
    return switch (status) {
        .ready => &.{
            "Ready audit/remediation bridge evidence may be inspected in the local SolidJS workbench report only.",
            "Agents should cite source preview records report sections checks and verification commands when reasoning about app-facing remediation state.",
        ...
        .blocked => &.{
            "Blocked report artifacts must not be treated as app runtime integration or live dashboard evidence.",
            "Repair source preview readiness authority drift bridge record coverage or report verification command evidence first.",
...
fn usage() []const u8 {
    return "usage: zig build causal-app-facing-production-integration-ci-advisory-remediation-report -- --from-preview <preview.json> approve|reject --reason <reason> [--verified-command <command>]... [--out-prefix <path-prefix>] [--format json|text]\n";
}

fn failUsage(err: anyerror) noreturn {
    std.debug.print("causal-app-facing-production-integration-ci-advisory-remediation-report error: {s}\n{s}", .{ @errorName(err), usage() });
    std.process.exit(2);
}

fn readRequiredArtifact(io: std.Io, allocator: std.mem.Allocator, path: []const u8) ![]const u8 {
    return std.Io.Dir.cwd().readFileAlloc(io, path, allocator, .limited(1024 * 1024)) catch |err| switch (err) {
        error.FileNotFound => error.MissingPreviewInput,
        else => return err,
    };
}
...
fn run(init: std.process.Init, options: Options) !void {
    ...
    const source_preview_json = try readRequiredArtifact(init.io, allocator, options.from_preview);
    defer allocator.free(source_preview_json);

    const reports = try formatReports(allocator, .{
        .options = options,
        .source_preview_json = source_preview_json,
    });
...
pub fn main(init: std.process.Init) !void {
    ...
    run(init, options) catch |err| switch (err) {
        error.MissingPreviewInput => failUsage(err),
        else => return err,
    };
}

const report_panels: []const []const u8 = &.{
    "status",
    "source-artifacts",
    "authority-boundary",
...
const app_report_sections: []const ReportSection = &.{
    ...
    .{ .id = "ci-advisory-report", .title = "CI advisory remediation report", .evidence_refs = &.{ "ci-advisory-remediation-report-bridge", "ci_artifact_ref", "advisory_status_ref" }, .blocked_authority = &.{ "required-status-check", "ci-enforcement", "workflow-mutation", "github-api-mutation" } },
};
...
const validation_checks: []const []const u8 = &.{
    ...
    "ci-advisory-report-section-present",
    "advisory-report-readonly",
...
const implementation_gates: []const []const u8 = &.{
    "source SolidJS read-only preview artifact is ready",
    "source preview checks and verification commands remain pass",
    "report emits advisory CI remediation state only",
...
const report_verified_commands_for_tests: []const []const u8 = required_verification_commands;

const sample_preview_json =
    \\{
    ...
    \\  "source_bridge": "source-solid-webui-readonly-preview.json",
    \\  "checks": [
    \\    { "name": "preview-schema", "status": "pass", "detail": "source preview schema" },
...
test "ci advisory remediation report constants define the milestone boundary" {
    try std.testing.expectEqualStrings("zigeffect.causal.app-facing-production-integration-ci-advisory-remediation-report.v1", schema);
    try std.testing.expectEqual(@as(u32, 1), schema_version);
    try std.testing.expectEqualStrings("zigeffect.causal.app-facing-production-integration-solid-webui-readonly-preview.v1", source_schema);
    try std.testing.expectEqualStrings("causal-app-facing-production-integration-ci-advisory-remediation-report", tool_name);
    try std.testing.expectEqualStrings("codex/zigeffect-causal-app-facing-production-integration-ci-advisory-remediation-report", source_branch);
    try std.testing.expectEqualStrings("start-app-facing-production-integration-ci-advisory-remediation-report-application-boundary", recommendation);
    try std.testing.expectEqualStrings("codex/zigeffect-causal-app-facing-production-integration-ci-advisory-remediation-report-application-boundary", next_branch_if_ready);
    try std.testing.expectEqualStrings("-ci-advisory-remediation-report", output_suffix);
}

test "ci advisory remediation report options require preview.json decision and reason" {
    try std.testing.expectError(error.MissingFromPreview, parseOptions(std.testing.allocator, &.{"tool"}));
    try std.testing.expectError(error.UnknownFlag, parseOptions(std.testing.allocator, &.{ "tool", "--from-handoff", "preview.json", "approve", "--reason", "reviewed" }));
    try std.testing.expectError(error.InvalidPreviewPath, parseOptions(std.testing.allocator, &.{ "tool", "--from-preview", "bridge.txt", "approve", "--reason", "reviewed" }));
    try std.testing.expectError(error.MissingDecision, parseOptions(std.testing.allocator, &.{ "tool", "--from-preview", "preview.json" }));
    try std.testing.expectError(error.InvalidDecision, parseOptions(std.testing.allocator, &.{ "tool", "--from-preview", "preview.json", "maybe", "--reason", "reviewed" }));
    try std.testing.expectError(error.MissingReason, parseOptions(std.testing.allocator, &.{ "tool", "--from-preview", "preview.json", "approve" }));
}

test "ci advisory remediation report options parse approval review with verification commands" {
    const parsed = try parseOptions(std.testing.allocator, &.{ "tool", "--from-preview", "preview.json", "approve", "--reason", "reviewed", "--verified-command", "bun run zigeffect:workbench:test", "--format", "json" });
    defer parsed.deinit(std.testing.allocator);

    try std.testing.expectEqualStrings("preview.json", parsed.from_preview);
...
test "ci advisory remediation report output paths replace source preview suffix" {
    const paths = try outputPathsForOptions(std.testing.allocator, .{
        .from_preview = "../../.zig-cache/causal-artifacts/source-solid-webui-readonly-preview.json",
        .decision = .approve,
        .reason = "reviewed",
    });
    defer paths.deinit(std.testing.allocator);

    try std.testing.expectEqualStrings("../../.zig-cache/causal-artifacts/source-ci-advisory-remediation-report.json", paths.json_path);
    try std.testing.expectEqualStrings("../../.zig-cache/causal-artifacts/source-ci-advisory-remediation-report.txt", paths.text_path);
}

test "ci advisory remediation report ready report preserves read-only SolidJS authority" {
    const reports = try formatReports(std.testing.allocator, .{
        .options = .{
            .from_preview = "source-solid-webui-readonly-preview.json",
            .decision = .approve,
            .reason = "ready app-facing audit remediation bridge reviewed for CI advisory remediation report",
            .verified_commands = report_verified_commands_for_tests,
        },
        .source_preview_json = sample_preview_json,
    });
    defer reports.deinit(std.testing.allocator);

    try std.testing.expect(std.mem.indexOf(u8, reports.json, "\"schema\": \"zigeffect.causal.app-facing-production-integration-ci-advisory-remediation-report.v1\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, reports.json, "\"source_schema\": \"zigeffect.causal.app-facing-production-integration-solid-webui-readonly-preview.v1\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, reports.json, "\"status\": \"ready\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, reports.json, "\"advisory_report\": true") != null);
    ...
    try std.testing.expect(std.mem.indexOf(u8, reports.json, "ci-advisory-report") != null);
...
test "ci advisory remediation report reject report remains blocked and authority-disabled" {
    const reports = try formatReports(std.testing.allocator, .{
        .options = .{
            .from_preview = "source-solid-webui-readonly-preview.json",
            .decision = .reject,
            .reason = "negative fixture keeps report blocked after reviewer rejection",
        },
        .source_preview_json = sample_preview_json,
    });
    defer reports.deinit(std.testing.allocator);
...
test "ci advisory remediation report blocks source authority drift" {
    ...
    try expectReportBlockedBy(app_data_write_json, "authority-boundary");

    const ci_gate_enabled_json = try std.mem.replaceOwned(
        u8,
        std.testing.allocator,
        sample_preview_json,
        "\"ci_gate_enabled\": false",
    ...
    defer std.testing.allocator.free(ci_gate_enabled_json);
    try expectReportBlockedBy(ci_gate_enabled_json, "authority-boundary");
}

test "ci advisory remediation report evaluator exposes failed source verification command evidence" {
    ...
    try expectReportBlockedBy(missing_command_json, "preview-verification-recorded");
}

test "ci advisory remediation report result helper sees source preview record catalog" {
    var parsed = try std.json.parseFromSlice(SourcePreviewArtifact, std.testing.allocator, sample_preview_json, .{ .ignore_unknown_fields = true });
    defer parsed.deinit();

    const result = try evaluateReport(std.testing.allocator, .{
        .from_preview = "source-solid-webui-readonly-preview.json",
        .decision = .approve,
        .reason = "reviewed",
        .verified_commands = report_verified_commands_for_tests,
    ...
    try std.testing.expectEqual(ReportStatus.ready, result.status);
    try std.testing.expect(hasCheck(result.checks, "advisory-report-readonly", .pass));
...
fn expectReportBlockedBy(source_preview_json: []const u8, check_name: []const u8) !void {
    var parsed = try std.json.parseFromSlice(SourcePreviewArtifact, std.testing.allocator, source_preview_json, .{ .ignore_unknown_fields = true });
    defer parsed.deinit();
    ...
    try std.testing.expectEqual(ReportStatus.blocked, result.status);
