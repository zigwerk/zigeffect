...
const source_report_schema = "zigeffect.causal.production-telemetry-ci-gate-required-status-check-enforcement-report.v1";
const generated_by = "causal-production-telemetry-ci-gate-required-status-check-enforcement-report-application-boundary";
const max_source_bytes = 1024 * 1024;
const source_report_suffix = "-ci-gate-required-status-check-enforcement-report.json";
const output_prefix_suffix = "-ci-gate-required-status-check-enforcement-report-application-boundary";
const compact_output_prefix_name = "production-telemetry-ci-gate-required-status-check-enforcement-report-application-boundary";
...
const Options = struct {
    ...
    fn deinit(self: Options, allocator: std.mem.Allocator) void {
        freeSliceOnly(allocator, self.application_changes);
...
const ApplicationResult = struct {
    status: ReportApplicationBoundaryStatus,
    applied: bool,
    mutation_authority: []const u8,
...
const required_report_markers: []const []const u8 = &.{ "zigeffect", "causal", "required status check", "enforcement", "report" };
...
const prohibited_report_markers: []const []const u8 = &.{
    ...
    "required_status_check_created_by_tool",
    "branch_protection_mutated_by_tool",
...
const application_boundary_rules: []const []const u8 = &.{
    "Plan mode records required status check enforcement report publication intent only and never records applied state.",
    "record-applied requires reviewed application-change evidence, before evidence, after evidence, report-after content, report safety, and full verification commands.",
    ...
    "Future publication policy work may interpret externally published required status check enforcement reports without creating required checks.",
...
const negative_fixtures: []const NegativeFixture = &.{
    .{ .id = "blocked-source-report-denied", .artifact_state = "required_status_check_enforcement_report_status=blocked", .decision = "deny", .failed_gate = "source-report-status", .reason = "blocked source reports cannot feed application records" },
    .{ .id = "source-application-enabled-denied", .artifact_state = "required-status-check channel executed", .decision = "deny", .failed_gate = "source-application-disabled", .reason = "source report tool must remain local and non-applying" },
    ...
    .{ .id = "missing-application-change-denied", .artifact_state = "record-applied application_changes=[]", .decision = "deny", .failed_gate = "application-change-present", .reason = "reviewed application change evidence is required" },
    ...
    .{ .id = "unsafe-report-after-required-check-denied", .artifact_state = "required_status_check_created_by_tool present", .decision = "deny", .failed_gate = "after-report-safe", .reason = "required status check creation by the tool is out of scope" },
    .{ .id = "unsafe-report-after-branch-protection-denied", .artifact_state = "branch_protection_mutated_by_tool present", .decision = "deny", .failed_gate = "after-report-safe", .reason = "branch protection mutation by the tool is out of scope" },
...
fn parseOptions(allocator: std.mem.Allocator, args: []const []const u8) !Options {
    ...
    var reviewed_by: []const u8 = "ci-gate-required-status-check-enforcement-report-application-boundary-reviewer";
    var policy: []const u8 = "manual-production-telemetry-ci-gate-required-status-check-enforcement-report-application-boundary";
    var reason: ?[]const u8 = null;
    var report_after_path: ?[]const u8 = null;
    var application_changes = std.ArrayList([]const u8).empty;
    ...
    errdefer application_changes.deinit(allocator);
    ...
    while (index < args.len) {
        ...
        } else if (std.mem.eql(u8, flag, "--application-change")) {
            try application_changes.append(allocator, value);
    ...
    return .{
        ...
        .application_changes = try application_changes.toOwnedSlice(allocator),
...
fn statusText(status: ReportApplicationBoundaryStatus) []const u8 {
...
fn evaluateBoundary(
    ...
    try appendCheck(allocator, &checks, "source-report-schema", if (std.mem.eql(u8, source.schema, source_report_schema) and source.schema_version == 1) .pass else .fail, "source required status check enforcement report schema is supported");
    try appendCheck(allocator, &checks, "source-report-status", if (sourceReportReady(source)) .pass else .fail, "source required status check enforcement report is ready or advisory and ready for next branch");
    try appendCheck(allocator, &checks, "source-application-disabled", if (sourceApplicationDisabled(source)) .pass else .fail, "source required status check enforcement report did not apply or publish externally");
    try appendCheck(allocator, &checks, "source-authority-disabled", if (sourceAuthorityDisabled(source)) .pass else .fail, "source keeps GitHub API branch protection workflow check-run upload summary comment live telemetry runtime durable and NenDB authority disabled");
    ...
    if (options.mode == .plan) {
        ...
        try appendCheck(allocator, &checks, "application-change-present", .skipped, "plan mode does not claim application change evidence");
    ...
    try appendCheck(allocator, &checks, "application-change-present", if (options.application_changes.len > 0) .pass else .fail, "record-applied requires reviewed application change evidence");
...
fn sourceReportReady(source: SourceReportArtifact) bool {
    return source.ready_for_next_branch and
        (std.mem.eql(u8, source.required_status_check_enforcement_report_status, "ready") or std.mem.eql(u8, source.required_status_check_enforcement_report_status, "advisory"));
}
...
fn sourceApplicationDisabled(source: SourceReportArtifact) bool {
    if (source.ci_report_publication_enabled or
        ...
        source.ci_workflow_mutation_enabled or
        source.github_api_mutation_enabled or
        source.github_check_run_creation_enabled or
...
fn externalPublicationChannel(id: []const u8) bool {
    return std.mem.eql(u8, id, "ci-upload-artifact") or
        ...
        std.mem.eql(u8, id, "branch-protection-update");
}
...
fn sourceVerificationRecorded(source: SourceReportArtifact) bool {
    return source.required_verification_commands.len > 0 and
        containsString(source.required_verification_commands, "zig build causal-production-telemetry-ci-gate-required-status-check-enforcement-evaluator") and
...
fn formatBoundaryJson(
    ...
    try appendJsonField(allocator, &output, "schema", production_telemetry_ci_gate_required_status_check_enforcement_report_application_boundary_schema, true);
    try output.appendSlice(allocator, "  \"schema_version\": 1,\n");
    try appendJsonField(allocator, &output, "source_enforcement_report", options.report_path, true);
    try appendJsonField(allocator, &output, "source_report_status", source.required_status_check_enforcement_report_status, true);
    try appendJsonField(allocator, &output, "mode", modeText(options.mode), true);
    try appendJsonField(allocator, &output, "report_application_boundary_status", statusText(result.status), true);
    ...
    try output.appendSlice(allocator, "  \"application_changes\": ");
    try appendStringArray(allocator, &output, options.application_changes);
    try output.appendSlice(allocator, ",\n  \"before_evidence\": ");
...
fn formatBoundaryText(
    ...
    try output.appendSlice(allocator, "zigeffect production telemetry CI gate required status check enforcement report application boundary\n");
    try output.print(allocator, "schema: {s}\n", .{production_telemetry_ci_gate_required_status_check_enforcement_report_application_boundary_schema});
    try output.print(allocator, "source required status check enforcement report: {s}\n", .{options.report_path});
    try output.print(allocator, "source report status: {s}\n", .{source.required_status_check_enforcement_report_status});
    try output.print(allocator, "after report digest: {s}\n", .{after_report_digest});
    try output.print(allocator, "mode: {s}\n", .{modeText(options.mode)});
    try output.print(allocator, "report_application_boundary_status: {s}\n", .{statusText(result.status)});
    try output.print(allocator, "applied: {}\n", .{result.applied});
    ...
    try appendTextList(allocator, &output, "application changes", options.application_changes);
...
fn agentGuidance(status: ReportApplicationBoundaryStatus) []const []const u8 {
    return switch (status) {
        .planned => &.{
            "Use planned required status check enforcement report application boundary evidence to prepare publication policy only.",
        ...
        .applied => &.{
            "Use applied required status check enforcement report application boundary evidence to start publication policy work only.",
            "Cite application changes before evidence after evidence report-after digest post-verification commands and source enforcement report.",
        },
        .blocked => &.{
            "Treat blocked required status check enforcement report application boundary evidence as a stop sign.",
...
fn usage() []const u8 {
    return "usage: zig build causal-production-telemetry-ci-gate-required-status-check-enforcement-report-application-boundary -- --from-report <required-status-check-enforcement-report.json> plan|record-applied --reason <reason> [--report-after <report.txt|report.md|report.json>] [--by <actor>] [--policy <policy>] [--application-change <evidence>] [--before <evidence>] [--after <evidence>] [--verified-command <command>]... [--out-prefix <path-prefix>]\n";
}

fn failUsage(err: anyerror) noreturn {
    std.debug.print("causal-production-telemetry-ci-gate-required-status-check-enforcement-report-application-boundary error: {s}\n{s}", .{ @errorName(err), usage() });
    std.process.exit(2);
}
...
const ready_source_report_json =
    ...
    \\  "required_status_check_enforcement_report_status": "advisory",
    ...
    \\  ],
    \\  "blocked_claims": ["required-status-check-created-by-tool", "workflow-mutated-by-tool"],
    \\  "required_verification_commands": [
    \\    "zig build causal-production-telemetry-ci-gate-required-status-check-enforcement-evaluator",
    \\    "zig build release-gate-report",
    \\    "zig build causal-production-hardening-backlog -- --format json"
    \\  ],
    \\  "generated_by": "causal-production-telemetry-ci-gate-required-status-check-enforcement-report",
    \\  "agent_guidance": ["Surface reviewer findings as reviewer guidance only."]
    \\}
;
...
const blocked_source_report_json =
    ...
    \\  "schema": "zigeffect.causal.production-telemetry-ci-gate-required-status-check-enforcement-report.v1",
    \\  "schema_version": 1,
    \\  "required_status_check_enforcement_report_status": "blocked",
    ...
    \\  ],
    \\  "blocked_claims": ["required-status-check-created-by-tool"],
    \\  "required_verification_commands": [
    \\    "zig build causal-production-telemetry-ci-gate-required-status-check-enforcement-evaluator",
    \\    "zig build release-gate-report",
    \\    "zig build causal-production-hardening-backlog -- --format json"
...
const safe_after_report_text = "zigeffect causal required status check enforcement report local publication evidence only";
const unsafe_after_report_text = "zigeffect causal required status check enforcement report required_status_check_created_by_tool enabled";

test "ci gate required status check enforcement report application boundary schema and branch constants are stable" {
    try std.testing.expectEqualStrings("zigeffect.causal.production-telemetry-ci-gate-required-status-check-enforcement-report-application-boundary.v1", production_telemetry_ci_gate_required_status_check_enforcement_report_application_boundary_schema);
    ...
    try std.testing.expectEqualStrings("codex/zigeffect-causal-production-telemetry-ci-gate-required-status-check-enforcement-report-application-boundary", source_branch);
    try std.testing.expectEqualStrings("start-production-telemetry-ci-gate-required-status-check-enforcement-report-policy", recommendation);
    try std.testing.expectEqualStrings("codex/zigeffect-causal-production-telemetry-ci-gate-required-status-check-enforcement-report-policy", next_branch_if_applied);
    try std.testing.expect(containsString(required_verification_commands, "zig build causal-production-telemetry-ci-gate-required-status-check-enforcement-report"));
    try std.testing.expect(containsString(required_verification_commands, "zig build release-gate-report"));
}

test "parses required status check enforcement report application boundary plan and record-applied options" {
    var plan_options = try parseOptions(std.testing.allocator, &.{
        ...
        "plan",
        "--reason",
        "CI required status check enforcement report application boundary planned",
    });
    defer plan_options.deinit(std.testing.allocator);
    try std.testing.expectEqual(Mode.plan, plan_options.mode);
    try std.testing.expectEqualStrings("ci-gate-required-status-check-enforcement-report-application-boundary-reviewer", plan_options.reviewed_by);
    try std.testing.expectEqualStrings("manual-production-telemetry-ci-gate-required-status-check-enforcement-report-application-boundary", plan_options.policy);
    ...
    try std.testing.expectEqual(@as(usize, 1), applied_options.application_changes.len);
...
test "default output path replaces required status check enforcement report suffix" {
...
test "long default output path stays within filesystem-safe basename length" {
    ...
    try std.testing.expect(contains(paths.json_path, "production-telemetry-ci-gate-required-status-check-enforcement-report-application-boundary-"));
}

test "ready required status check enforcement report source produces planned application boundary" {
    const options: Options = .{
        .report_path = ".zig-cache/causal-artifacts/source-ci-gate-required-status-check-enforcement-report.json",
        .mode = .plan,
        .reason = "planned",
    };
    ...
    try std.testing.expect(contains(reports.json, "\"report_application_boundary_status\": \"planned\""));
...
test "blocked required status check enforcement report source produces blocked application boundary" {
    const options: Options = .{
        .report_path = ".zig-cache/causal-artifacts/source-ci-gate-required-status-check-enforcement-report.json",
        .mode = .plan,
        .reason = "planned",
    ...
    defer reports.deinit(std.testing.allocator);
    try std.testing.expect(contains(reports.json, "\"report_application_boundary_status\": \"blocked\""));
    try std.testing.expect(contains(reports.json, "\"applied\": false"));
...
test "enabled source publication authority blocks boundary" {
    ...
    defer reports.deinit(std.testing.allocator);
    try std.testing.expect(contains(reports.json, "\"report_application_boundary_status\": \"blocked\""));
    try std.testing.expect(contains(reports.json, "\"source-application-disabled\", \"status\": \"fail\""));
}
...
test "record-applied missing evidence stays blocked" {
    ...
    defer reports.deinit(std.testing.allocator);
    try std.testing.expect(contains(reports.json, "\"report_application_boundary_status\": \"blocked\""));
    try std.testing.expect(contains(reports.json, "\"application-change-present\", \"status\": \"fail\""));
...
test "record-applied complete evidence produces applied boundary" {
    ...
    defer reports.deinit(std.testing.allocator);
    try std.testing.expect(contains(reports.json, "\"report_application_boundary_status\": \"applied\""));
