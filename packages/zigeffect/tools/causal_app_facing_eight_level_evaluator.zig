...
const Options = struct {
    ...
    reason: []const u8,
    reviewed_by: []const u8 = "app-facing-eight-level-evaluator-reviewer",
    policy: []const u8 = "manual-app-facing-eight-level-evaluator",
...
fn parseOptions(allocator: std.mem.Allocator, args: []const []const u8) !Options {
    ...
    var reviewed_by: []const u8 = "app-facing-eight-level-evaluator-reviewer";
    var policy: []const u8 = "manual-app-facing-eight-level-evaluator";
...
fn classifyFile(path: []const u8, contents: []const u8, detected_schema: []const u8) FileClass {
    ...
    if (is_json and (std.mem.indexOf(u8, path, "eight-level-policy") != null or std.mem.indexOf(u8, path, "consumption-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-policy") != null or std.mem.eql(u8, detected_schema, source_policy_schema))) return .policy_json;
...
fn evaluatePolicyAndFiles(
    ...
    try appendCheckAndFinding(allocator, &checks, &findings, "source-report-policy-ready", std.mem.eql(u8, source.consumption_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_policy_status, "ready"), "source-report-policy-valid", "source consumption report policy status is ready");
...
fn sourceRefsPresent(source: SourcePolicyArtifact) bool {
    return sourceEightLevelApplicationBoundary(source).len > 0 and
...
fn sourceEightLevelReport(source: SourcePolicyArtifact) []const u8 {
...
fn sourceEightLevelReportStatus(source: SourcePolicyArtifact) []const u8 {
...
fn sourceEightLevelAfterPresent(source: SourcePolicyArtifact) bool {
    return source.source_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_after_present or
...
fn formatEvaluatorJson(
    ...
    try appendJsonField(allocator, &output, "source_eight_level_policy", options.policy_path, true);
    try appendJsonField(allocator, &output, "source_eight_level_policy_schema", source.schema, true);
    try appendJsonField(allocator, &output, "source_eight_level_policy_status", source.consumption_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_policy_status, true);
...
fn formatEvaluatorText(
    ...
    try output.print(allocator, "source_eight_level_policy_status: {s}\n", .{source.consumption_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_policy_status});
...
fn nextQueries(status: EvaluationStatus) []const []const u8 {
    return switch (status) {
        .ready => &.{
            "causal-query --artifact <app-facing-ci-eight-level-evaluator.json> --kind request",
            "causal-query --artifact <app-facing-ci-eight-level-evaluator.json> --kind denied-claim",
            "start codex/zigeffect-causal-app-facing-nine-level-report with this evaluator artifact",
        },
        .advisory_findings => &.{
            "add support evidence for the request and rerun the evaluator",
            "inspect advisory findings before rendering a nine-level report",
        },
        .blocked => &.{
...
fn agentGuidance(status: EvaluationStatus) []const []const u8 {
    return switch (status) {
        .ready => &.{
            "Ready eight-level evaluator artifacts may feed a local nine-level report.",
...
fn usage() []const u8 {
    return "usage: zig build causal-app-facing-eight-level-evaluator -- --from-policy <eight-level-policy.json> evaluate --reason <reason> --request <path>... [--evidence <path>]... [--by <actor>] [--policy <policy>] [--out-prefix <path-prefix>]\n";
}

fn failUsage(err: anyerror) noreturn {
    std.debug.print("causal-app-facing-eight-level-evaluator error: {s}\n{s}", .{ @errorName(err), usage() });
    std.process.exit(2);
}
...
fn readyPolicyJson() []const u8 {
    ...
    \\  "schema_version": 1,
    \\  "decision": "approve",
    \\  "consumption_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_policy_status": "ready",
    ...
    \\  "source_eight_level_report": "app-facing-ci-eight-level-report.json",
...
fn blockedPolicyJson() []const u8 {
    ...
    \\  "schema_version": 1,
    \\  "decision": "reject",
    \\  "consumption_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_policy_status": "blocked",
...
test "parses app-facing eight-level evaluator options with requests and evidence" {
    var options = try parseOptions(std.testing.allocator, &.{
        "zigeffect-causal-app-facing-eight-level-evaluator",
        "--from-policy",
        ".zig-cache/causal-artifacts/app-facing-ci-eight-level-policy.json",
        "evaluate",
        "--reason",
        ...
        "codex",
        "--policy",
        "manual-app-facing-eight-level-evaluator",
        "--request",
        ...
        "--out-prefix",
        ".zig-cache/causal-artifacts/custom-app-facing-ci-eight-level-evaluator",
    });
    defer options.deinit(std.testing.allocator);

    try std.testing.expectEqualStrings(".zig-cache/causal-artifacts/app-facing-ci-eight-level-policy.json", options.policy_path);
    ...
    try std.testing.expectEqualStrings("manual-app-facing-eight-level-evaluator", options.policy);
    ...
    try std.testing.expectEqualStrings(".zig-cache/causal-artifacts/custom-app-facing-ci-eight-level-evaluator", options.out_prefix.?);
}
...
test "default output path replaces report policy suffix and compacts long names" {
    ...
    try std.testing.expectEqualStrings(
        ".zig-cache/causal-artifacts/app-facing-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluator.json",
        paths.json_path,
    );
    try std.testing.expectEqualStrings(
        ".zig-cache/causal-artifacts/app-facing-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluator.txt",
        paths.text_path,
    );
}
...
test "classifies request policy causal workbench and denied files" {
    var evidence = try analyzeFiles(std.testing.allocator, &.{
        ...
        .{ .path = ".zig-cache/causal-artifacts/app-facing-ci-eight-level-policy.json", .contents = readyPolicyJson() },
...
test "ready advisory and blocked evaluator output preserve read-only authority" {
    var ready_options = try parseOptions(std.testing.allocator, &.{
        "tool",
        "--from-policy",
        "source-app-facing-ci-eight-level-policy.json",
        "evaluate",
        "--reason",
    ...
    try std.testing.expect(std.mem.indexOf(u8, ready_reports.json, "\"source_eight_level_policy\": \"source-app-facing-ci-eight-level-policy.json\"") != null);
    ...
    try std.testing.expect(std.mem.indexOf(u8, ready_reports.json, "\"source_eight_level_report\": \"app-facing-ci-eight-level-report.json\"") != null);
