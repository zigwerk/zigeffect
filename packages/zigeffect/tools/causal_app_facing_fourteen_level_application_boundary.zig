...
pub const source_branch = "codex/zigeffect-causal-app-facing-fourteen-level-application-boundary";
pub const recommendation = "start-app-facing-fourteen-level-policy";
pub const next_branch_if_applied = "codex/zigeffect-causal-app-facing-fourteen-level-policy";
...
const generated_by = "causal-app-facing-fourteen-level-application-boundary";
const source_report_suffix = "-ci-fourteen-level-report.json";
const output_prefix_suffix = "-ci-fourteen-level-application-boundary";
const compact_output_prefix_name = "app-facing-ci-fourteen-level-application-boundary";
...
const Options = struct {
    ...
    mode: Mode,
    reviewed_by: []const u8 = "app-facing-fourteen-level-application-boundary-reviewer",
    policy: []const u8 = "manual-app-facing-fourteen-level-application-boundary",
    reason: []const u8,
...
const SourceReportArtifact = struct {
    ...
    source_thirteen_level_evaluator: []const u8 = "",
    source_thirteen_level_evaluator_schema: []const u8 = "",
    source_thirteen_level_evaluator_status: []const u8 = "",
    ...
    source_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_after_digest: []const u8 = "",
    source_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_after_present: bool = false,
...
const boundary_rules: []const []const u8 = &.{
    "Plan mode records local fourteen-level application-boundary intent only and never records applied state.",
    "record-applied requires reviewed fourteen-level application change evidence, before evidence, after evidence, safe after-report content, and full verification commands.",
    ...
    "Future fourteen-level policy work may interpret applied records without creating mutation authority or public publication authority.",
};
...
const negative_fixtures: []const NegativeFixture = &.{
    .{ .id = "blocked-source-report-denied", .artifact_state = "consumption_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_status=blocked", .decision = "deny", .failed_gate = "source-report-ready", .reason = "blocked source evaluation-report evaluation reports cannot feed application records" },
    ...
    .{ .id = "missing-report-application-change-denied", .artifact_state = "record-applied evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_application_changes=[]", .decision = "deny", .failed_gate = "report-application-change-present", .reason = "reviewed evaluation report evaluation report evaluation report evaluation report evaluation report evaluation report evaluation report application change evidence is required" },
...
fn parseOptions(allocator: std.mem.Allocator, args: []const []const u8) !Options {
    ...
    var reviewed_by: []const u8 = "app-facing-fourteen-level-application-boundary-reviewer";
    var policy: []const u8 = "manual-app-facing-fourteen-level-application-boundary";
    ...
    while (index < args.len) {
        ...
        if (std.mem.eql(u8, flag, "--by")) {
            ...
            reason = value;
        } else if (std.mem.eql(u8, flag, "--after-report") or std.mem.eql(u8, flag, "--evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-after")) {
            if (!reportAfterPathValid(value)) return error.InvalidReportAfterPath;
            report_after_path = value;
        } else if (std.mem.eql(u8, flag, "--application-change") or std.mem.eql(u8, flag, "--evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-application-change")) {
...
fn evaluateBoundary(
    ...
    try appendCheck(allocator, &checks, "source-report-generated-by", if (sourceReportGeneratedByExpectedTool(source)) .pass else .fail, "source report was generated by causal-app-facing-fourteen-level-report");
    try appendCheck(allocator, &checks, "source-report-ready", if (sourceReportReady(source)) .pass else .fail, "source consumption-report evaluation report evaluation report evaluation report evaluation report evaluation report evaluation report is ready or advisory and ready for this branch with no blocked findings");
    ...
    if (options.mode == .plan) {
        try appendCheck(allocator, &checks, "plan-is-not-applied", .pass, "plan mode records local fourteen-level application-boundary intent without applied state");
        try appendCheck(allocator, &checks, "report-application-change-present", .skipped, "plan mode does not claim fourteen-level application change evidence");
    ...
    try appendCheck(allocator, &checks, "report-application-change-present", if (options.report_application_changes.len > 0) .pass else .fail, "record-applied requires reviewed fourteen-level application change evidence");
    ...
    try appendCheck(allocator, &checks, "after-report-safe", if (report_after_text) |after_text| reportAfterSafetyStatus(after_text) else .fail, "after-report evidence preserves read-only fourteen-level application constraints");
...
fn sourceReportReady(source: SourceReportArtifact) bool {
    return source.ready_for_next_branch and
        ...
        (std.mem.eql(u8, source.consumption_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_status, "ready") or std.mem.eql(u8, source.consumption_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_status, "advisory")) and
        std.mem.eql(u8, source.mutation_authority, "none");
}

fn sourceReportGeneratedByExpectedTool(source: SourceReportArtifact) bool {
    return std.mem.eql(u8, source.generated_by, "causal-app-facing-fourteen-level-report");
}
...
fn sourceEightLevelPolicy(source: SourceReportArtifact) []const u8 {
...
fn sourceEightLevelPolicySchema(source: SourceReportArtifact) []const u8 {
...
fn sourceEightLevelPolicyStatus(source: SourceReportArtifact) []const u8 {
...
fn sourceEightLevelApplicationBoundary(source: SourceReportArtifact) []const u8 {
...
fn sourceEightLevelApplicationBoundarySchema(source: SourceReportArtifact) []const u8 {
...
fn sourceEightLevelReport(source: SourceReportArtifact) []const u8 {
...
fn sourceEightLevelReportStatus(source: SourceReportArtifact) []const u8 {
...
fn sourceEightLevelApplicationStatus(source: SourceReportArtifact) []const u8 {
...
fn sourceEightLevelAfterDigest(source: SourceReportArtifact) []const u8 {
...
fn sourceEightLevelAfterPresent(source: SourceReportArtifact) bool {
...
fn sourceEightLevelApplicationChanges(source: SourceReportArtifact) []const []const u8 {
...
fn sourceInheritedPolicy(source: SourceReportArtifact) []const u8 {
...
fn sourceInheritedPolicySchema(source: SourceReportArtifact) []const u8 {
...
fn sourceInheritedPolicyStatus(source: SourceReportArtifact) []const u8 {
...
fn sourceInheritedApplicationBoundary(source: SourceReportArtifact) []const u8 {
...
fn sourceInheritedApplicationBoundarySchema(source: SourceReportArtifact) []const u8 {
...
fn sourceInheritedReport(source: SourceReportArtifact) []const u8 {
...
fn sourceInheritedReportStatus(source: SourceReportArtifact) []const u8 {
...
fn sourceInheritedApplicationStatus(source: SourceReportArtifact) []const u8 {
...
fn sourceInheritedApplicationChanges(source: SourceReportArtifact) []const []const u8 {
...
fn sourceNineLevelRefsPresent(source: SourceReportArtifact) bool {
...
fn sourceTenLevelRefsPresent(source: SourceReportArtifact) bool {
...
fn sourceElevenLevelRefsPresent(source: SourceReportArtifact) bool {
    return source.source_eleven_level_policy.len > 0 and
        source.source_eleven_level_policy_schema.len > 0 and
        std.mem.eql(u8, source.source_eleven_level_policy_status, "ready") and
        source.source_eleven_level_application_boundary.len > 0 and
        source.source_eleven_level_application_boundary_schema.len > 0 and
        source.source_eleven_level_report.len > 0 and
        source.source_eleven_level_report_schema.len > 0 and
        std.mem.eql(u8, source.source_eleven_level_report_status, "ready");
}

fn sourceThirteenLevelRefsPresent(source: SourceReportArtifact) bool {
    return source.source_thirteen_level_evaluator.len > 0 and
        source.source_thirteen_level_evaluator_schema.len > 0 and
        std.mem.eql(u8, source.source_thirteen_level_evaluator_status, "ready") and
        source.source_thirteen_level_policy.len > 0 and
        source.source_thirteen_level_policy_schema.len > 0 and
        std.mem.eql(u8, source.source_thirteen_level_policy_status, "ready") and
        source.source_thirteen_level_application_boundary.len > 0 and
        source.source_thirteen_level_application_boundary_schema.len > 0 and
        std.mem.eql(u8, source.source_thirteen_level_application_boundary_status, "applied") and
        source.source_thirteen_level_report.len > 0 and
        source.source_thirteen_level_report_schema.len > 0 and
        std.mem.eql(u8, source.source_thirteen_level_report_status, "ready") and
        source.source_thirteen_level_after_digest.len > 0 and
        source.source_thirteen_level_after_present;
}

fn sourceTwelveLevelRefsPresent(source: SourceReportArtifact) bool {
    return source.source_twelve_level_evaluator.len > 0 and
        source.source_twelve_level_evaluator_schema.len > 0 and
        std.mem.eql(u8, source.source_twelve_level_evaluator_status, "ready") and
        source.source_twelve_level_policy.len > 0 and
        source.source_twelve_level_policy_schema.len > 0 and
        std.mem.eql(u8, source.source_twelve_level_policy_status, "ready") and
        source.source_twelve_level_application_boundary.len > 0 and
...
fn sourceRefsPresent(source: SourceReportArtifact) bool {
    return source.source_evaluator.len > 0 and
        ...
        sourceThirteenLevelRefsPresent(source) and
        sourceTwelveLevelRefsPresent(source) and
        sourceElevenLevelRefsPresent(source) and
...
fn sourceSummariesPresent(source: SourceReportArtifact) bool {
    return source.request_summary.len > 0 and
        ...
        source.source_thirteen_level_application_changes.len > 0 and
...
fn formatBoundaryJson(
    ...
    try appendJsonField(allocator, &output, "source_fourteen_level_report", options.report_path, true);
    try appendJsonField(allocator, &output, "source_fourteen_level_report_schema", source.schema, true);
    try appendJsonField(allocator, &output, "source_fourteen_level_report_status", source.consumption_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_status, true);
    ...
    try appendJsonField(allocator, &output, "source_thirteen_level_evaluator", source.source_thirteen_level_evaluator, true);
    try appendJsonField(allocator, &output, "source_thirteen_level_evaluator_schema", source.source_thirteen_level_evaluator_schema, true);
    try appendJsonField(allocator, &output, "source_thirteen_level_evaluator_status", source.source_thirteen_level_evaluator_status, true);
    try appendJsonField(allocator, &output, "source_thirteen_level_policy", source.source_thirteen_level_policy, true);
    try appendJsonField(allocator, &output, "source_thirteen_level_policy_schema", source.source_thirteen_level_policy_schema, true);
    try appendJsonField(allocator, &output, "source_thirteen_level_policy_status", source.source_thirteen_level_policy_status, true);
    try appendJsonField(allocator, &output, "source_thirteen_level_application_boundary", source.source_thirteen_level_application_boundary, true);
    try appendJsonField(allocator, &output, "source_thirteen_level_application_boundary_schema", source.source_thirteen_level_application_boundary_schema, true);
    try appendJsonField(allocator, &output, "source_thirteen_level_application_boundary_status", source.source_thirteen_level_application_boundary_status, true);
    try appendJsonField(allocator, &output, "source_thirteen_level_report", source.source_thirteen_level_report, true);
    try appendJsonField(allocator, &output, "source_thirteen_level_report_schema", source.source_thirteen_level_report_schema, true);
    try appendJsonField(allocator, &output, "source_thirteen_level_report_status", source.source_thirteen_level_report_status, true);
    ...
    try appendJsonField(allocator, &output, "source_twelve_level_evaluator", source.source_twelve_level_evaluator, true);
    try appendJsonField(allocator, &output, "source_twelve_level_evaluator_schema", source.source_twelve_level_evaluator_schema, true);
    try appendJsonField(allocator, &output, "source_twelve_level_evaluator_status", source.source_twelve_level_evaluator_status, true);
    try appendJsonField(allocator, &output, "source_twelve_level_policy", source.source_twelve_level_policy, true);
    try appendJsonField(allocator, &output, "source_twelve_level_policy_schema", source.source_twelve_level_policy_schema, true);
    try appendJsonField(allocator, &output, "source_twelve_level_policy_status", source.source_twelve_level_policy_status, true);
    try appendJsonField(allocator, &output, "source_twelve_level_application_boundary", source.source_twelve_level_application_boundary, true);
    try appendJsonField(allocator, &output, "source_twelve_level_application_boundary_schema", source.source_twelve_level_application_boundary_schema, true);
    try appendJsonField(allocator, &output, "source_twelve_level_report", source.source_twelve_level_report, true);
    try appendJsonField(allocator, &output, "source_twelve_level_report_schema", source.source_twelve_level_report_schema, true);
    try appendJsonField(allocator, &output, "source_twelve_level_report_status", source.source_twelve_level_report_status, true);
    ...
    try appendJsonField(allocator, &output, "source_eleven_level_policy", source.source_eleven_level_policy, true);
    try appendJsonField(allocator, &output, "source_eleven_level_policy_schema", source.source_eleven_level_policy_schema, true);
    try appendJsonField(allocator, &output, "source_eleven_level_policy_status", source.source_eleven_level_policy_status, true);
    try appendJsonField(allocator, &output, "source_eleven_level_application_boundary", source.source_eleven_level_application_boundary, true);
    try appendJsonField(allocator, &output, "source_eleven_level_application_boundary_schema", source.source_eleven_level_application_boundary_schema, true);
    try appendJsonField(allocator, &output, "source_eleven_level_report", source.source_eleven_level_report, true);
    try appendJsonField(allocator, &output, "source_eleven_level_report_schema", source.source_eleven_level_report_schema, true);
    try appendJsonField(allocator, &output, "source_eleven_level_report_status", source.source_eleven_level_report_status, true);
    ...
    try appendJsonField(allocator, &output, "source_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_after_digest", source.source_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_after_digest, true);
    try output.print(allocator, "  \"source_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_after_present\": {},\n", .{source.source_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_after_present});
    try appendJsonField(allocator, &output, "source_consumption_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_policy", source.source_consumption_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_policy, true);
    try appendJsonField(allocator, &output, "source_consumption_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_policy_schema", source.source_consumption_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_policy_schema, true);
    try appendJsonField(allocator, &output, "source_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_policy_status", source.source_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_policy_status, true);
    try appendJsonField(allocator, &output, "source_policy_decision", source.source_policy_decision, true);
    try appendJsonField(allocator, &output, "source_consumption_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_application_boundary", source.source_consumption_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_application_boundary, true);
    try appendJsonField(allocator, &output, "source_consumption_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_application_boundary_schema", source.source_consumption_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_application_boundary_schema, true);
    ...
    try appendJsonField(allocator, &output, "source_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_application_status", source.source_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_application_status, true);
    ...
    try appendJsonField(allocator, &output, "source_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_after_digest", source.source_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_after_digest, true);
    try output.print(allocator, "  \"source_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_after_present\": {},\n", .{source.source_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_after_present});
    try appendJsonField(allocator, &output, "mode", modeText(options.mode), true);
    try appendJsonField(allocator, &output, "evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_application_status", statusText(result.status), true);
    ...
    try output.appendSlice(allocator, "  \"evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_after_path\": ");
    try appendOptionalJsonString(allocator, &output, options.report_after_path);
    try output.appendSlice(allocator, ",\n");
    try appendJsonField(allocator, &output, "evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_after_digest", after_report_digest, true);
    try output.print(allocator, "  \"evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_after_present\": {},\n", .{report_after_text != null});
    ...
    try output.appendSlice(allocator, "  \"evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_application_changes\": ");
...
fn formatBoundaryText(
    ...
    try output.appendSlice(allocator, "zigeffect app-facing fourteen-level application boundary\n");
    try output.print(allocator, "schema: {s}\n", .{schema});
    try output.print(allocator, "source fourteen-level report: {s}\n", .{options.report_path});
    try output.print(allocator, "source fourteen-level report status: {s}\n", .{source.consumption_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_status});
    ...
    try output.print(allocator, "source evaluation report evaluation report evaluation report evaluation report evaluation report evaluation report policy: {s}\n", .{source.source_consumption_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_policy});
    try output.print(allocator, "source evaluation report evaluation report evaluation report evaluation report evaluation report evaluation report policy status: {s}\n", .{source.source_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_policy_status});
    try output.print(allocator, "source evaluation report evaluation report evaluation report evaluation report evaluation report evaluation report application boundary: {s}\n", .{source.source_consumption_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_application_boundary});
    try output.print(allocator, "source evaluation report evaluation report evaluation report evaluation report evaluation report evaluation report application status: {s}\n", .{source.source_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_application_status});
    try output.print(allocator, "source consumption report evaluation report evaluation report evaluation report evaluation report evaluation report evaluation report: {s}\n", .{source.source_consumption_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report});
    try output.print(allocator, "source consumption report evaluation report evaluation report evaluation report evaluation report evaluation report evaluation report status: {s}\n", .{source.source_consumption_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_status});
    try output.print(allocator, "inherited source evaluation report evaluation report evaluation report evaluation report evaluation report policy: {s}\n", .{source.source_consumption_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_policy});
    try output.print(allocator, "inherited source evaluation report evaluation report evaluation report evaluation report evaluation report application boundary: {s}\n", .{source.source_consumption_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_application_boundary});
    ...
    try output.print(allocator, "evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_application_status: {s}\n", .{statusText(result.status)});
    try output.print(allocator, "applied: {}\n", .{result.applied});
    try output.print(allocator, "ready_for_next_branch: {}\n", .{result.ready_for_next_branch});
    ...
    try output.print(allocator, "evaluation report evaluation report evaluation report evaluation report evaluation report evaluation report evaluation report after digest: {s}\n", .{after_report_digest});
    ...
    try appendTextList(allocator, &output, "evaluation report evaluation report evaluation report evaluation report evaluation report evaluation report evaluation report application changes", options.report_application_changes);
    ...
    try appendTextList(allocator, &output, "source evaluation report evaluation report evaluation report evaluation report evaluation report evaluation report application changes", source.source_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_application_changes);
    try appendTextList(allocator, &output, "inherited source evaluation report evaluation report evaluation report evaluation report evaluation report application changes", source.source_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_application_changes);
...
fn agentGuidance(status: ReportApplicationStatus) []const []const u8 {
    return switch (status) {
        .planned => &.{
            "Use planned fourteen-level application-boundary evidence to prepare fourteen-level policy work only.",
        ...
        .applied => &.{
            "Use applied fourteen-level application-boundary evidence to start fourteen-level policy work only.",
            "Cite twelve-level application changes before evidence after evidence after-report digest source report checks denied claims and post-verification commands.",
        },
        .blocked => &.{
            "Treat blocked fourteen-level application-boundary evidence as a stop sign.",
...
fn usage() []const u8 {
    return "usage: zig build causal-app-facing-fourteen-level-application-boundary -- --from-report <fourteen-level-report.json> plan|record-applied --reason <reason> [--after-report <report.txt|report.md|report.json>] [--application-change <evidence>] [--before <evidence>] [--after <evidence>] [--verified-command <command>]... [--by <actor>] [--policy <policy>] [--out-prefix <path-prefix>]\n";
}

fn failUsage(err: anyerror) noreturn {
    std.debug.print("causal-app-facing-fourteen-level-application-boundary error: {s}\n{s}", .{ @errorName(err), usage() });
    std.process.exit(2);
}
...
const safe_report_after_text =
    "zigeffect causal read-only consumption report evaluation-report evaluation report evaluation report evaluation report evaluation report evaluation report evaluation report application boundary reviewed locally for agents reviewers non-blocking CI advisory readers and SolidJS webui.";

const ready_source_report_json =
    \\{
    \\  "schema": "zigeffect.causal.app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report.v1",
    \\  "schema_version": 1,
    \\  "generated_by": "causal-app-facing-fourteen-level-report",
    \\  "source_evaluator": "evaluator.json",
    \\  "source_evaluator_schema": "zigeffect.causal.app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluator.v1",
    \\  "source_evaluator_status": "ready",
    \\  "source_thirteen_level_evaluator": "thirteen-level-evaluator.json",
    \\  "source_thirteen_level_evaluator_schema": "zigeffect.causal.app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluator.v1",
    \\  "source_thirteen_level_evaluator_status": "ready",
    \\  "source_thirteen_level_policy": "thirteen-level-policy.json",
    ...
    \\  "source_thirteen_level_application_boundary": "thirteen-level-application-boundary.json",
    ...
    \\  "source_thirteen_level_report": "thirteen-level-report.json",
    ...
    \\  "source_twelve_level_evaluator": "twelve-level-evaluator.json",
    \\  "source_twelve_level_evaluator_schema": "twelve-level-evaluator-schema",
    \\  "source_twelve_level_evaluator_status": "ready",
    \\  "source_twelve_level_policy": "twelve-level-policy.json",
    \\  "source_twelve_level_policy_schema": "twelve-level-policy-schema",
    \\  "source_twelve_level_policy_status": "ready",
    \\  "source_twelve_level_application_boundary": "twelve-level-application-boundary.json",
    \\  "source_twelve_level_application_boundary_schema": "twelve-level-application-boundary-schema",
    \\  "source_twelve_level_report": "twelve-level-report.json",
    \\  "source_twelve_level_report_schema": "twelve-level-report-schema",
    ...
    \\  "source_eleven_level_policy": "eleven-level-policy.json",
    ...
    \\  "source_eleven_level_application_boundary": "eleven-level-application-boundary.json",
    ...
    \\  "source_eleven_level_report": "eleven-level-report.json",
    ...
    \\  "source_ten_level_policy": "ten-level-policy.json",
    ...
    \\  "source_ten_level_application_boundary": "ten-level-application-boundary.json",
    ...
    \\  "source_ten_level_report": "ten-level-report.json",
    ...
    \\  "source_nine_level_policy": "nine-level-policy.json",
    ...
    \\  "source_nine_level_application_boundary": "nine-level-application-boundary.json",
    ...
    \\  "source_nine_level_report": "nine-level-report.json",
    ...
    \\  "source_consumption_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_policy": "evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-policy.json",
    ...
    \\  "source_consumption_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report": "evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report.json",
    ...
    \\  "source_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_after_digest": "sha256:evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-after",
    ...
    \\  "source_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_application_changes": ["reviewed local evaluation report evaluation report evaluation report evaluation report evaluation report evaluation report application"],
    \\  "source_consumption_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_policy": "evaluation-report-policy.json",
    \\  "source_consumption_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_policy_schema": "zigeffect.causal.app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluation-report-policy.v1",
    ...
    \\  "source_consumption_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_application_boundary": "evaluation-report-application-boundary.json",
    ...
    \\  "source_consumption_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report": "evaluation-report.json",
    ...
    \\  "source_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_after_digest": "sha256:evaluation-report-after",
    \\  "source_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_after_present": true,
    \\  "source_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_application_changes": ["reviewed local evaluation report application"],
...
const blocked_source_report_json =
    ...
    \\  "source_thirteen_level_evaluator": "thirteen-level-evaluator.json",
    \\  "source_thirteen_level_evaluator_schema": "zigeffect.causal.app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluator.v1",
    \\  "source_thirteen_level_evaluator_status": "blocked",
    \\  "source_thirteen_level_policy": "thirteen-level-policy.json",
    \\  "source_thirteen_level_policy_schema": "zigeffect.causal.app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-policy.v1",
    \\  "source_thirteen_level_policy_status": "ready",
    \\  "source_thirteen_level_application_boundary": "thirteen-level-application-boundary.json",
    ...
    \\  "source_thirteen_level_after_digest": "sha256:thirteen-after",
    \\  "source_thirteen_level_after_present": true,
    \\  "source_thirteen_level_application_changes": ["reviewed blocked local thirteen-level application boundary"],
    \\  "source_twelve_level_evaluator": "twelve-level-evaluator.json",
    \\  "source_twelve_level_evaluator_schema": "twelve-level-evaluator-schema",
    \\  "source_twelve_level_evaluator_status": "ready",
    \\  "source_twelve_level_policy": "twelve-level-policy.json",
    ...
    \\  "source_consumption_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_application_boundary_schema": "zigeffect.causal.app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-application-boundary.v1",
    \\  "source_consumption_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report": "evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report.json",
    \\  "source_consumption_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_status": "blocked",
    \\  "source_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_application_status": "applied",
    \\  "source_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_after_digest": "sha256:evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-after",
    \\  "source_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_after_present": true,
    \\  "source_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_application_changes": ["reviewed blocked local evaluation report evaluation report evaluation report evaluation report evaluation report evaluation report application"],
    \\  "source_consumption_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_policy": "evaluation-report-policy.json",
    \\  "source_consumption_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_policy_schema": "zigeffect.causal.app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluation-report-policy.v1",
    \\  "source_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_policy_status": "ready",
    \\  "source_policy_decision": "approve",
    ...
    \\  "source_consumption_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_application_boundary_schema": "zigeffect.causal.app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluation-report-application-boundary.v1",
    \\  "source_consumption_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report": "evaluation-report.json",
    \\  "source_consumption_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_status": "blocked",
    \\  "source_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_application_status": "applied",
    \\  "source_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_after_digest": "sha256:evaluation-report-after",
    \\  "source_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_after_present": true,
    \\  "source_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_application_changes": ["reviewed blocked local evaluation report application"],
    \\  "source_consumption_report_application_boundary": "report-application-boundary.json",
    \\  "source_report_application_status": "applied",
    \\  "source_consumption_report": "consumption-report.json",
    ...
    \\  "source_advisory_findings": [],
    \\  "consumption_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_status": "blocked",
    \\  "ready_for_next_branch": false,
...
const authority_drift_source_report_json =
    ...
    \\  "source_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_after_digest": "sha256:evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-after",
    \\  "source_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_after_present": true,
    \\  "source_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_application_changes": ["reviewed authority local evaluation report evaluation report evaluation report evaluation report evaluation report evaluation report application"],
    \\  "source_consumption_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_policy": "evaluation-report-policy.json",
    \\  "source_consumption_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_policy_schema": "zigeffect.causal.app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluation-report-policy.v1",
    \\  "source_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_policy_status": "ready",
    \\  "source_policy_decision": "approve",
    ...
    \\  "source_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_after_digest": "sha256:evaluation-report-after",
    \\  "source_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_after_present": true,
    \\  "source_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_application_changes": ["reviewed authority local evaluation report application"],
    \\  "source_consumption_report_application_boundary": "report-application-boundary.json",
    \\  "source_report_application_status": "applied",
    \\  "source_consumption_report": "consumption-report.json",
    \\  "source_consumption_report_status": "ready",
...
test "parses app-facing consumption report evaluation-report evaluation-report evaluation-report application boundary options" {
    const args = &.{
        "tool",
        "--from-report",
        "source-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report.json",
        "record-applied",
        "--reason",
        ...
        "after-report.md",
        "--application-change",
        "reviewed evaluation report evaluation report evaluation report evaluation report evaluation report evaluation report application change",
        "--before",
        "before evidence",
        "--after",
        "after evidence",
    ...
    try std.testing.expectEqualStrings("source-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report.json", options.report_path);
    try std.testing.expectEqualStrings("app-facing-fourteen-level-application-boundary-reviewer", options.reviewed_by);
    try std.testing.expectEqualStrings("manual-app-facing-fourteen-level-application-boundary", options.policy);
...
test "default output path replaces consumption report suffix" {
    const prefix = try defaultOutputPrefix(std.testing.allocator, ".zig-cache/causal-artifacts/source-ci-fourteen-level-report.json");
    defer std.testing.allocator.free(prefix);

    try std.testing.expect(fileName(prefix).len + ".json".len <= max_default_output_file_name_len);
    try std.testing.expect(std.mem.indexOf(u8, prefix, "application-boundary") != null);
}

test "plan mode records planned state without applied authority" {
    const reports = try formatReports(std.testing.allocator, .{
        .options = .{
            .report_path = "source-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report.json",
            .mode = .plan,
...
test "plan mode carries source eleven-level ten-level and nine-level lineage" {
    const reports = try formatReports(std.testing.allocator, .{
        .options = .{
            .report_path = "source-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report.json",
            .mode = .plan,
    ...
    try std.testing.expect(std.mem.indexOf(u8, reports.json, "\"source_eleven_level_policy\": \"eleven-level-policy.json\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, reports.json, "\"source_eleven_level_application_boundary\": \"eleven-level-application-boundary.json\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, reports.json, "\"source_eleven_level_report\": \"eleven-level-report.json\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, reports.json, "\"source_ten_level_policy\": \"ten-level-policy.json\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, reports.json, "\"source_ten_level_application_boundary\": \"ten-level-application-boundary.json\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, reports.json, "\"source_ten_level_report\": \"ten-level-report.json\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, reports.json, "\"source_nine_level_policy\": \"nine-level-policy.json\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, reports.json, "\"source_nine_level_application_boundary\": \"nine-level-application-boundary.json\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, reports.json, "\"source_nine_level_report\": \"nine-level-report.json\"") != null);
}

test "source refs accept explicit eleven-level ten-level and nine-level lineage without sparse eight-level report aliases" {
    const source = SourceReportArtifact{
        .schema = source_report_schema,
        .schema_version = 1,
        .source_evaluator = "ten-level-evaluator.json",
        .source_evaluator_status = "ready",
        .source_thirteen_level_evaluator = "thirteen-level-evaluator.json",
        .source_thirteen_level_evaluator_schema = "thirteen-level-evaluator-schema",
        .source_thirteen_level_evaluator_status = "ready",
        .source_thirteen_level_policy = "thirteen-level-policy.json",
        .source_thirteen_level_policy_schema = "thirteen-level-policy-schema",
        .source_thirteen_level_policy_status = "ready",
        .source_thirteen_level_application_boundary = "thirteen-level-application-boundary.json",
        .source_thirteen_level_application_boundary_schema = "thirteen-level-application-boundary-schema",
        .source_thirteen_level_application_boundary_status = "applied",
        .source_thirteen_level_report = "thirteen-level-report.json",
        .source_thirteen_level_report_schema = "thirteen-level-report-schema",
        .source_thirteen_level_report_status = "ready",
        .source_thirteen_level_after_digest = "sha256:thirteen-after",
        .source_thirteen_level_after_present = true,
        .source_twelve_level_evaluator = "twelve-level-evaluator.json",
        ...
        .source_twelve_level_after_present = true,
        .source_eleven_level_policy = "eleven-level-policy.json",
        .source_eleven_level_policy_schema = "eleven-level-policy-schema",
        .source_eleven_level_policy_status = "ready",
        .source_eleven_level_application_boundary = "eleven-level-application-boundary.json",
        .source_eleven_level_application_boundary_schema = "eleven-level-application-boundary-schema",
        .source_eleven_level_report = "eleven-level-report.json",
        .source_eleven_level_report_schema = "eleven-level-report-schema",
        .source_eleven_level_report_status = "ready",
        .source_ten_level_policy = "ten-level-policy.json",
        .source_ten_level_policy_schema = "ten-level-policy-schema",
        .source_ten_level_policy_status = "ready",
        .source_ten_level_application_boundary = "ten-level-application-boundary.json",
        .source_ten_level_application_boundary_schema = "ten-level-application-boundary-schema",
        .source_ten_level_report = "ten-level-report.json",
        .source_ten_level_report_schema = "ten-level-report-schema",
        .source_ten_level_report_status = "ready",
        .source_nine_level_policy = "nine-level-policy.json",
        .source_nine_level_policy_schema = "nine-level-policy-schema",
        .source_nine_level_policy_status = "ready",
        .source_nine_level_application_boundary = "nine-level-application-boundary.json",
        .source_nine_level_application_boundary_schema = "nine-level-application-boundary-schema",
        .source_nine_level_report = "nine-level-report.json",
        .source_nine_level_report_schema = "nine-level-report-schema",
        .source_nine_level_report_status = "ready",
        .source_eight_level_policy = "eight-level-policy.json",
        .source_eight_level_policy_schema = "eight-level-policy-schema",
        .source_eight_level_policy_status = "ready",
        .source_eight_level_application_boundary = "eight-level-application-boundary.json",
        .source_eight_level_application_boundary_schema = "eight-level-application-boundary-schema",
        .source_eight_level_application_status = "applied",
        .source_inherited_policy = "inherited-policy.json",
        .source_inherited_policy_schema = "inherited-policy-schema",
        .source_inherited_policy_status = "ready",
        .source_policy_decision = "approve",
        .source_inherited_application_boundary = "inherited-application-boundary.json",
        .source_inherited_application_boundary_schema = "inherited-application-boundary-schema",
        .source_inherited_report = "inherited-report.json",
        ...
        .source_publication_policy = "publication-policy.json",
        .source_after_report_digest = "sha256:after-report",
    ...
    try std.testing.expect(sourceRefsPresent(source));
}
...
test "record-applied requires reviewed evidence before applied is true" {
    const options = Options{
        ...
        .report_application_changes = &.{"reviewed local evaluation report evaluation report evaluation report evaluation report evaluation report evaluation report application for agents reviewers CI advisory readers and SolidJS webui"},
...
test "unsafe after report blocks application" {
    const options = Options{
        ...
        .report_application_changes = &.{"reviewed local evaluation report evaluation report evaluation report evaluation report evaluation report evaluation report application"},
...
test "blocked source report blocks application" {
    const reports = try formatReports(std.testing.allocator, .{
        .options = .{
            .report_path = "blocked-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report.json",
            .mode = .plan,
...
test "source authority drift blocks application" {
    const reports = try formatReports(std.testing.allocator, .{
        .options = .{
            .report_path = "drift-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report.json",
            .mode = .plan,
            .reason = "drift source",
        },
