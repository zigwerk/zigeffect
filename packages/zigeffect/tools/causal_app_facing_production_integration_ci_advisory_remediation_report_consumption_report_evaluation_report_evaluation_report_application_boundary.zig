...
pub const schema = "zigeffect.causal.app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-application-boundary.v1";
pub const schema_version: u32 = 1;
pub const source_branch = "codex/zigeffect-causal-app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-application-boundary";
pub const recommendation = "start-app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-policy";
pub const next_branch_if_applied = "codex/zigeffect-causal-app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-policy";

const source_report_schema = "zigeffect.causal.app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report.v1";
const generated_by = "causal-app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-application-boundary";
const source_report_suffix = "-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report.json";
const output_prefix_suffix = "-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-application-boundary";
const compact_output_prefix_name = "app-facing-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-application-boundary";
...
const Options = struct {
    ...
    mode: Mode,
    reviewed_by: []const u8 = "app-facing-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-application-boundary-reviewer",
    policy: []const u8 = "manual-app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-application-boundary",
    reason: []const u8,
...
const SourceReportArtifact = struct {
    ...
    source_evaluation_report_after_digest: []const u8 = "",
    source_evaluation_report_after_present: bool = false,
...
const boundary_rules: []const []const u8 = &.{
    "Plan mode records local consumption-report evaluation-report evaluation-report application intent only and never records applied state.",
    "record-applied requires reviewed evaluation-report evaluation-report application change evidence, before evidence, after evidence, safe after-report content, and full verification commands.",
    ...
    "Future consumption-report evaluation-report evaluation-report policy work may interpret applied records without creating mutation authority or public publication authority.",
};
...
const negative_fixtures: []const NegativeFixture = &.{
    .{ .id = "blocked-source-report-denied", .artifact_state = "consumption_report_evaluation_report_evaluation_report_status=blocked", .decision = "deny", .failed_gate = "source-report-ready", .reason = "blocked source evaluation-report evaluation reports cannot feed application records" },
    ...
    .{ .id = "missing-report-application-change-denied", .artifact_state = "record-applied evaluation_report_evaluation_report_application_changes=[]", .decision = "deny", .failed_gate = "report-application-change-present", .reason = "reviewed evaluation report evaluation report application change evidence is required" },
...
fn parseOptions(allocator: std.mem.Allocator, args: []const []const u8) !Options {
    ...
    var reviewed_by: []const u8 = "app-facing-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-application-boundary-reviewer";
    var policy: []const u8 = "manual-app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-application-boundary";
    ...
    while (index < args.len) {
        ...
        if (std.mem.eql(u8, flag, "--by")) {
            ...
            reason = value;
        } else if (std.mem.eql(u8, flag, "--evaluation-report-evaluation-report-after")) {
            if (!reportAfterPathValid(value)) return error.InvalidReportAfterPath;
            report_after_path = value;
        } else if (std.mem.eql(u8, flag, "--evaluation-report-evaluation-report-application-change")) {
...
fn evaluateBoundary(
    ...
    try appendCheck(allocator, &checks, "source-report-schema", if (std.mem.eql(u8, source.schema, source_report_schema) and source.schema_version == 1) .pass else .fail, "source consumption-report evaluation-report evaluation-report schema is supported");
    try appendCheck(allocator, &checks, "source-report-ready", if (sourceReportReady(source)) .pass else .fail, "source consumption-report evaluation report evaluation report is ready or advisory and ready for this branch with no blocked findings");
    ...
    if (options.mode == .plan) {
        try appendCheck(allocator, &checks, "plan-is-not-applied", .pass, "plan mode records local evaluation-report evaluation-report application intent without applied state");
        try appendCheck(allocator, &checks, "report-application-change-present", .skipped, "plan mode does not claim evaluation-report evaluation-report application change evidence");
    ...
    try appendCheck(allocator, &checks, "report-application-change-present", if (options.report_application_changes.len > 0) .pass else .fail, "record-applied requires reviewed evaluation-report evaluation-report application change evidence");
    ...
    try appendCheck(allocator, &checks, "after-report-safe", if (report_after_text) |after_text| reportAfterSafetyStatus(after_text) else .fail, "after-report evidence preserves read-only evaluation-report evaluation-report application constraints");
...
fn sourceReportReady(source: SourceReportArtifact) bool {
    return source.ready_for_next_branch and
        ...
        (std.mem.eql(u8, source.consumption_report_evaluation_report_evaluation_report_status, "ready") or std.mem.eql(u8, source.consumption_report_evaluation_report_evaluation_report_status, "advisory")) and
...
fn sourceRefsPresent(source: SourceReportArtifact) bool {
    return source.source_evaluator.len > 0 and
        ...
        source.source_evaluation_report_after_digest.len > 0 and
        source.source_evaluation_report_after_present and
...
fn formatBoundaryJson(
    ...
    try appendJsonField(allocator, &output, "source_consumption_report_evaluation_report_evaluation_report", options.report_path, true);
    try appendJsonField(allocator, &output, "source_consumption_report_evaluation_report_evaluation_report_status", source.consumption_report_evaluation_report_evaluation_report_status, true);
    ...
    try appendJsonField(allocator, &output, "source_evaluation_report_after_digest", source.source_evaluation_report_after_digest, true);
    try output.print(allocator, "  \"source_evaluation_report_after_present\": {},\n", .{source.source_evaluation_report_after_present});
    try appendJsonField(allocator, &output, "mode", modeText(options.mode), true);
    try appendJsonField(allocator, &output, "evaluation_report_evaluation_report_application_status", statusText(result.status), true);
    ...
    try output.appendSlice(allocator, "  \"evaluation_report_evaluation_report_after_path\": ");
    try appendOptionalJsonString(allocator, &output, options.report_after_path);
    try output.appendSlice(allocator, ",\n");
    try appendJsonField(allocator, &output, "evaluation_report_evaluation_report_after_digest", after_report_digest, true);
    try output.print(allocator, "  \"evaluation_report_evaluation_report_after_present\": {},\n", .{report_after_text != null});
    ...
    try output.appendSlice(allocator, "  \"evaluation_report_evaluation_report_application_changes\": ");
...
fn formatBoundaryText(
    ...
    try output.appendSlice(allocator, "zigeffect app-facing CI advisory remediation report consumption report evaluation-report evaluation-report application boundary\n");
    try output.print(allocator, "schema: {s}\n", .{schema});
    try output.print(allocator, "source consumption report evaluation report evaluation report: {s}\n", .{options.report_path});
    try output.print(allocator, "source consumption report evaluation report evaluation report status: {s}\n", .{source.consumption_report_evaluation_report_evaluation_report_status});
    ...
    try output.print(allocator, "source evaluation report policy: {s}\n", .{source.source_consumption_report_evaluation_report_policy});
    try output.print(allocator, "source evaluation report policy status: {s}\n", .{source.source_evaluation_report_policy_status});
    try output.print(allocator, "source evaluation report application boundary: {s}\n", .{source.source_consumption_report_evaluation_report_application_boundary});
    try output.print(allocator, "source evaluation report application status: {s}\n", .{source.source_evaluation_report_application_status});
    try output.print(allocator, "source consumption report evaluation report: {s}\n", .{source.source_consumption_report_evaluation_report});
    try output.print(allocator, "source consumption report evaluation report status: {s}\n", .{source.source_consumption_report_evaluation_report_status});
    ...
    try output.print(allocator, "evaluation_report_evaluation_report_application_status: {s}\n", .{statusText(result.status)});
    try output.print(allocator, "applied: {}\n", .{result.applied});
    try output.print(allocator, "ready_for_next_branch: {}\n", .{result.ready_for_next_branch});
    ...
    try output.print(allocator, "evaluation report evaluation report after digest: {s}\n", .{after_report_digest});
    ...
    try appendTextList(allocator, &output, "evaluation report evaluation report application changes", options.report_application_changes);
...
fn agentGuidance(status: ReportApplicationStatus) []const []const u8 {
    return switch (status) {
        .planned => &.{
            "Use planned consumption-report evaluation-report evaluation-report application boundary evidence to prepare evaluation-report evaluation-report policy work only.",
        ...
        .applied => &.{
            "Use applied consumption-report evaluation-report evaluation-report application boundary evidence to start evaluation-report evaluation-report policy work only.",
            "Cite evaluation-report evaluation-report application changes before evidence after evidence after-report digest source report checks denied claims and post-verification commands.",
        },
        .blocked => &.{
            "Treat blocked consumption-report evaluation-report evaluation-report application boundary evidence as a stop sign.",
...
fn usage() []const u8 {
    return "usage: zig build causal-app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-application-boundary -- --from-report <consumption-report-evaluation-report-evaluation-report.json> plan|record-applied --reason <reason> [--evaluation-report-evaluation-report-after <report.txt|report.md|report.json>] [--evaluation-report-evaluation-report-application-change <evidence>] [--before <evidence>] [--after <evidence>] [--verified-command <command>]... [--by <actor>] [--policy <policy>] [--out-prefix <path-prefix>]\n";
}

fn failUsage(err: anyerror) noreturn {
    std.debug.print("causal-app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-application-boundary error: {s}\n{s}", .{ @errorName(err), usage() });
    std.process.exit(2);
}
...
const safe_report_after_text =
    "zigeffect causal read-only consumption report evaluation-report evaluation report application boundary reviewed locally for agents reviewers non-blocking CI advisory readers and SolidJS webui.";
...
const ready_source_report_json =
    ...
    \\  "schema_version": 1,
    \\  "generated_by": "causal-app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report",
    \\  "source_evaluator": "evaluator.json",
    \\  "source_evaluator_schema": "zigeffect.causal.app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluator.v1",
    ...
    \\  "source_evaluation_report_after_digest": "sha256:evaluation-report-after",
    \\  "source_evaluation_report_after_present": true,
    ...
    \\  "source_blocked_findings": [],
    \\  "source_advisory_findings": [],
    \\  "consumption_report_evaluation_report_evaluation_report_status": "ready",
...
const blocked_source_report_json =
    ...
    \\  "source_consumption_report_evaluation_report_application_boundary_schema": "zigeffect.causal.app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluation-report-application-boundary.v1",
    \\  "source_consumption_report_evaluation_report": "evaluation-report.json",
    \\  "source_consumption_report_evaluation_report_status": "blocked",
    \\  "source_evaluation_report_application_status": "applied",
    \\  "source_evaluation_report_after_digest": "sha256:evaluation-report-after",
    \\  "source_evaluation_report_after_present": true,
    \\  "source_evaluation_report_application_changes": ["reviewed blocked local evaluation report application"],
    \\  "source_consumption_report_application_boundary": "report-application-boundary.json",
    \\  "source_report_application_status": "applied",
    \\  "source_consumption_report": "consumption-report.json",
    ...
    \\  "source_advisory_findings": [],
    \\  "consumption_report_evaluation_report_evaluation_report_status": "blocked",
    \\  "ready_for_next_branch": false,
...
const authority_drift_source_report_json =
    ...
    \\  "source_evaluation_report_after_digest": "sha256:evaluation-report-after",
    \\  "source_evaluation_report_after_present": true,
    \\  "source_evaluation_report_application_changes": ["reviewed authority local evaluation report application"],
    \\  "source_consumption_report_application_boundary": "report-application-boundary.json",
    \\  "source_report_application_status": "applied",
    \\  "source_consumption_report": "consumption-report.json",
    \\  "source_consumption_report_status": "ready",
...
test "parses app-facing consumption report evaluation-report evaluation-report application boundary options" {
    const args = &.{
        "tool",
        "--from-report",
        "source-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report.json",
        "record-applied",
        "--reason",
        "reviewed local report application",
        "--evaluation-report-evaluation-report-after",
        "after-report.md",
        "--evaluation-report-evaluation-report-application-change",
        "reviewed evaluation report evaluation report application change",
        "--before",
        "before evidence",
        "--after",
        "after evidence",
    ...
    try std.testing.expectEqualStrings("source-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report.json", options.report_path);
...
test "default output path replaces consumption report suffix" {
    const prefix = try defaultOutputPrefix(std.testing.allocator, ".zig-cache/causal-artifacts/source-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report.json");
    defer std.testing.allocator.free(prefix);

    try std.testing.expectEqualStrings(".zig-cache/causal-artifacts/source-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-application-boundary", prefix);
}

test "plan mode records planned state without applied authority" {
    const reports = try formatReports(std.testing.allocator, .{
        .options = .{
            .report_path = "source-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report.json",
            .mode = .plan,
    ...
    try std.testing.expect(std.mem.indexOf(u8, reports.json, "\"evaluation_report_evaluation_report_application_status\": \"planned\"") != null);
...
test "record-applied requires reviewed evidence before applied is true" {
    const options = Options{
        ...
        .report_application_changes = &.{"reviewed local evaluation report evaluation report application for agents reviewers CI advisory readers and SolidJS webui"},
    ...
    try std.testing.expect(std.mem.indexOf(u8, reports.json, "\"evaluation_report_evaluation_report_application_status\": \"applied\"") != null);
...
test "record-applied without evidence is blocked" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, reports.json, "\"evaluation_report_evaluation_report_application_status\": \"blocked\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, reports.json, "\"applied\": false") != null);
...
test "unsafe after report blocks application" {
    const options = Options{
        ...
        .report_application_changes = &.{"reviewed local evaluation report evaluation report application"},
...
test "blocked source report blocks application" {
    const reports = try formatReports(std.testing.allocator, .{
        .options = .{
            .report_path = "blocked-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report.json",
            .mode = .plan,
...
test "source authority drift blocks application" {
    const reports = try formatReports(std.testing.allocator, .{
        .options = .{
            .report_path = "drift-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report.json",
            .mode = .plan,
            .reason = "drift source",
        },
