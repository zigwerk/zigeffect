...
const source_report_schema = "zigeffect.causal.app-facing-production-integration-ci-advisory-remediation-report-consumption-report.v1";
const generated_by = "causal-app-facing-production-integration-ci-advisory-remediation-report-consumption-report-application-boundary";
const source_report_suffix = "-ci-advisory-remediation-report-consumption-report.json";
const output_prefix_suffix = "-ci-advisory-remediation-report-consumption-report-application-boundary";
const compact_output_prefix_name = "app-facing-ci-advisory-remediation-report-consumption-report-application-boundary";
...
const Options = struct {
    ...
    mode: Mode,
    reviewed_by: []const u8 = "app-facing-ci-advisory-remediation-report-consumption-report-application-boundary-reviewer",
    policy: []const u8 = "manual-app-facing-production-integration-ci-advisory-remediation-report-consumption-report-application-boundary",
    reason: []const u8,
...
const SourceReportArtifact = struct {
    ...
    consumption_report_status: []const u8,
    ...
    public_artifact_upload_enabled: bool = false,
    hosted_live_dashboard_enabled: bool = false,
    production_health_claim_enabled: bool = false,
...
const required_report_after_markers: []const []const u8 = &.{ "zigeffect", "causal", "read-only", "consumption", "report", "application" };
...
const boundary_rules: []const []const u8 = &.{
    "Plan mode records local consumption-report application intent only and never records applied state.",
    "record-applied requires reviewed report application change evidence, before evidence, after evidence, safe after-report content, and full verification commands.",
    ...
    "Future consumption-report policy work may interpret applied records without creating mutation authority or public publication authority.",
};
...
const negative_fixtures: []const NegativeFixture = &.{
    .{ .id = "blocked-source-report-denied", .artifact_state = "consumption_report_status=blocked", .decision = "deny", .failed_gate = "source-report-ready", .reason = "blocked source reports cannot feed application records" },
    ...
    .{ .id = "blocked-findings-denied", .artifact_state = "blocked_findings_count>0", .decision = "deny", .failed_gate = "source-report-ready", .reason = "blocked findings stop report application" },
    ...
    .{ .id = "missing-report-application-change-denied", .artifact_state = "record-applied report_application_changes=[]", .decision = "deny", .failed_gate = "report-application-change-present", .reason = "reviewed report application change evidence is required" },
...
fn parseOptions(allocator: std.mem.Allocator, args: []const []const u8) !Options {
    ...
    var reviewed_by: []const u8 = "app-facing-ci-advisory-remediation-report-consumption-report-application-boundary-reviewer";
    var policy: []const u8 = "manual-app-facing-production-integration-ci-advisory-remediation-report-consumption-report-application-boundary";
    ...
    while (index < args.len) {
        ...
        } else if (std.mem.eql(u8, flag, "--report-application-change")) {
...
fn evaluateBoundary(
    ...
    try appendCheck(allocator, &checks, "source-report-schema", if (std.mem.eql(u8, source.schema, source_report_schema) and source.schema_version == 1) .pass else .fail, "source consumption-report schema is supported");
    try appendCheck(allocator, &checks, "source-report-ready", if (sourceReportReady(source)) .pass else .fail, "source consumption report is ready or advisory and ready for this branch with no blocked findings");
    ...
    try appendCheck(allocator, &checks, "source-refs-present", if (sourceRefsPresent(source)) .pass else .fail, "source evaluator policy boundary readiness publication policy and digest refs are present");
    ...
    if (options.mode == .plan) {
        try appendCheck(allocator, &checks, "plan-is-not-applied", .pass, "plan mode records local report application intent without applied state");
        try appendCheck(allocator, &checks, "report-application-change-present", .skipped, "plan mode does not claim report application change evidence");
    ...
    try appendCheck(allocator, &checks, "report-application-change-present", if (options.report_application_changes.len > 0) .pass else .fail, "record-applied requires reviewed report application change evidence");
    ...
    try appendCheck(allocator, &checks, "after-report-safe", if (report_after_text) |after_text| reportAfterSafetyStatus(after_text) else .fail, "after-report evidence preserves read-only report application constraints");
...
fn sourceReportReady(source: SourceReportArtifact) bool {
    return source.ready_for_next_branch and
        ...
        (std.mem.eql(u8, source.consumption_report_status, "ready") or std.mem.eql(u8, source.consumption_report_status, "advisory")) and
...
fn sourceRefsPresent(source: SourceReportArtifact) bool {
    return source.source_evaluator.len > 0 and
        ...
        source.source_policy_status.len > 0 and
...
fn formatBoundaryJson(
    ...
    try appendJsonField(allocator, &output, "source_consumption_report", options.report_path, true);
    try appendJsonField(allocator, &output, "source_consumption_report_status", source.consumption_report_status, true);
    ...
    try output.appendSlice(allocator, ",\n");
    try appendJsonField(allocator, &output, "report_after_digest", after_report_digest, true);
    try output.print(allocator, "  \"report_after_present\": {},\n", .{report_after_text != null});
    ...
    try output.appendSlice(allocator, "  \"report_application_changes\": ");
...
fn formatBoundaryText(
    ...
    try output.appendSlice(allocator, "zigeffect app-facing CI advisory remediation report consumption report application boundary\n");
    try output.print(allocator, "schema: {s}\n", .{schema});
    try output.print(allocator, "source consumption report: {s}\n", .{options.report_path});
    try output.print(allocator, "source consumption report status: {s}\n", .{source.consumption_report_status});
    ...
    try appendTextList(allocator, &output, "report application changes", options.report_application_changes);
...
fn agentGuidance(status: ReportApplicationStatus) []const []const u8 {
    return switch (status) {
        .planned => &.{
            "Use planned consumption-report application boundary evidence to prepare report-policy work only.",
        ...
        .applied => &.{
            "Use applied consumption-report application boundary evidence to start report-policy work only.",
            "Cite report application changes before evidence after evidence after-report digest source report checks denied claims and post-verification commands.",
        },
        .blocked => &.{
            "Treat blocked consumption-report application boundary evidence as a stop sign.",
...
fn usage() []const u8 {
    return "usage: zig build causal-app-facing-production-integration-ci-advisory-remediation-report-consumption-report-application-boundary -- --from-report <consumption-report.json> plan|record-applied --reason <reason> [--report-after <report.txt|report.md|report.json>] [--report-application-change <evidence>] [--before <evidence>] [--after <evidence>] [--verified-command <command>]... [--by <actor>] [--policy <policy>] [--out-prefix <path-prefix>]\n";
}

fn failUsage(err: anyerror) noreturn {
    std.debug.print("causal-app-facing-production-integration-ci-advisory-remediation-report-consumption-report-application-boundary error: {s}\n{s}", .{ @errorName(err), usage() });
    std.process.exit(2);
}
...
const safe_report_after_text =
    "zigeffect causal read-only consumption report application boundary reviewed locally for agents reviewers non-blocking CI advisory readers and SolidJS webui.";
...
const ready_source_report_json =
    ...
    \\  "schema_version": 1,
    \\  "generated_by": "causal-app-facing-production-integration-ci-advisory-remediation-report-consumption-report",
    ...
    \\  "consumption_report_status": "ready",
...
const blocked_source_report_json =
    ...
    \\  "source_after_report_digest": "sha256:after",
    \\  "source_consumer_after_digest": "sha256:consumer",
    \\  "consumption_report_status": "blocked",
    \\  "ready_for_next_branch": false,
...
test "app-facing consumption report application boundary constants are stable" {
    ...
    try std.testing.expectEqualStrings("start-app-facing-production-integration-ci-advisory-remediation-report-consumption-report-policy", recommendation);
...
test "parses app-facing consumption report application boundary options" {
    const args = &.{
        "tool",
        "--from-report",
        "source-ci-advisory-remediation-report-consumption-report.json",
        "record-applied",
        "--reason",
        "reviewed local report application",
        "--report-after",
        "after-report.md",
        "--report-application-change",
        "reviewed report application change",
        "--before",
        "before evidence",
        "--after",
        "after evidence",
    ...
    try std.testing.expectEqualStrings("source-ci-advisory-remediation-report-consumption-report.json", options.report_path);
...
test "default output path replaces consumption report suffix" {
    const prefix = try defaultOutputPrefix(std.testing.allocator, ".zig-cache/causal-artifacts/source-ci-advisory-remediation-report-consumption-report.json");
    defer std.testing.allocator.free(prefix);

    try std.testing.expectEqualStrings(".zig-cache/causal-artifacts/source-ci-advisory-remediation-report-consumption-report-application-boundary", prefix);
}
...
test "record-applied requires reviewed evidence before applied is true" {
    const options = Options{
        ...
        .report_application_changes = &.{"reviewed local report application for agents reviewers CI advisory readers and SolidJS webui"},
    ...
    try std.testing.expect(std.mem.indexOf(u8, reports.json, "\"report_application_status\": \"applied\"") != null);
...
test "record-applied without evidence is blocked" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, reports.json, "\"report_application_status\": \"blocked\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, reports.json, "\"applied\": false") != null);
...
test "unsafe after report blocks application" {
    const options = Options{
        ...
        .report_application_changes = &.{"reviewed local report application"},
    ...
    const reports = try formatReports(std.testing.allocator, .{
        .options = options,
        .source_report_json = ready_source_report_json,
        .report_after_text = "zigeffect causal read-only consumption report application app_runtime_integration_enabled=true",
    });
    defer reports.deinit(std.testing.allocator);
...
test "blocked source report blocks application" {
    const reports = try formatReports(std.testing.allocator, .{
        .options = .{
            .report_path = "blocked-ci-advisory-remediation-report-consumption-report.json",
            .mode = .plan,
...
test "source authority drift blocks application" {
    const reports = try formatReports(std.testing.allocator, .{
        .options = .{
            .report_path = "drift-ci-advisory-remediation-report-consumption-report.json",
            .mode = .plan,
            .reason = "drift source",
        },
