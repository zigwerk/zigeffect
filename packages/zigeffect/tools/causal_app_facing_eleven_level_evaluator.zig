...
const Options = struct {
    ...
    reason: []const u8,
    reviewed_by: []const u8 = "app-facing-eleven-level-evaluator-reviewer",
    policy: []const u8 = "manual-app-facing-eleven-level-evaluator",
...
fn parseOptions(allocator: std.mem.Allocator, args: []const []const u8) !Options {
    ...
    var reviewed_by: []const u8 = "app-facing-eleven-level-evaluator-reviewer";
    var policy: []const u8 = "manual-app-facing-eleven-level-evaluator";
...
fn classifyFile(path: []const u8, contents: []const u8, detected_schema: []const u8) FileClass {
    ...
    if (is_json and (std.mem.indexOf(u8, path, "eleven-level-policy") != null or std.mem.indexOf(u8, path, "consumption-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-policy") != null or std.mem.eql(u8, detected_schema, source_policy_schema))) return .policy_json;
...
fn sourceRefsPresent(source: SourcePolicyArtifact) bool {
    return sourceElevenLevelApplicationBoundary(source).len > 0 and
        ...
        sourceElevenLevelAfterDigest(source).len > 0 and
        sourceElevenLevelAfterPresent(source) and
...
fn sourceElevenLevelApplicationBoundary(source: SourcePolicyArtifact) []const u8 {
    return source.source_eleven_level_application_boundary;
}

fn sourceElevenLevelApplicationBoundarySchema(source: SourcePolicyArtifact) []const u8 {
    return source.source_eleven_level_application_boundary_schema;
}

fn sourceElevenLevelReport(source: SourcePolicyArtifact) []const u8 {
    return source.source_eleven_level_report;
}

fn sourceElevenLevelReportSchema(source: SourcePolicyArtifact) []const u8 {
    return source.source_eleven_level_report_schema;
}

fn sourceElevenLevelReportStatus(source: SourcePolicyArtifact) []const u8 {
...
fn sourceElevenLevelAfterDigest(source: SourcePolicyArtifact) []const u8 {
    if (source.source_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_after_digest.len > 0) return source.source_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_after_digest;
...
fn sourceElevenLevelAfterPresent(source: SourcePolicyArtifact) bool {
    return source.source_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_after_present or
        source.source_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_after_present or
...
fn formatEvaluatorJson(
    ...
    try appendJsonField(allocator, &output, "source_eleven_level_policy", options.policy_path, true);
    try appendJsonField(allocator, &output, "source_eleven_level_policy_schema", source.schema, true);
    try appendJsonField(allocator, &output, "source_eleven_level_policy_status", source.consumption_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_policy_status, true);
    ...
    try appendJsonField(allocator, &output, "source_eleven_level_after_digest", sourceElevenLevelAfterDigest(source), true);
    try output.print(allocator, "  \"source_eleven_level_after_present\": {},\n", .{sourceElevenLevelAfterPresent(source)});
    ...
    try output.appendSlice(allocator, "  \"source_eleven_level_application_changes\": ");
    try appendStringArray(allocator, &output, source.source_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_application_changes);
...
fn formatEvaluatorText(
    ...
    try appendTextList(allocator, &output, "source eleven-level application changes", source.source_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_application_changes);
...
fn nextQueries(status: EvaluationStatus) []const []const u8 {
    return switch (status) {
        .ready => &.{
            "causal-query --artifact <app-facing-ci-eleven-level-evaluator.json> --kind request",
            "causal-query --artifact <app-facing-ci-eleven-level-evaluator.json> --kind denied-claim",
            "start codex/zigeffect-causal-app-facing-twelve-level-report with this evaluator artifact",
        },
        .advisory_findings => &.{
            "add support evidence for the request and rerun the evaluator",
            "inspect advisory findings before rendering a twelve-level report",
        },
        .blocked => &.{
...
fn agentGuidance(status: EvaluationStatus) []const []const u8 {
    return switch (status) {
        .ready => &.{
            "Ready eleven-level evaluator artifacts may feed a local twelve-level report.",
...
fn usage() []const u8 {
    return "usage: zig build causal-app-facing-eleven-level-evaluator -- --from-policy <eleven-level-policy.json> evaluate --reason <reason> --request <path>... [--evidence <path>]... [--by <actor>] [--policy <policy>] [--out-prefix <path-prefix>]\n";
}

fn failUsage(err: anyerror) noreturn {
    std.debug.print("causal-app-facing-eleven-level-evaluator error: {s}\n{s}", .{ @errorName(err), usage() });
    std.process.exit(2);
}
...
test "app-facing eleven-level evaluator constants are stable" {
    try std.testing.expectEqualStrings(
        "zigeffect.causal.app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluator.v1",
        schema,
    );
    try std.testing.expectEqual(@as(u32, 1), schema_version);
    try std.testing.expectEqualStrings("codex/zigeffect-causal-app-facing-eleven-level-evaluator", source_branch);
    try std.testing.expectEqualStrings("start-app-facing-twelve-level-report", recommendation);
    try std.testing.expectEqualStrings("codex/zigeffect-causal-app-facing-twelve-level-report", next_branch_if_ready);
}

test "parses app-facing eleven-level evaluator options with requests and evidence" {
    var options = try parseOptions(std.testing.allocator, &.{
        ...
        "codex",
        "--policy",
        "manual-app-facing-eleven-level-evaluator",
        "--request",
        ...
        "--out-prefix",
        ".zig-cache/causal-artifacts/custom-app-facing-ci-eleven-level-evaluator",
    });
    defer options.deinit(std.testing.allocator);

    try std.testing.expectEqualStrings(".zig-cache/causal-artifacts/app-facing-ci-eleven-level-policy.json", options.policy_path);
    ...
    try std.testing.expectEqualStrings("manual-app-facing-eleven-level-evaluator", options.policy);
    ...
    try std.testing.expectEqualStrings(".zig-cache/causal-artifacts/custom-app-facing-ci-eleven-level-evaluator", options.out_prefix.?);
}
...
test "default output path replaces report policy suffix" {
    ...
    try std.testing.expectEqualStrings(".zig-cache/causal-artifacts/app-facing-ci-eleven-level-evaluator.json", paths.json_path);
    try std.testing.expectEqualStrings(".zig-cache/causal-artifacts/app-facing-ci-eleven-level-evaluator.txt", paths.text_path);
}
...
test "classifies request policy causal workbench and denied files" {
    var evidence = try analyzeFiles(std.testing.allocator, &.{
        ...
        .{ .path = ".zig-cache/causal-artifacts/app-facing-ci-eleven-level-policy.json", .contents = readyPolicyJson() },
...
test "ready advisory and blocked evaluator output preserve read-only authority" {
    var ready_options = try parseOptions(std.testing.allocator, &.{
        "tool",
        "--from-policy",
        "source-app-facing-ci-eleven-level-policy.json",
        "evaluate",
        "--reason",
    ...
    try std.testing.expect(std.mem.indexOf(u8, ready_reports.json, "\"source_eleven_level_policy\": \"source-app-facing-ci-eleven-level-policy.json\"") != null);
    ...
    try std.testing.expect(std.mem.indexOf(u8, ready_reports.json, "\"source_eleven_level_after_digest\": \"sha256:eleven-after\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, ready_reports.json, "\"source_eleven_level_after_present\": true") != null);
