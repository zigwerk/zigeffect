...
pub const schema = "zigeffect.causal.app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluator.v1";
pub const schema_version: u32 = 1;
pub const source_branch = "codex/zigeffect-causal-app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluator";
pub const recommendation = "start-app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluation-report";
pub const next_branch_if_ready = "codex/zigeffect-causal-app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluation-report";

const source_policy_schema = "zigeffect.causal.app-facing-production-integration-ci-advisory-remediation-report-consumption-report-policy.v1";
const generated_by = "causal-app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluator";
const source_policy_suffix = "-ci-advisory-remediation-report-consumption-report-policy.json";
const output_prefix_suffix = "-ci-advisory-remediation-report-consumption-report-evaluator";
const compact_output_prefix_name = "app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluator";
...
const Options = struct {
    ...
    reason: []const u8,
    reviewed_by: []const u8 = "app-facing-ci-advisory-remediation-report-consumption-report-evaluator-reviewer",
    policy: []const u8 = "manual-app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluator",
...
fn parseOptions(allocator: std.mem.Allocator, args: []const []const u8) !Options {
    ...
    var reviewed_by: []const u8 = "app-facing-ci-advisory-remediation-report-consumption-report-evaluator-reviewer";
    var policy: []const u8 = "manual-app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluator";
...
fn classifyFile(path: []const u8, contents: []const u8, detected_schema: []const u8) FileClass {
    ...
    if (is_json and (std.mem.indexOf(u8, path, "consumption-report-policy") != null or std.mem.eql(u8, detected_schema, source_policy_schema))) return .policy_json;
...
fn fileDeniedReason(path: []const u8, contents: []const u8, class: FileClass) []const u8 {
    ...
    if (std.mem.indexOf(u8, contents, "secrets.") != null) return "secret-shaped reference marker is not valid consumption report evaluator input";
    if (std.mem.indexOf(u8, contents, "PRODUCTION_") != null) return "production secret-shaped environment marker is not valid consumption report evaluator input";
    if (std.mem.indexOf(u8, contents, "BEGIN PRIVATE KEY") != null) return "private key material is not valid consumption report evaluator input";
    if (std.mem.indexOf(u8, contents, "Authorization:") != null) return "authorization header material is not valid consumption report evaluator input";
    if (std.mem.indexOf(u8, contents, "sk-") != null) return "OpenAI-style secret token marker is not valid consumption report evaluator input";
    if (std.mem.indexOf(u8, contents, "ghp_") != null) return "GitHub token marker is not valid consumption report evaluator input";
    if (std.mem.indexOf(u8, contents, "xoxb-") != null) return "Slack token marker is not valid consumption report evaluator input";
    if (std.mem.indexOf(u8, contents, "\"raw_prompt\"") != null) return "raw prompt capture is not valid consumption report evaluator input";
    if (std.mem.indexOf(u8, contents, "\"raw_response\"") != null) return "raw response capture is not valid consumption report evaluator input";
    ...
    if (std.mem.indexOf(u8, contents, "cockroach") != null) return "Cockroach scope is not valid consumption report evaluator input";
    if (std.mem.indexOf(u8, contents, "deployment success") != null or jsonBoolTrue(contents, "deployment_mutation_enabled")) return "deployment claims are not valid consumption report evaluator input";
    if (std.mem.indexOf(u8, contents, "production health") != null or std.mem.indexOf(u8, contents, "production_health") != null) return "production health claims are not valid consumption report evaluator input";
    ...
    if (std.mem.indexOf(u8, contents, "auto-apply") != null or std.mem.indexOf(u8, contents, "auto_apply") != null) return "auto-apply claims are not valid consumption report evaluator input";
    if (std.mem.indexOf(u8, contents, "\"mutation_authority\": \"granted\"") != null) return "mutation authority must remain none";
    if (std.mem.indexOf(u8, contents, "artifact_visibility=public") != null) return "public artifact upload claims are not valid consumption report evaluator input";
    _ = path;
    return "";
}
...
fn evaluatePolicyAndFiles(
    ...
    try appendCheckAndFinding(allocator, &checks, &findings, "source-report-policy-ready", std.mem.eql(u8, source.consumption_report_policy_status, "ready"), "source-report-policy-valid", "source consumption report policy status is ready");
...
fn sourceRefsPresent(source: SourcePolicyArtifact) bool {
    return source.source_consumption_report_application_boundary.len > 0 and
...
fn formatEvaluatorJson(
    ...
    try appendJsonField(allocator, &output, "source_consumption_report_policy", options.policy_path, true);
    try appendJsonField(allocator, &output, "source_consumption_report_policy_schema", source.schema, true);
    try appendJsonField(allocator, &output, "source_report_policy_status", source.consumption_report_policy_status, true);
...
fn formatEvaluatorText(
    ...
    try output.appendSlice(allocator, "app-facing CI advisory remediation report consumption report evaluator\n");
...
fn nextQueries(status: EvaluationStatus) []const []const u8 {
    return switch (status) {
        .ready => &.{
            "causal-query --artifact <consumption-report-evaluator.json> --kind request",
            "causal-query --artifact <consumption-report-evaluator.json> --kind denied-claim",
            "start consumption-report-evaluation-report branch with this evaluator artifact",
        },
        .advisory_findings => &.{
...
fn agentGuidance(status: EvaluationStatus) []const []const u8 {
    return switch (status) {
        .ready => &.{
            "Ready consumption report evaluator artifacts may feed a local consumption-report evaluation report.",
...
fn usage() []const u8 {
    return "usage: zig build causal-app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluator -- --from-policy <consumption-report-policy.json> evaluate --reason <reason> --request <path>... [--evidence <path>]... [--by <actor>] [--policy <policy>] [--out-prefix <path-prefix>]\n";
}

fn failUsage(err: anyerror) noreturn {
    std.debug.print("causal-app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluator error: {s}\n{s}", .{ @errorName(err), usage() });
    std.process.exit(2);
}
...
fn readyPolicyJson() []const u8 {
    return
        ...
        \\  "schema_version": 1,
        \\  "decision": "approve",
        \\  "consumption_report_policy_status": "ready",
...
fn blockedPolicyJson() []const u8 {
    return
        ...
        \\  "schema_version": 1,
        \\  "decision": "reject",
        \\  "consumption_report_policy_status": "blocked",
...
test "parses app-facing consumption report evaluator options with requests and evidence" {
    var options = try parseOptions(std.testing.allocator, &.{
        "zigeffect-causal-app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluator",
        "--from-policy",
        ...
        "codex",
        "--policy",
        "manual-consumption-report-evaluator",
        "--request",
        ...
        "--out-prefix",
        ".zig-cache/causal-artifacts/custom-consumption-report-evaluator",
    });
    defer options.deinit(std.testing.allocator);

    try std.testing.expectEqualStrings(".zig-cache/causal-artifacts/app-facing-ci-advisory-remediation-report-consumption-report-policy.json", options.policy_path);
    ...
    try std.testing.expectEqualStrings("manual-consumption-report-evaluator", options.policy);
    ...
    try std.testing.expectEqualStrings(".zig-cache/causal-artifacts/custom-consumption-report-evaluator", options.out_prefix.?);
}
...
test "default output path replaces report policy suffix and compacts long names" {
    ...
    try std.testing.expectEqualStrings(
        ".zig-cache/causal-artifacts/app-facing-ci-advisory-remediation-report-consumption-report-evaluator.txt",
        paths.text_path,
    );
}
...
test "classifies request policy causal workbench and denied files" {
    var evidence = try analyzeFiles(std.testing.allocator, &.{
        ...
        .{ .path = ".zig-cache/causal-artifacts/app-facing-consumption-report-policy.json", .contents = readyPolicyJson() },
...
test "ready advisory and blocked evaluator output preserve read-only authority" {
    var ready_options = try parseOptions(std.testing.allocator, &.{
        "tool",
        "--from-policy",
        "source-consumption-report-policy.json",
        "evaluate",
        "--reason",
