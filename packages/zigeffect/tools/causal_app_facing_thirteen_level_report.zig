...
const Options = struct {
    ...
    reason: []const u8,
    reviewed_by: []const u8 = "app-facing-thirteen-level-report-reviewer",
    policy: []const u8 = "manual-app-facing-thirteen-level-report",
...
fn parseOptions(args: []const []const u8) !Options {
    ...
    var reviewed_by: []const u8 = "app-facing-thirteen-level-report-reviewer";
    var policy: []const u8 = "manual-app-facing-thirteen-level-report";
...
fn evaluateReport(allocator: std.mem.Allocator, source: EvaluatorArtifact) !ReportResult {
    ...
    try appendCheck(allocator, &checks, "source-twelve-level-evaluator-schema", if (sourceSchemaValid(source)) .pass else .fail, "source twelve-level evaluator schema is supported");
    try appendCheck(allocator, &checks, "source-twelve-level-evaluator-reportable", if (sourceReportable(source)) .pass else .fail, "source evaluator status is ready or advisory-findings");
    try appendCheck(allocator, &checks, "source-twelve-level-ready-for-next-branch", if (source.ready_for_next_branch) .pass else .fail, "source evaluator is ready for thirteen-level report work");
    try appendCheck(allocator, &checks, "source-twelve-level-no-blocked-findings", if (sourceHasNoBlockedFindings(source)) .pass else .fail, "source evaluator has no blocked findings");
    try appendCheck(allocator, &checks, "source-twelve-level-policy-ready", if (sourcePolicyReady(source)) .pass else .fail, "source twelve-level policy application report and lower lineage remain ready");
    try appendCheck(allocator, &checks, "source-twelve-level-application-evidence", if (sourceReportApplicationEvidencePresent(source)) .pass else .fail, "source twelve-level eleven-level ten-level and inherited report application evidence next queries and local channels are present");
...
fn sourcePolicyReady(source: EvaluatorArtifact) bool {
    return sourceTwelveLevelPolicy(source).len > 0 and
...
fn formatReportJson(
    ...
    try appendJsonField(allocator, &output, "source_twelve_level_evaluator", options.evaluator_path, true);
    try appendJsonField(allocator, &output, "source_twelve_level_evaluator_schema", source.schema, true);
    try appendJsonField(allocator, &output, "source_twelve_level_evaluator_status", source.evaluation_status, true);
    ...
    try appendJsonField(allocator, &output, "consumption_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_status", reportStatusText(result.status), true);
...
fn formatReportText(
    ...
    try output.appendSlice(allocator, "zigeffect app-facing thirteen-level report\n");
    ...
    try output.print(allocator, "consumption report evaluation report evaluation report evaluation report evaluation report evaluation report evaluation report evaluation report evaluation report evaluation report evaluation report evaluation report evaluation report status: {s}\n", .{reportStatusText(result.status)});
...
fn headlineForStatus(status: ReportStatus) []const u8 {
    return switch (status) {
        .ready => "App-facing twelve-level evaluator evidence is ready for thirteen-level report presentation.",
        .advisory => "App-facing twelve-level evaluator evidence is reportable with advisory findings.",
        .blocked => "App-facing twelve-level evaluator evidence is blocked and must not be used as reportable context.",
    };
}
...
fn usage() []const u8 {
    return "usage: zig build causal-app-facing-thirteen-level-report -- --from-evaluator <twelve-level-evaluator.json> summarize --reason <reason> [--by <actor>] [--policy <policy>] [--out-prefix <path-prefix>]\n";
}

fn failUsage(err: anyerror) noreturn {
    std.debug.print("causal-app-facing-thirteen-level-report error: {s}\n{s}", .{ @errorName(err), usage() });
    std.process.exit(2);
}
...
test "parses app-facing thirteen-level report options" {
    const options = try parseOptions(&.{
        "zigeffect-causal-app-facing-thirteen-level-report",
        "--from-evaluator",
        ".zig-cache/causal-artifacts/app-facing-ci-twelve-level-evaluator.json",
        "summarize",
        "--reason",
        "reviewed app-facing thirteen-level report",
        "--by",
        "codex",
        "--policy",
        "manual-app-facing-thirteen-level-report",
        "--out-prefix",
        ".zig-cache/causal-artifacts/custom-thirteen-level-report",
    });

    try std.testing.expectEqualStrings(".zig-cache/causal-artifacts/app-facing-ci-twelve-level-evaluator.json", options.evaluator_path);
    try std.testing.expectEqualStrings("reviewed app-facing thirteen-level report", options.reason);
    try std.testing.expectEqualStrings("codex", options.reviewed_by);
    try std.testing.expectEqualStrings("manual-app-facing-thirteen-level-report", options.policy);
    try std.testing.expectEqualStrings(".zig-cache/causal-artifacts/custom-thirteen-level-report", options.out_prefix.?);
}

test "app-facing thirteen-level report rejects invalid options" {
...
test "default output path replaces twelve-level evaluator suffix" {
    const paths = try outputPathsForOptions(std.testing.allocator, .{
        .evaluator_path = "../../.zig-cache/causal-artifacts/example-ci-twelve-level-evaluator.json",
        .reason = "reviewed",
    });
    defer paths.deinit(std.testing.allocator);

    try std.testing.expectEqualStrings("../../.zig-cache/causal-artifacts/example-ci-thirteen-level-report.json", paths.json_path);
    try std.testing.expectEqualStrings("../../.zig-cache/causal-artifacts/example-ci-thirteen-level-report.txt", paths.text_path);
}
...
test "ready evaluator produces ready thirteen-level report" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, reports.json, "\"schema\": \"zigeffect.causal.app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report.v1\"") != null);
    ...
    try std.testing.expect(std.mem.indexOf(u8, reports.json, "\"next_branch_if_ready\": \"codex/zigeffect-causal-app-facing-thirteen-level-application-boundary\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, reports.json, "\"source_twelve_level_evaluator\": \"../../.zig-cache/causal-artifacts/app-facing-ci-twelve-level-evaluator.json\"") != null);
    ...
    try std.testing.expect(std.mem.indexOf(u8, reports.text, "consumption report evaluation report evaluation report evaluation report evaluation report evaluation report evaluation report evaluation report evaluation report evaluation report evaluation report evaluation report evaluation report status: ready") != null);

    var parsed = try std.json.parseFromSlice(std.json.Value, std.testing.allocator, reports.json, .{});
    parsed.deinit();
}

test "advisory evaluator produces advisory thirteen-level report" {
    const reports = try formatReports(std.testing.allocator, .{
        .options = sampleOptions(),
...
test "blocked evaluator produces blocked thirteen-level report" {
    const reports = try formatReports(std.testing.allocator, .{
        .options = sampleOptions(),
...
fn sampleOptions() Options {
    return .{
        .evaluator_path = "../../.zig-cache/causal-artifacts/app-facing-ci-twelve-level-evaluator.json",
        .reason = "reviewed app-facing thirteen-level report",
    };
}
...
const sample_ready_evaluator_json =
    ...
    \\  "schema": "zigeffect.causal.app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluator.v1",
    \\  "schema_version": 1,
    \\  "generated_by": "causal-app-facing-twelve-level-evaluator",
