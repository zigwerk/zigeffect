...
const Options = struct {
    ...
    reason: []const u8,
    reviewed_by: []const u8 = "app-facing-twelve-level-report-reviewer",
    policy: []const u8 = "manual-app-facing-twelve-level-report",
...
fn parseOptions(args: []const []const u8) !Options {
    ...
    var reviewed_by: []const u8 = "app-facing-twelve-level-report-reviewer";
    var policy: []const u8 = "manual-app-facing-twelve-level-report";
...
fn evaluateReport(allocator: std.mem.Allocator, source: EvaluatorArtifact) !ReportResult {
    ...
    try appendCheck(allocator, &checks, "source-evaluator-schema", if (sourceSchemaValid(source)) .pass else .fail, "source eleven-level evaluator schema is supported");
    ...
    try appendCheck(allocator, &checks, "source-ready-for-next-branch", if (source.ready_for_next_branch) .pass else .fail, "source evaluator is ready for twelve-level report work");
    ...
    try appendCheck(allocator, &checks, "source-eleven-level-policy-ready", if (sourcePolicyReady(source)) .pass else .fail, "source eleven-level policy application report and inherited refs remain ready");
    try appendCheck(allocator, &checks, "source-eleven-level-application-evidence", if (sourceReportApplicationEvidencePresent(source)) .pass else .fail, "source eleven-level ten-level and inherited report application evidence next queries and local channels are present");
...
fn sourcePolicyReady(source: EvaluatorArtifact) bool {
    return sourceElevenLevelPolicy(source).len > 0 and
...
fn formatReportJson(
    ...
    try appendJsonField(allocator, &output, "consumption_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_status", reportStatusText(result.status), true);
...
fn formatReportText(
    ...
    try output.appendSlice(allocator, "zigeffect app-facing twelve-level report\n");
...
fn headlineForStatus(status: ReportStatus) []const u8 {
    return switch (status) {
        .ready => "App-facing eleven-level evaluator evidence is ready for twelve-level report presentation.",
        .advisory => "App-facing eleven-level evaluator evidence is reportable with advisory findings.",
        .blocked => "App-facing eleven-level evaluator evidence is blocked and must not be used as reportable context.",
    };
}
...
fn usage() []const u8 {
    return "usage: zig build causal-app-facing-twelve-level-report -- --from-evaluator <eleven-level-evaluator.json> summarize --reason <reason> [--by <actor>] [--policy <policy>] [--out-prefix <path-prefix>]\n";
}

fn failUsage(err: anyerror) noreturn {
    std.debug.print("causal-app-facing-twelve-level-report error: {s}\n{s}", .{ @errorName(err), usage() });
    std.process.exit(2);
}

test "app-facing twelve-level report constants are stable" {
    try std.testing.expectEqualStrings("zigeffect.causal.app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report.v1", schema);
    try std.testing.expectEqual(@as(u32, 1), schema_version);
    try std.testing.expectEqualStrings("codex/zigeffect-causal-app-facing-twelve-level-report", source_branch);
    try std.testing.expectEqualStrings("start-app-facing-twelve-level-application-boundary", recommendation);
    try std.testing.expectEqualStrings("codex/zigeffect-causal-app-facing-twelve-level-application-boundary", next_branch_if_ready);
}

test "parses app-facing twelve-level report options" {
    const options = try parseOptions(&.{
        "zigeffect-causal-app-facing-twelve-level-report",
        "--from-evaluator",
        ".zig-cache/causal-artifacts/app-facing-ci-eleven-level-evaluator.json",
        "summarize",
        "--reason",
        "reviewed app-facing twelve-level report",
        "--by",
        "codex",
        "--policy",
        "manual-app-facing-twelve-level-report",
        "--out-prefix",
        ".zig-cache/causal-artifacts/custom-twelve-level-report",
    });

    try std.testing.expectEqualStrings(".zig-cache/causal-artifacts/app-facing-ci-eleven-level-evaluator.json", options.evaluator_path);
    try std.testing.expectEqualStrings("reviewed app-facing twelve-level report", options.reason);
    try std.testing.expectEqualStrings("codex", options.reviewed_by);
    try std.testing.expectEqualStrings("manual-app-facing-twelve-level-report", options.policy);
    try std.testing.expectEqualStrings(".zig-cache/causal-artifacts/custom-twelve-level-report", options.out_prefix.?);
}

test "app-facing twelve-level report rejects invalid options" {
...
test "default output path replaces eleven-level evaluator suffix" {
    const paths = try outputPathsForOptions(std.testing.allocator, .{
        .evaluator_path = "../../.zig-cache/causal-artifacts/example-ci-eleven-level-evaluator.json",
        .reason = "reviewed",
    });
    defer paths.deinit(std.testing.allocator);

    try std.testing.expectEqualStrings("../../.zig-cache/causal-artifacts/example-ci-twelve-level-report.json", paths.json_path);
    try std.testing.expectEqualStrings("../../.zig-cache/causal-artifacts/example-ci-twelve-level-report.txt", paths.text_path);
}
...
test "ready evaluator produces ready twelve-level report" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, reports.json, "\"next_branch_if_ready\": \"codex/zigeffect-causal-app-facing-twelve-level-application-boundary\"") != null);
...
test "advisory evaluator produces advisory twelve-level report" {
    const reports = try formatReports(std.testing.allocator, .{
        .options = sampleOptions(),
...
test "blocked evaluator produces blocked twelve-level report" {
    const reports = try formatReports(std.testing.allocator, .{
        .options = sampleOptions(),
...
fn sampleOptions() Options {
    return .{
        .evaluator_path = "../../.zig-cache/causal-artifacts/app-facing-ci-eleven-level-evaluator.json",
        .reason = "reviewed app-facing twelve-level report",
    };
}
...
const sample_ready_evaluator_json =
    ...
    \\  "schema": "zigeffect.causal.app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluator.v1",
    \\  "schema_version": 1,
    \\  "generated_by": "causal-app-facing-eleven-level-evaluator",
