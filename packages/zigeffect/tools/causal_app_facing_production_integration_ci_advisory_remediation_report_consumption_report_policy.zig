...
const source_boundary_schema = "zigeffect.causal.app-facing-production-integration-ci-advisory-remediation-report-consumption-report-application-boundary.v1";
const generated_by = "causal-app-facing-production-integration-ci-advisory-remediation-report-consumption-report-policy";
const source_boundary_suffix = "-ci-advisory-remediation-report-consumption-report-application-boundary.json";
const output_prefix_suffix = "-ci-advisory-remediation-report-consumption-report-policy";
const compact_output_prefix_name = "app-facing-production-integration-ci-advisory-remediation-report-consumption-report-policy";
...
const Options = struct {
    ...
    decision: Decision,
    reviewed_by: []const u8 = "app-facing-ci-advisory-remediation-report-consumption-report-policy-reviewer",
    policy: []const u8 = "manual-app-facing-production-integration-ci-advisory-remediation-report-consumption-report-policy",
    reason: []const u8,
...
const SourceApplicationArtifact = struct {
    ...
    report_after_digest: []const u8 = "",
    report_after_present: bool = false,
...
const interpretation_rules: []const InterpretationRule = &.{
    ...
    .{ .id = "future-consumption-report-evaluator-input", .consumer_role = "future-consumption-report-evaluator", .allowed_use = "input to bounded read-only consumption request classification only", .failure_effect = "evaluator-input", .denied_claim = "app runtime integration" },
};

const consumption_scopes: []const ConsumptionScope = &.{
    .{ .id = "local-json-artifact", .visibility_class = "local-redacted", .executed_by_tool = false, .mutation_authority = "none", .interpretation_scope = "machine-readable consumption report policy evidence" },
    .{ .id = "local-text-report", .visibility_class = "local-redacted", .executed_by_tool = false, .mutation_authority = "none", .interpretation_scope = "human-readable consumption report policy evidence" },
    ...
    .{ .id = "future-consumption-report-evaluator-input", .visibility_class = "local-evaluator-input", .executed_by_tool = false, .mutation_authority = "none", .interpretation_scope = "future bounded consumption report evaluator input" },
};
...
const negative_fixtures: []const NegativeFixture = &.{
    .{ .id = "planned-application-boundary-denied", .artifact_state = "report_application_status=planned", .decision = "deny", .failed_gate = "source-applied-application", .reason = "policy approval requires an applied consumption-report application-boundary artifact" },
    .{ .id = "missing-report-application-evidence-denied", .artifact_state = "report_application_changes=[]", .decision = "deny", .failed_gate = "source-application-evidence-present", .reason = "policy approval requires reviewed report application evidence" },
    ...
    .{ .id = "runtime-integration-denied", .artifact_state = "app_runtime_integration_enabled=true", .decision = "deny", .failed_gate = "source-authority-disabled", .reason = "consumption report policy cannot infer app runtime integration" },
    .{ .id = "nendb-adapter-execution-denied", .artifact_state = "nendb_adapter_execution_enabled=true", .decision = "deny", .failed_gate = "source-authority-disabled", .reason = "consumption report policy cannot execute a NenDB adapter" },
    .{ .id = "public-upload-denied", .artifact_state = "public_artifact_upload_enabled=true", .decision = "deny", .failed_gate = "source-authority-disabled", .reason = "consumption report policy cannot publish public artifacts" },
    ...
    .{ .id = "required-status-check-denied", .artifact_state = "ci_required_status_check_enabled=true", .decision = "deny", .failed_gate = "source-authority-disabled", .reason = "consumption report policy cannot create or prove required status checks" },
    .{ .id = "production-health-denied", .artifact_state = "claim=production-health-proof", .decision = "deny", .failed_gate = "denied-inference-rules", .reason = "read-only consumption report policy cannot prove production health" },
...
fn parseOptions(allocator: std.mem.Allocator, args: []const []const u8) !Options {
    ...
    var reviewed_by: []const u8 = "app-facing-ci-advisory-remediation-report-consumption-report-policy-reviewer";
    var policy: []const u8 = "manual-app-facing-production-integration-ci-advisory-remediation-report-consumption-report-policy";
...
fn evaluatePolicy(allocator: std.mem.Allocator, options: Options, source: SourceApplicationArtifact) !PolicyResult {
    ...
    try appendCheck(allocator, &checks, "source-schema", std.mem.eql(u8, source.schema, source_boundary_schema) and source.schema_version == 1, "source uses app-facing consumption-report application-boundary schema v1");
    try appendCheck(allocator, &checks, "source-applied-application", sourceIsAppliedBoundary(source), "source is record-applied applied ready record-only application-boundary evidence");
    try appendCheck(allocator, &checks, "source-refs-present", sourceRefsPresent(source), "source consumption report evaluator policy boundary readiness publication policy and digest refs are present");
    try appendCheck(allocator, &checks, "source-application-evidence-present", sourceEvidencePresent(source), "source report application changes before after summaries denied claims next queries publication channels and negative fixtures are present");
    ...
    if (options.decision == .approve) {
        ...
        try appendCheck(allocator, &checks, "reviewer-decision", false, "reviewer rejected the consumption report policy");
    }
...
fn sourceRefsPresent(source: SourceApplicationArtifact) bool {
    return source.source_consumption_report.len > 0 and
...
fn formatPolicyJson(allocator: std.mem.Allocator, options: Options, source: SourceApplicationArtifact, result: PolicyResult, paths: OutputPaths) ![]const u8 {
    ...
    try appendJsonField(allocator, &output, "source_consumption_report_application_boundary", options.application_path, true);
    try appendJsonField(allocator, &output, "source_consumption_report_application_boundary_schema", source.schema, true);
    ...
    try appendJsonField(allocator, &output, "source_report_after_digest", source.report_after_digest, true);
    try output.print(allocator, "  \"source_report_after_present\": {},\n", .{source.report_after_present});
    ...
    try appendJsonField(allocator, &output, "consumption_report_policy_status", policyStatusText(result.status), true);
    ...
    try output.appendSlice(allocator, "  \"source_report_application_changes\": ");
    try appendStringArray(allocator, &output, source.report_application_changes);
...
fn formatPolicyText(allocator: std.mem.Allocator, options: Options, source: SourceApplicationArtifact, result: PolicyResult, paths: OutputPaths) ![]const u8 {
    ...
    try output.appendSlice(allocator, "app-facing CI advisory remediation report consumption report policy\n");
    try output.print(allocator, "schema: {s}\n", .{schema});
    try output.print(allocator, "schema_version: {d}\n", .{schema_version});
    try output.print(allocator, "generated_by: {s}\n", .{generated_by});
    try output.print(allocator, "decision: {s}\n", .{decisionText(options.decision)});
    try output.print(allocator, "consumption_report_policy_status: {s}\n", .{policyStatusText(result.status)});
    try output.print(allocator, "ready_for_next_branch: {}\n", .{result.ready_for_next_branch});
    ...
    try appendTextList(allocator, &output, "source report application changes", source.report_application_changes);
...
fn agentGuidance(status: PolicyStatus) []const []const u8 {
    return switch (status) {
        .ready => &.{
            "Use ready consumption report policy as interpretation evidence only.",
            "Agents and the SolidJS workbench may consume bounded source ids application checks denied claims and next-query guidance.",
            "Start read-only consumption report evaluator work only as a separate branch; do not infer runtime integration mutation authority CI enforcement public upload or production health.",
        },
        .blocked => &.{
            "Treat blocked consumption report policy evidence as a stop sign.",
...
fn usage() []const u8 {
    return "usage: zig build causal-app-facing-production-integration-ci-advisory-remediation-report-consumption-report-policy -- --from-application <consumption-report-application-boundary.json> approve|reject --reason <reason> [--by <actor>] [--policy <policy>] [--verified-command <command>]... [--out-prefix <path-prefix>]\n";
}

fn failUsage(err: anyerror) noreturn {
    std.debug.print("causal-app-facing-production-integration-ci-advisory-remediation-report-consumption-report-policy error: {s}\n{s}", .{ @errorName(err), usage() });
    std.process.exit(2);
}
...
fn readyApplicationBoundaryJson() []const u8 {
    ...
    \\  "schema_version": 1,
    \\  "generated_by": "causal-app-facing-production-integration-ci-advisory-remediation-report-consumption-report-application-boundary",
    ...
    \\  "report_after_path": "after-report.txt",
    \\  "report_after_digest": "sha256:after-report",
    \\  "report_after_present": true,
    \\  "source_blocked_findings_count": 0,
    \\  "source_advisory_findings_count": 0,
    \\  "report_application_changes": ["reviewed local consumption report application boundary for agents reviewers CI advisory readers and SolidJS webui"],
    \\  "before_evidence": ["before local consumption report application boundary evidence"],
    \\  "after_evidence": ["after local consumption report application boundary evidence"],
    \\  "application_checks": [
    ...
    \\  ],
    \\  "boundary_rules": ["Future consumption report policy work may interpret applied records for agents and the SolidJS workbench without creating mutation authority."],
...
test "app-facing advisory remediation report consumption report policy schema and branch constants are stable" {
...
test "parses app-facing advisory remediation report consumption report policy approval and rejection options" {
    var approve_options = try parseOptions(std.testing.allocator, &.{
        "zigeffect-causal-app-facing-production-integration-ci-advisory-remediation-report-consumption-report-policy",
        "--from-application",
        ".zig-cache/causal-artifacts/app-facing-ci-advisory-remediation-report-consumption-report-application-boundary.json",
        "approve",
        "--reason",
        "consumption report policy reviewed",
        "--by",
        "codex",
        "--policy",
        "manual-consumption-report-policy",
        "--verified-command",
        "zig build test",
        "--out-prefix",
        ".zig-cache/causal-artifacts/app-facing-ci-advisory-remediation-report-consumption-report-policy",
    });
    defer approve_options.deinit(std.testing.allocator);

    try std.testing.expectEqual(Decision.approve, approve_options.decision);
    try std.testing.expectEqualStrings(".zig-cache/causal-artifacts/app-facing-ci-advisory-remediation-report-consumption-report-application-boundary.json", approve_options.application_path);
    try std.testing.expectEqualStrings("consumption report policy reviewed", approve_options.reason);
    try std.testing.expectEqualStrings("codex", approve_options.reviewed_by);
    try std.testing.expectEqualStrings("manual-consumption-report-policy", approve_options.policy);
    ...
    try std.testing.expectEqualStrings(".zig-cache/causal-artifacts/app-facing-ci-advisory-remediation-report-consumption-report-policy", approve_options.out_prefix.?);
    ...
    var reject_options = try parseOptions(std.testing.allocator, &.{
        ...
        "reject",
        "--reason",
        "blocked consumption report policy",
    });
    defer reject_options.deinit(std.testing.allocator);

    try std.testing.expectEqual(Decision.reject, reject_options.decision);
    try std.testing.expectEqualStrings("blocked consumption report policy", reject_options.reason);
}
...
test "default output path replaces boundary suffix and compacts long names" {
    ...
    try std.testing.expectEqualStrings(
        ".zig-cache/causal-artifacts/app-facing-ci-advisory-remediation-report-consumption-report-policy.json",
        paths.json_path,
    );
    try std.testing.expectEqualStrings(
        ".zig-cache/causal-artifacts/app-facing-ci-advisory-remediation-report-consumption-report-policy.txt",
        paths.text_path,
    );
...
test "approved and rejected consumption report policy output preserve read-only authority" {
    const source_json = readyApplicationBoundaryJson();

    var approve_options = try parseOptions(std.testing.allocator, &.{
        "tool",
        "--from-application",
        "source-consumption-report-application-boundary.json",
        "approve",
        "--reason",
        "consumption report policy approved",
        "--verified-command",
        "bun run zigeffect:workbench:typecheck",
        "--verified-command",
        "bun run zigeffect:workbench:test",
    ...
    try std.testing.expect(std.mem.indexOf(u8, approve_reports.json, "\"consumption_report_policy_status\": \"ready\"") != null);
    ...
    try std.testing.expect(std.mem.indexOf(u8, approve_reports.json, "\"id\": \"future-consumption-report-evaluator-input\"") != null);
    ...
    var reject_options = try parseOptions(std.testing.allocator, &.{
        ...
        "reject",
        "--reason",
        "consumption report policy rejected",
    });
    defer reject_options.deinit(std.testing.allocator);
    ...
    try std.testing.expect(std.mem.indexOf(u8, reject_reports.json, "\"consumption_report_policy_status\": \"blocked\"") != null);
...
test "consumption report policy JSON output is parseable" {
    var approve_options = try parseOptions(std.testing.allocator, &.{
        "tool",
        "--from-application",
        "source-consumption-report-application-boundary.json",
    ...
    var parsed = try std.json.parseFromSlice(struct {
        schema: []const u8,
        consumption_report_policy_status: []const u8,
        ready_for_next_branch: bool,
    ...
    try std.testing.expectEqualStrings("ready", parsed.value.consumption_report_policy_status);
...
test "unsafe source and missing verification block approved consumption report policy" {
    var options = try parseOptions(std.testing.allocator, &.{
        ...
        "approve",
        "--reason",
        "consumption report policy approved without verification",
    });
    ...
    try std.testing.expect(std.mem.indexOf(u8, missing_verification.json, "\"consumption_report_policy_status\": \"blocked\"") != null);
    ...
    const unsafe_source = try formatReports(std.testing.allocator, .{
        ...
        \\  "report_after_digest": "sha256:after",
        \\  "report_after_present": true,
        \\  "report_application_changes": ["change"],
    ...
    try std.testing.expect(std.mem.indexOf(u8, unsafe_source.json, "\"consumption_report_policy_status\": \"blocked\"") != null);
}
