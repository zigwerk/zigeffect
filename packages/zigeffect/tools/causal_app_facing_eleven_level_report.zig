...
const Options = struct {
    ...
    reason: []const u8,
    reviewed_by: []const u8 = "app-facing-eleven-level-report-reviewer",
    policy: []const u8 = "manual-app-facing-eleven-level-report",
...
fn parseOptions(args: []const []const u8) !Options {
    ...
    var reviewed_by: []const u8 = "app-facing-eleven-level-report-reviewer";
    var policy: []const u8 = "manual-app-facing-eleven-level-report";
...
fn evaluateReport(allocator: std.mem.Allocator, source: EvaluatorArtifact) !ReportResult {
    ...
    try appendCheck(allocator, &checks, "source-evaluator-schema", if (sourceSchemaValid(source)) .pass else .fail, "source ten-level evaluator schema is supported");
    ...
    try appendCheck(allocator, &checks, "source-ready-for-next-branch", if (source.ready_for_next_branch) .pass else .fail, "source evaluator is ready for eleven-level report work");
    ...
    try appendCheck(allocator, &checks, "source-ten-level-policy-ready", if (sourcePolicyReady(source)) .pass else .fail, "source ten-level policy application report and inherited refs remain ready");
    try appendCheck(allocator, &checks, "source-ten-level-application-evidence", if (sourceReportApplicationEvidencePresent(source)) .pass else .fail, "source ten-level and inherited report application evidence next queries and local channels are present");
...
fn sourcePolicyReady(source: EvaluatorArtifact) bool {
    return sourceTenLevelPolicy(source).len > 0 and
...
fn formatReportJson(
    ...
    try appendJsonField(allocator, &output, "consumption_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_status", reportStatusText(result.status), true);
...
fn formatReportText(
    ...
    try output.appendSlice(allocator, "zigeffect app-facing eleven-level report\n");
    ...
    try output.print(allocator, "consumption report evaluation report evaluation report evaluation report evaluation report evaluation report evaluation report evaluation report evaluation report evaluation report evaluation report evaluation report status: {s}\n", .{reportStatusText(result.status)});
...
fn headlineForStatus(status: ReportStatus) []const u8 {
    return switch (status) {
        .ready => "App-facing ten-level evaluator evidence is ready for eleven-level report presentation.",
        .advisory => "App-facing ten-level evaluator evidence is reportable with advisory findings.",
        .blocked => "App-facing ten-level evaluator evidence is blocked and must not be used as reportable context.",
    };
}
...
fn usage() []const u8 {
    return "usage: zig build causal-app-facing-eleven-level-report -- --from-evaluator <ten-level-evaluator.json> summarize --reason <reason> [--by <actor>] [--policy <policy>] [--out-prefix <path-prefix>]\n";
}

fn failUsage(err: anyerror) noreturn {
    std.debug.print("causal-app-facing-eleven-level-report error: {s}\n{s}", .{ @errorName(err), usage() });
    std.process.exit(2);
}
...
test "parses app-facing eleven-level report options" {
    const options = try parseOptions(&.{
        "zigeffect-causal-app-facing-eleven-level-report",
        "--from-evaluator",
        ".zig-cache/causal-artifacts/app-facing-ci-ten-level-evaluator.json",
        "summarize",
        "--reason",
        "reviewed app-facing eleven-level report",
        "--by",
        "codex",
        "--policy",
        "manual-app-facing-eleven-level-report",
        "--out-prefix",
        ".zig-cache/causal-artifacts/custom-eleven-level-report",
    });

    try std.testing.expectEqualStrings(".zig-cache/causal-artifacts/app-facing-ci-ten-level-evaluator.json", options.evaluator_path);
    try std.testing.expectEqualStrings("reviewed app-facing eleven-level report", options.reason);
    try std.testing.expectEqualStrings("codex", options.reviewed_by);
    try std.testing.expectEqualStrings("manual-app-facing-eleven-level-report", options.policy);
    try std.testing.expectEqualStrings(".zig-cache/causal-artifacts/custom-eleven-level-report", options.out_prefix.?);
}

test "app-facing eleven-level report rejects invalid options" {
...
test "default output path replaces ten-level evaluator suffix" {
    const paths = try outputPathsForOptions(std.testing.allocator, .{
        .evaluator_path = "../../.zig-cache/causal-artifacts/example-ci-ten-level-evaluator.json",
        .reason = "reviewed",
    });
    defer paths.deinit(std.testing.allocator);

    try std.testing.expectEqualStrings("../../.zig-cache/causal-artifacts/example-ci-eleven-level-report.json", paths.json_path);
    try std.testing.expectEqualStrings("../../.zig-cache/causal-artifacts/example-ci-eleven-level-report.txt", paths.text_path);
}
...
test "ready evaluator produces ready eleven-level report" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, reports.json, "\"consumption_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_status\": \"ready\"") != null);
    ...
    try std.testing.expect(std.mem.indexOf(u8, reports.json, "\"next_branch_if_ready\": \"codex/zigeffect-causal-app-facing-eleven-level-application-boundary\"") != null);
    ...
    try std.testing.expect(std.mem.indexOf(u8, reports.text, "consumption report evaluation report evaluation report evaluation report evaluation report evaluation report evaluation report evaluation report evaluation report evaluation report evaluation report evaluation report status: ready") != null);

    var parsed = try std.json.parseFromSlice(std.json.Value, std.testing.allocator, reports.json, .{});
    parsed.deinit();
}

test "advisory evaluator produces advisory eleven-level report" {
    const reports = try formatReports(std.testing.allocator, .{
        .options = sampleOptions(),
    ...
    try std.testing.expect(std.mem.indexOf(u8, reports.json, "\"consumption_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_status\": \"advisory\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, reports.json, "\"ready_for_next_branch\": true") != null);
...
test "blocked evaluator produces blocked eleven-level report" {
    const reports = try formatReports(std.testing.allocator, .{
        .options = sampleOptions(),
    ...
    try std.testing.expect(std.mem.indexOf(u8, reports.json, "\"consumption_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_status\": \"blocked\"") != null);
...
fn sampleOptions() Options {
    return .{
        .evaluator_path = "../../.zig-cache/causal-artifacts/app-facing-ci-ten-level-evaluator.json",
        .reason = "reviewed app-facing eleven-level report",
    };
}
...
const sample_ready_evaluator_json =
    ...
    \\  "schema_version": 1,
    \\  "generated_by": "causal-app-facing-ten-level-evaluator",
