...
const Options = struct {
    ...
    reason: []const u8,
    reviewed_by: []const u8 = "app-facing-twelve-level-evaluator-reviewer",
    policy: []const u8 = "manual-app-facing-twelve-level-evaluator",
...
fn parseOptions(allocator: std.mem.Allocator, args: []const []const u8) !Options {
    ...
    var reviewed_by: []const u8 = "app-facing-twelve-level-evaluator-reviewer";
    var policy: []const u8 = "manual-app-facing-twelve-level-evaluator";
...
fn classifyFile(path: []const u8, contents: []const u8, detected_schema: []const u8) FileClass {
    ...
    if (is_json and (std.mem.indexOf(u8, path, "twelve-level-policy") != null or std.mem.indexOf(u8, path, "consumption-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-evaluation-report-policy") != null or std.mem.eql(u8, detected_schema, source_policy_schema))) return .policy_json;
...
fn sourceTwelveLevelRefsPresent(source: SourcePolicyArtifact) bool {
    return sourceTwelveLevelApplicationBoundary(source).len > 0 and
...
fn sourceEvidencePresent(source: SourcePolicyArtifact) bool {
    return source.source_twelve_level_application_changes.len > 0 and
...
fn formatEvaluatorJson(
    ...
    try appendJsonField(allocator, &output, "source_twelve_level_policy", options.policy_path, true);
    try appendJsonField(allocator, &output, "source_twelve_level_policy_schema", source.schema, true);
    try appendJsonField(allocator, &output, "source_twelve_level_policy_status", source.consumption_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_evaluation_report_policy_status, true);
    ...
    try output.appendSlice(allocator, "  \"source_twelve_level_application_changes\": ");
...
fn nextQueries(status: EvaluationStatus) []const []const u8 {
    return switch (status) {
        .ready => &.{
            "causal-query --artifact <app-facing-ci-twelve-level-evaluator.json> --kind request",
            "causal-query --artifact <app-facing-ci-twelve-level-evaluator.json> --kind denied-claim",
            "start codex/zigeffect-causal-app-facing-thirteen-level-report with this evaluator artifact",
        },
        .advisory_findings => &.{
            "add support evidence for the request and rerun the evaluator",
            "inspect advisory findings before rendering a thirteen-level report",
        },
        .blocked => &.{
...
fn agentGuidance(status: EvaluationStatus) []const []const u8 {
    return switch (status) {
        .ready => &.{
            "Ready twelve-level evaluator artifacts may feed a local thirteen-level report.",
...
fn usage() []const u8 {
    return "usage: zig build causal-app-facing-twelve-level-evaluator -- --from-policy <twelve-level-policy.json> evaluate --reason <reason> --request <path>... [--evidence <path>]... [--by <actor>] [--policy <policy>] [--out-prefix <path-prefix>]\n";
}

fn failUsage(err: anyerror) noreturn {
    std.debug.print("causal-app-facing-twelve-level-evaluator error: {s}\n{s}", .{ @errorName(err), usage() });
    std.process.exit(2);
}
...
test "parses app-facing twelve-level evaluator options with requests and evidence" {
    var options = try parseOptions(std.testing.allocator, &.{
        "zigeffect-causal-app-facing-twelve-level-evaluator",
        "--from-policy",
        ...
        "codex",
        "--policy",
        "manual-app-facing-twelve-level-evaluator",
        "--request",
        ...
        "--out-prefix",
        ".zig-cache/causal-artifacts/custom-app-facing-ci-twelve-level-evaluator",
    });
    defer options.deinit(std.testing.allocator);

    try std.testing.expectEqualStrings(".zig-cache/causal-artifacts/app-facing-ci-twelve-level-policy.json", options.policy_path);
    ...
    try std.testing.expectEqualStrings("manual-app-facing-twelve-level-evaluator", options.policy);
    ...
    try std.testing.expectEqualStrings(".zig-cache/causal-artifacts/custom-app-facing-ci-twelve-level-evaluator", options.out_prefix.?);
}
...
test "default output path replaces report policy suffix" {
    ...
    try std.testing.expectEqualStrings(".zig-cache/causal-artifacts/app-facing-ci-twelve-level-evaluator.json", paths.json_path);
    try std.testing.expectEqualStrings(".zig-cache/causal-artifacts/app-facing-ci-twelve-level-evaluator.txt", paths.text_path);
}
...
test "classifies request policy causal workbench and denied files" {
    var evidence = try analyzeFiles(std.testing.allocator, &.{
        ...
        .{ .path = ".zig-cache/causal-artifacts/app-facing-ci-twelve-level-policy.json", .contents = readyPolicyJson() },
...
test "ready advisory and blocked evaluator output preserve read-only authority" {
    var ready_options = try parseOptions(std.testing.allocator, &.{
        "tool",
        "--from-policy",
        "source-app-facing-ci-twelve-level-policy.json",
        "evaluate",
        "--reason",
    ...
    try std.testing.expect(std.mem.indexOf(u8, ready_reports.json, "\"source_twelve_level_policy\": \"source-app-facing-ci-twelve-level-policy.json\"") != null);
