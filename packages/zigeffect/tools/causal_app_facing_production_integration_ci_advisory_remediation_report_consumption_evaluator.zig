...
pub const schema = "zigeffect.causal.app-facing-production-integration-ci-advisory-remediation-report-consumption-evaluator.v1";
pub const schema_version: u32 = 1;
pub const source_branch = "codex/zigeffect-causal-app-facing-production-integration-ci-advisory-remediation-report-consumption-evaluator";
pub const recommendation = "start-app-facing-production-integration-ci-advisory-remediation-report-consumption-report";
pub const next_branch_if_ready = "codex/zigeffect-causal-app-facing-production-integration-ci-advisory-remediation-report-consumption-report";

const source_policy_schema = "zigeffect.causal.app-facing-production-integration-ci-advisory-remediation-report-consumption-policy.v1";
const generated_by = "causal-app-facing-production-integration-ci-advisory-remediation-report-consumption-evaluator";
const source_policy_suffix = "-ci-advisory-remediation-report-consumption-policy.json";
const output_prefix_suffix = "-ci-advisory-remediation-report-consumption-evaluator";
const compact_output_prefix_name = "app-facing-production-integration-ci-advisory-remediation-report-consumption-evaluator";
...
const Options = struct {
    ...
    reason: []const u8,
    reviewed_by: []const u8 = "app-facing-ci-advisory-remediation-report-consumption-evaluator-reviewer",
    policy: []const u8 = "manual-app-facing-production-integration-ci-advisory-remediation-report-consumption-evaluator",
...
const SourcePolicyArtifact = struct {
    ...
    source_consumption_boundary_status: []const u8 = "",
    ...
    source_denied_boundary_claims: []const []const u8 = &.{},
    source_blocked_claims: []const []const u8 = &.{},
...
fn parseOptions(allocator: std.mem.Allocator, args: []const []const u8) !Options {
    ...
    var reviewed_by: []const u8 = "app-facing-ci-advisory-remediation-report-consumption-evaluator-reviewer";
    var policy: []const u8 = "manual-app-facing-production-integration-ci-advisory-remediation-report-consumption-evaluator";
...
fn classifyFile(path: []const u8, contents: []const u8, detected_schema: []const u8) FileClass {
    ...
    if (is_json and (std.mem.indexOf(u8, path, "consumption-policy") != null or std.mem.eql(u8, detected_schema, source_policy_schema))) return .policy_json;
...
fn fileDeniedReason(path: []const u8, contents: []const u8, class: FileClass) []const u8 {
    ...
    if (std.mem.indexOf(u8, contents, "secrets.") != null) return "secret-shaped reference marker is not valid consumption evaluator input";
    if (std.mem.indexOf(u8, contents, "PRODUCTION_") != null) return "production secret-shaped environment marker is not valid consumption evaluator input";
    if (std.mem.indexOf(u8, contents, "BEGIN PRIVATE KEY") != null) return "private key material is not valid consumption evaluator input";
    if (std.mem.indexOf(u8, contents, "Authorization:") != null) return "authorization header material is not valid consumption evaluator input";
    if (std.mem.indexOf(u8, contents, "sk-") != null) return "OpenAI-style secret token marker is not valid consumption evaluator input";
    if (std.mem.indexOf(u8, contents, "ghp_") != null) return "GitHub token marker is not valid consumption evaluator input";
    if (std.mem.indexOf(u8, contents, "xoxb-") != null) return "Slack token marker is not valid consumption evaluator input";
    if (std.mem.indexOf(u8, contents, "\"raw_prompt\"") != null) return "raw prompt capture is not valid consumption evaluator input";
    if (std.mem.indexOf(u8, contents, "\"raw_response\"") != null) return "raw response capture is not valid consumption evaluator input";
    ...
    if (std.mem.indexOf(u8, contents, "cockroach") != null) return "Cockroach scope is not valid consumption evaluator input";
    if (std.mem.indexOf(u8, contents, "deployment success") != null or jsonBoolTrue(contents, "deployment_mutation_enabled")) return "deployment claims are not valid consumption evaluator input";
    if (std.mem.indexOf(u8, contents, "production health") != null or std.mem.indexOf(u8, contents, "production_health") != null) return "production health claims are not valid consumption evaluator input";
    ...
    if (std.mem.indexOf(u8, contents, "auto-apply") != null or std.mem.indexOf(u8, contents, "auto_apply") != null) return "auto-apply claims are not valid consumption evaluator input";
    if (std.mem.indexOf(u8, contents, "\"mutation_authority\": \"granted\"") != null) return "mutation authority must remain none";
    if (std.mem.indexOf(u8, contents, "artifact_visibility=public") != null) return "public artifact upload claims are not valid consumption evaluator input";
    _ = path;
    return "";
}
...
fn evaluatePolicyAndFiles(
    ...
    try appendCheckAndFinding(allocator, &checks, &findings, "source-policy-schema", sourceSchemaVersionSupported(source), "source-policy-valid", "source consumption policy schema is supported");
    try appendCheckAndFinding(allocator, &checks, &findings, "source-policy-ready", std.mem.eql(u8, source.consumption_policy_status, "ready"), "source-policy-valid", "source consumption policy status is ready");
    try appendCheckAndFinding(allocator, &checks, &findings, "source-ready-for-next-branch", source.ready_for_next_branch, "source-policy-valid", "source consumption policy is ready for evaluator handoff");
    try appendCheckAndFinding(allocator, &checks, &findings, "source-review-approved", std.mem.eql(u8, source.decision, "approve"), "source-policy-valid", "source consumption policy review decision is approve");
    try appendCheckAndFinding(allocator, &checks, &findings, "source-mutation-authority-none", std.mem.eql(u8, source.mutation_authority, "none"), "source-policy-valid", "source mutation authority remains none");
    try appendCheckAndFinding(allocator, &checks, &findings, "source-refs-present", sourceRefsPresent(source), "source-policy-valid", "source boundary readiness publication and digest refs are present");
    try appendCheckAndFinding(allocator, &checks, &findings, "source-policy-checks-passed", sourcePolicyChecksPass(source.policy_checks), "source-policy-valid", "source policy checks contain no failures");
    try appendCheckAndFinding(allocator, &checks, &findings, "source-catalogs-present", sourceCatalogsPresent(source), "source-policy-valid", "source interpretation rules scopes denied claims fixtures and verification catalogs are present");
    try appendCheckAndFinding(allocator, &checks, &findings, "source-authority-disabled", sourceAuthorityDisabled(source), "source-policy-valid", "source keeps CI GitHub app runtime storage deployment and adapter authority disabled");
    try appendCheckAndFinding(allocator, &checks, &findings, "source-solid-webui", sourceSolidWebuiValid(source), "source-policy-valid", "source remains SolidJS inside webui-dev/zig-webui with read-only consumption enabled");
    try appendCheckAndFinding(allocator, &checks, &findings, "request-files-present", requests.files.len > 0, "request-present", "explicit read-only consumption request files are present");
    ...
    try appendSignal(allocator, &signals, &findings, "source-policy-valid", if (source_valid) .observed else .blocked, if (source_valid) "source policy is ready approved and non-mutating" else "source policy failed evaluator preconditions", "");
    try appendSignal(allocator, &signals, &findings, "request-present", if (has_request) .observed else .blocked, if (has_request) "safe read-only consumption request is present" else "safe read-only consumption request is missing", firstFilePath(requests));
...
fn sourceRefsPresent(source: SourcePolicyArtifact) bool {
    return source.source_consumption_boundary.len > 0 and
        std.mem.eql(u8, source.source_consumption_boundary_status, "applied") and
        ...
        source.source_consumer_after_digest.len > 0;
}
...
fn sourceCatalogsPresent(source: SourcePolicyArtifact) bool {
    return source.interpretation_rules.len > 0 and
        ...
        source.source_denied_boundary_claims.len > 0 and
        source.source_blocked_claims.len > 0 and
...
fn sourceDeniedClaimsPresent(source: SourcePolicyArtifact) bool {
    return source.denied_inference_rules.len > 0 and source.source_denied_boundary_claims.len > 0 and source.source_blocked_claims.len > 0;
}
...
fn formatEvaluatorJson(
    ...
    try appendJsonField(allocator, &output, "source_consumption_policy", options.policy_path, true);
    try appendJsonField(allocator, &output, "source_consumption_policy_schema", source.schema, true);
    try appendJsonField(allocator, &output, "source_policy_status", source.consumption_policy_status, true);
    ...
    try appendJsonField(allocator, &output, "source_consumption_boundary_status", source.source_consumption_boundary_status, true);
    ...
    try appendJsonField(allocator, &output, "evaluation_mode", "read-only-consumption", true);
...
fn appendCombinedDeniedClaimsJson(allocator: std.mem.Allocator, output: *std.ArrayList(u8), source: SourcePolicyArtifact) !void {
    ...
    for (source.source_denied_boundary_claims) |claim| {
        if (written > 0) try output.appendSlice(allocator, ", ");
        try appendJsonString(allocator, output, claim);
        written += 1;
    }
    for (source.source_blocked_claims) |claim| {
        if (written > 0) try output.appendSlice(allocator, ", ");
        try appendJsonString(allocator, output, claim);
        written += 1;
    }
...
fn formatEvaluatorText(
    ...
    try output.appendSlice(allocator, "app-facing CI advisory remediation report consumption evaluator\n");
    ...
    try output.print(allocator, "source_policy_status: {s}\n", .{source.consumption_policy_status});
...
fn nextQueries(status: EvaluationStatus) []const []const u8 {
    return switch (status) {
        .ready => &.{
            "causal-query --artifact <consumption-evaluator.json> --kind request",
            "causal-query --artifact <consumption-evaluator.json> --kind denied-claim",
            "start consumption-report branch with this evaluator artifact",
        },
        .advisory_findings => &.{
            "add support evidence for the request and rerun the evaluator",
            "inspect advisory findings before rendering a consumption report",
        },
        .blocked => &.{
...
fn agentGuidance(status: EvaluationStatus) []const []const u8 {
    return switch (status) {
        .ready => &.{
            "Ready consumption evaluator artifacts may feed a local consumption report.",
            "Use only bounded redacted source ids policy rule ids denied claims and next-query guidance.",
            "Do not infer app runtime integration mutation authority required checks GitHub mutation NenDB writes adapter execution deployment or production health.",
        },
        .advisory_findings => &.{
...
fn usage() []const u8 {
    return "usage: zig build causal-app-facing-production-integration-ci-advisory-remediation-report-consumption-evaluator -- --from-policy <consumption-policy.json> evaluate --reason <reason> --request <path>... [--evidence <path>]... [--by <actor>] [--policy <policy>] [--out-prefix <path-prefix>]\n";
}

fn failUsage(err: anyerror) noreturn {
    std.debug.print("causal-app-facing-production-integration-ci-advisory-remediation-report-consumption-evaluator error: {s}\n{s}", .{ @errorName(err), usage() });
    std.process.exit(2);
}
...
fn readyPolicyJson() []const u8 {
    return
        ...
        \\  "source_denied_boundary_claims": ["app-runtime-integration-proof"],
        \\  "source_blocked_claims": ["mutation-authority-granted"],
        \\  "negative_fixtures": [
...
test "app-facing advisory remediation report consumption evaluator schema and branch constants are stable" {
...
test "parses app-facing consumption evaluator options with requests and evidence" {
    var options = try parseOptions(std.testing.allocator, &.{
        "zigeffect-causal-app-facing-production-integration-ci-advisory-remediation-report-consumption-evaluator",
        "--from-policy",
        ".zig-cache/causal-artifacts/app-facing-ci-advisory-remediation-report-consumption-policy.json",
        "evaluate",
        "--reason",
        "consumption request evaluated",
        "--by",
        "codex",
        "--policy",
        "manual-consumption-evaluator",
        "--request",
        ...
        "--out-prefix",
        ".zig-cache/causal-artifacts/custom-consumption-evaluator",
    });
    defer options.deinit(std.testing.allocator);

    try std.testing.expectEqualStrings(".zig-cache/causal-artifacts/app-facing-ci-advisory-remediation-report-consumption-policy.json", options.policy_path);
    try std.testing.expectEqualStrings("consumption request evaluated", options.reason);
    try std.testing.expectEqualStrings("codex", options.reviewed_by);
    try std.testing.expectEqualStrings("manual-consumption-evaluator", options.policy);
    ...
    try std.testing.expectEqualStrings(".zig-cache/causal-artifacts/custom-consumption-evaluator", options.out_prefix.?);
}

test "default output path replaces consumption policy suffix and compacts long names" {
    ...
    try std.testing.expectEqualStrings(
        ".zig-cache/causal-artifacts/app-facing-ci-advisory-remediation-report-consumption-evaluator.json",
        paths.json_path,
    );
    try std.testing.expectEqualStrings(
        ".zig-cache/causal-artifacts/app-facing-ci-advisory-remediation-report-consumption-evaluator.txt",
        paths.text_path,
    );
}
...
test "classifies request policy causal workbench and denied files" {
    var evidence = try analyzeFiles(std.testing.allocator, &.{
        ...
        .{ .path = ".zig-cache/causal-artifacts/app-facing-consumption-policy.json", .contents = readyPolicyJson() },
...
test "ready advisory and blocked evaluator output preserve read-only authority" {
    var ready_options = try parseOptions(std.testing.allocator, &.{
        "tool",
        "--from-policy",
        "source-consumption-policy.json",
        "evaluate",
        "--reason",
        "ready consumption request",
        "--request",
        ".zig-cache/causal-artifacts/app-facing-consumption-request.json",
        "--evidence",
