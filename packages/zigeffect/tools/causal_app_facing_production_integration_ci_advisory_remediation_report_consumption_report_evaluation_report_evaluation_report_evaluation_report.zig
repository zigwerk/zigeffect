...
pub const schema = "zigeffect.causal.app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-evaluation-report.v1";
pub const schema_version: u32 = 1;
pub const source_branch = "codex/zigeffect-causal-app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-evaluation-report";
pub const recommendation = "start-app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-evaluation-report-application-boundary";
pub const next_branch_if_ready = "codex/zigeffect-causal-app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-evaluation-report-application-boundary";

const source_evaluator_schema = "zigeffect.causal.app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-evaluator.v1";
const generated_by = "causal-app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-evaluation-report";
const evaluator_suffix = "-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-evaluator.json";
const output_suffix = "-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-evaluation-report";
...
const Options = struct {
    ...
    reason: []const u8,
    reviewed_by: []const u8 = "app-facing-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-evaluation-report-reviewer",
    policy: []const u8 = "manual-app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-evaluation-report",
...
const publication_channels: []const PublicationChannel = &.{
    ...
    .{ .id = "required-status-check", .allowed = false, .executed_by_tool = false, .detail = "Required status checks remain outside this app-facing consumption report evaluation report evaluation report evaluation report." },
...
fn parseOptions(args: []const []const u8) !Options {
    ...
    var reviewed_by: []const u8 = "app-facing-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-evaluation-report-reviewer";
    var policy: []const u8 = "manual-app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-evaluation-report";
...
fn evaluateReport(allocator: std.mem.Allocator, source: EvaluatorArtifact) !ReportResult {
    ...
    try appendCheck(allocator, &checks, "source-evaluator-schema", if (sourceSchemaValid(source)) .pass else .fail, "source consumption-report evaluation-report evaluation-report evaluator schema is supported");
    ...
    try appendCheck(allocator, &checks, "source-ready-for-next-branch", if (source.ready_for_next_branch) .pass else .fail, "source evaluator is ready for consumption report evaluation report evaluation report evaluation report work");
    ...
    try appendCheck(allocator, &checks, "source-evaluation-report-policy-ready", if (sourcePolicyReady(source)) .pass else .fail, "source evaluation-report evaluation-report policy and application refs remain ready");
    try appendCheck(allocator, &checks, "source-evaluation-report-application-evidence", if (sourceReportApplicationEvidencePresent(source)) .pass else .fail, "source evaluation-report evaluation-report and inherited report application evidence next queries and local channels are present");
...
fn sourcePolicyReady(source: EvaluatorArtifact) bool {
    return source.source_consumption_report_evaluation_report_evaluation_report_policy.len > 0 and
...
fn formatReportJson(
    ...
    try appendJsonField(allocator, &output, "consumption_report_evaluation_report_evaluation_report_evaluation_report_status", reportStatusText(result.status), true);
...
fn formatReportText(
    ...
    try output.appendSlice(allocator, "zigeffect app-facing CI advisory remediation report consumption report evaluation report evaluation report evaluation report\n");
    ...
    try output.print(allocator, "source evaluation report evaluation report policy: {s}\n", .{source.source_consumption_report_evaluation_report_evaluation_report_policy});
    try output.print(allocator, "source evaluation report evaluation report policy status: {s}\n", .{source.source_evaluation_report_evaluation_report_policy_status});
    ...
    try output.print(allocator, "consumption report evaluation report evaluation report evaluation report status: {s}\n", .{reportStatusText(result.status)});
...
fn headlineForStatus(status: ReportStatus) []const u8 {
    return switch (status) {
        .ready => "App-facing consumption report evaluation-report evaluation-report evaluator evidence is ready for report presentation.",
        .advisory => "App-facing consumption report evaluation-report evaluation-report evaluator evidence is reportable with advisory findings.",
        .blocked => "App-facing consumption report evaluation-report evaluation-report evaluator evidence is blocked and must not be used as reportable context.",
    };
}
...
fn usage() []const u8 {
    return "usage: zig build causal-app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-evaluation-report -- --from-evaluator <consumption-report-evaluation-report-evaluation-report-evaluator.json> summarize --reason <reason> [--by <actor>] [--policy <policy>] [--out-prefix <path-prefix>]\n";
}

fn failUsage(err: anyerror) noreturn {
    std.debug.print("causal-app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-evaluation-report error: {s}\n{s}", .{ @errorName(err), usage() });
    std.process.exit(2);
}

test "app-facing advisory remediation report consumption report evaluation report evaluation report evaluation report constants are stable" {
    try std.testing.expectEqualStrings("zigeffect.causal.app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-evaluation-report.v1", schema);
    try std.testing.expectEqual(@as(u32, 1), schema_version);
    try std.testing.expectEqualStrings("codex/zigeffect-causal-app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-evaluation-report", source_branch);
    try std.testing.expectEqualStrings("start-app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-evaluation-report-application-boundary", recommendation);
    try std.testing.expectEqualStrings("codex/zigeffect-causal-app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-evaluation-report-application-boundary", next_branch_if_ready);
}

test "parses app-facing consumption report evaluation report evaluation report evaluation report options" {
    const options = try parseOptions(&.{
        "zigeffect-causal-app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-evaluation-report",
        "--from-evaluator",
        ...
        "summarize",
        "--reason",
        "reviewed app-facing consumption report evaluation report evaluation report evaluation report",
        "--by",
        "codex",
        "--policy",
        "manual-app-facing-consumption-report-evaluation-report-evaluation-report-evaluation-report",
        "--out-prefix",
        ".zig-cache/causal-artifacts/custom-consumption-report-evaluation-report-evaluation-report-evaluation-report",
    });

    try std.testing.expectEqualStrings(".zig-cache/causal-artifacts/app-facing-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-evaluator.json", options.evaluator_path);
    try std.testing.expectEqualStrings("reviewed app-facing consumption report evaluation report evaluation report evaluation report", options.reason);
    try std.testing.expectEqualStrings("codex", options.reviewed_by);
    try std.testing.expectEqualStrings("manual-app-facing-consumption-report-evaluation-report-evaluation-report-evaluation-report", options.policy);
    try std.testing.expectEqualStrings(".zig-cache/causal-artifacts/custom-consumption-report-evaluation-report-evaluation-report-evaluation-report", options.out_prefix.?);
}

test "consumption report evaluation report evaluation report evaluation report rejects invalid options" {
...
test "default output path replaces consumption report evaluator suffix" {
    const paths = try outputPathsForOptions(std.testing.allocator, .{
        .evaluator_path = "../../.zig-cache/causal-artifacts/example-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-evaluator.json",
        .reason = "reviewed",
    });
    defer paths.deinit(std.testing.allocator);

    try std.testing.expectEqualStrings("../../.zig-cache/causal-artifacts/example-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-evaluation-report.json", paths.json_path);
    try std.testing.expectEqualStrings("../../.zig-cache/causal-artifacts/example-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-evaluation-report.txt", paths.text_path);
}

test "ready evaluator produces ready consumption report evaluation report evaluation report evaluation report" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, reports.json, "\"schema\": \"zigeffect.causal.app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-evaluation-report.v1\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, reports.json, "\"consumption_report_evaluation_report_evaluation_report_evaluation_report_status\": \"ready\"") != null);
    ...
    try std.testing.expect(std.mem.indexOf(u8, reports.json, "\"next_branch_if_ready\": \"codex/zigeffect-causal-app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-evaluation-report-application-boundary\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, reports.text, "consumption report evaluation report evaluation report evaluation report status: ready") != null);

    var parsed = try std.json.parseFromSlice(std.json.Value, std.testing.allocator, reports.json, .{});
    parsed.deinit();
}

test "advisory evaluator produces advisory consumption report evaluation report evaluation report evaluation report" {
    const reports = try formatReports(std.testing.allocator, .{
        .options = sampleOptions(),
    ...
    try std.testing.expect(std.mem.indexOf(u8, reports.json, "\"consumption_report_evaluation_report_evaluation_report_evaluation_report_status\": \"advisory\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, reports.json, "\"ready_for_next_branch\": true") != null);
...
test "blocked evaluator produces blocked consumption report evaluation report evaluation report evaluation report" {
    const reports = try formatReports(std.testing.allocator, .{
        .options = sampleOptions(),
    ...
    try std.testing.expect(std.mem.indexOf(u8, reports.json, "\"consumption_report_evaluation_report_evaluation_report_evaluation_report_status\": \"blocked\"") != null);
...
fn sampleOptions() Options {
    return .{
        .evaluator_path = "../../.zig-cache/causal-artifacts/app-facing-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-evaluator.json",
        .reason = "reviewed app-facing consumption report evaluation report evaluation report evaluation report",
    };
}
...
const sample_ready_evaluator_json =
    ...
    \\  "schema_version": 1,
    \\  "generated_by": "causal-app-facing-production-integration-ci-advisory-remediation-report-consumption-report-evaluation-report-evaluation-report-evaluator",
