...
pub fn Cause(comptime Failure: type) type {
    return union(enum) {
        const Self = @This();
        pub const FailureThenFinalizerFailure = struct {
            failure: Failure,
            finalizer_failure: []const u8,
        };
        pub const DefectThenFinalizerFailure = struct {
            defect: []const u8,
            finalizer_failure: []const u8,
        };
        pub const InterruptedThenFinalizerFailure = struct {
            fiber_id: u64,
            finalizer_failure: []const u8,
        };
        ...
        interrupted: u64,
        finalizer_failure: []const u8,
        failure_then_finalizer_failure: FailureThenFinalizerFailure,
        defect_then_finalizer_failure: DefectThenFinalizerFailure,
        interrupted_then_finalizer_failure: InterruptedThenFinalizerFailure,
        sequential: Sequential,
        parallel: Parallel,
        annotated: Annotated,

        pub fn match(self: Self, comptime Return: type, handlers: anytype) Return {
            return match_mod.exhaustive(Return, self, handlers);
        }
    };
}
