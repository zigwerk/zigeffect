...
pub const Metrics = struct {
    ...
    pub fn increment(self: *Metrics, name: []const u8, amount: i64) Allocator.Error!void {
        ...
        try self.counters.put(owned_name, amount);
    }
    ...
    pub fn gauge(self: *Metrics, name: []const u8, value: i64) Allocator.Error!void {
        ...
        const owned_name = try self.allocator.dupe(u8, name);
        errdefer self.allocator.free(owned_name);
        try self.counters.put(owned_name, value);
    }
    ...
    pub fn observe(self: *Metrics, name: []const u8, value: i64) Allocator.Error!void {
        ...
        const owned_name = try self.allocator.dupe(u8, name);
        errdefer self.allocator.free(owned_name);
        try self.histograms.put(owned_name, histogram_value);
    }
...
test "Metrics releases owned keys on every allocation failure" {
    const Harness = struct {
        fn run(allocator: Allocator) !void {
            var metrics = Metrics.init(allocator);
            defer metrics.deinit();
            try metrics.increment("requests.total", 1);
            try metrics.gauge("workers", 2);
            try metrics.observe("latency", 3);
        }
    };
