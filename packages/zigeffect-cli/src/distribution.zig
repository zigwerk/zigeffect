...
pub fn completionScript(shell: Shell) []const u8 {
    return switch (shell) {
        ...
        \\_zigeffect_complete() {
        \\  local commands="new add generate project safety agent benchmark compatibility upgrade completions help version"
        ...
        \\  COMPREPLY=( $(compgen -W "--root --json --jsonl --dry-run --apply --force --target --component --provider --command" -- "${COMP_WORDS[COMP_CWORD]}") )
        \\}
        ...
        \\_zigeffect() {
        \\  local -a commands
        \\  commands=(new add generate project safety agent benchmark compatibility upgrade completions help version)
        ...
        \\complete -c zigeffect -n '__fish_use_subcommand' -a 'new add generate project safety agent benchmark compatibility upgrade completions help version'
