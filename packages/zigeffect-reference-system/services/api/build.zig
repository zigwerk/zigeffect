...
pub fn build(b: *std.Build) void {
    ...
    const zigeffect_s3 = b.dependency("zigeffect_s3", .{ .target = target, .optimize = optimize }).module("zigeffect_s3");
    ...
    app.addImport("zigeffect_s3", zigeffect_s3);
    ...
    const executable = b.addExecutable(.{ .name = "api", .root_module = main_module });
    b.installArtifact(executable);

    const run = b.addRunArtifact(executable);
    if (b.args) |args| run.addArgs(args);
    const run_step = b.step("run", "Run api locally");
    ...
    const tests = b.addTest(.{ .name = "api-tests", .root_module = test_module, .test_runner = .{ .path = testing_runner, .mode = .server } });
    const test_step = b.step("test", "Run api tests");
