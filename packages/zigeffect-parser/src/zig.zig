...
pub const schema = "zigeffect.zig-structural-facts.v1";
pub const schema_version: u32 = 1;
pub const parser_id = "zigeffect-parser.std-zig-ast";
pub const parser_version = "std.zig.Ast-0.16-v1";
...
pub fn parse(
    ...
    if (tree.nodes.len > options.max_nodes) return error.ZigNodeLimitExceeded;
    try validateSyntaxDepth(&tree, options.max_depth);
...
pub fn validate(result: *const Result) !void {
    if (!result.owns_memory or !validPath(result.path) or result.source_bytes == 0 or
...
fn validateOptions(path: []const u8, source: []const u8, options: Options) !void {
    ...
    {
        return error.InvalidZigParserOptions;
    }
    if (source.len == 0) return error.InvalidZigSource;
    if (source.len > options.max_source_bytes) return error.ZigSourceLimitExceeded;
}

fn validateSyntaxDepth(tree: *const std.zig.Ast, max_depth: usize) !void {
    var depth: usize = 0;
    var token_integer: usize = 0;
    while (token_integer < tree.tokens.len) : (token_integer += 1) {
        const token: std.zig.Ast.TokenIndex = @intCast(token_integer);
        switch (tree.tokenTag(token)) {
            .l_paren, .l_brace, .l_bracket => {
                depth = std.math.add(usize, depth, 1) catch return error.ZigDepthLimitExceeded;
                if (depth > max_depth) return error.ZigDepthLimitExceeded;
            },
            .r_paren, .r_brace, .r_bracket => {
                if (depth == 0) return error.InvalidZigSource;
                depth -= 1;
            },
            else => {},
        }
